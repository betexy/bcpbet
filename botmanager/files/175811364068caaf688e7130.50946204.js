(function () {
    ...
    let pageLoadedAt = 0;
    let loaded = Date.now();
    const loginSelector = 'button:textEquals("Log in")';
    const bkHere = document.location.href.indexOf('fairspin') > -1 ? 'fairspin' : 'weiss';
    const bkIndex = document.location.href.indexOf('fairspin') > -1 ? 'Fairspin' : 'Weiss';
    ...
    function getBalance(returnNull) {
        const $b = $('span.auth-wallet__balance-amount');
        return $b.length === 0
    ...
    const switchCurrency = async () => {
        if ($('div.auth-wallet svg[alt="USDT"]').length === 0) {
            const $chat = $('span.auth-wallet__balance-amount:visible');
            if ($chat.length > 0) {
                await mouseChain({
                    target: $chat[0],
                ...
                const $usdt = $('div.auth-wallet-currency__currency svg[alt="USDT"]');

                if ($usdt.length > 0) {
                    await mouseChain({target: $usdt[0], events: fullClick, error: 'usdt!',})
                }
            }
        }
    };
    ...
    const switchLang = async () => {
        if (busy === false
            && $('div.settings').length > 0
        ) {
            if ($('div.settings-title:textEquals("Settings")').length === 0) {
                await selectLikePuppeteer($('#language-select')[0], ['en']);
    ...
    const authCheck = function () {
        ...
        (async () => {
            ...
            const $bonus = await waitForElement('div.header__authorized-profile', 333, 5555).catch(() => $([]));
            if ($(loginSelector).length > 0 && Date.now() - pageLoadedAt < 30000) {
            ...
            .catch(e => dLog('red', bkHere, `authCheck: ${e}`))
    ...
    const goToSport = async () => {
        ...
        if (document.location.href.indexOf('/sport/play') === -1) {
            await mouseChain({
                target: $('a[href="/sport/play"]:visible')[0],
                events: fullClick,
                error: 'sport-link'
            });
            await delayPromise(555);
    ...
    const tryToLogin = async () => {
        ...
        const controls = {
            ...
            submit: 'button.auth-sign-in__submit:textEquals("Log in")',
        };
        ...
        dLog('blue', bkHere, `Auth clicked ${authClicked}!`);
        await delayPromise(777);
        const $errorMessage = await waitForElement('div.auth-view-common-error__title:visible', 222, 5555).catch(() => $([]));

        if ($errorMessage.length > 0) {
            enterError = true;
            await mouseChain({target: $('button.auth-view-common-error__close')[0], events: fullClick, error: 'Auth error close!'});
            throw 'Auth error!';
        }
    };
    ...
    const checkEventName = (eventNameIn, controlNameIn) => {
        ...
        dLog('green', bkHere, `Event name '${eventName}' ${res ? '==' : '!='} '${controlName}'`);
        return res;
    };

    const openEvent = async data => {
        dLog('red', bkHere, ['openEvent', data]);
        let $el = $([]);
        let parseLink;
        if (!!data.direct_link) {
        ...
        const findEventClassic = async sport => {
            ...
            $('div.grid-line__content div.grid-event').each(function () {
                ...
                dLog('blue', bkHere, `Check: '${checkEvent}' === '${eventName}'`);
        ...
        const findEvent = async sport => {
            const searchStr = 'i.iconFont-search-pro';
            ...
            const $sle = await waitForElement(`section.search-popover div.grid-el-event${type}`,
                333, 15000);
            $sle.each(function () {
                const
                    $t = $(this),
                    team1 = $t.find('div.grid-el-event__competitor-name').eq(0).trt(),
                    team2 = $t.find('div.grid-el-event__competitor-name').eq(1).trt(),
                    checkEvent = `${team1} - ${team2}`;
                if (checkEventName(checkEvent, eventName)) {
                    $el = $t.find('a.grid-el-event__competitors');
                    return false;
                }
            });
            return $el.length > 0;
        ...
        if (!!data.direct_link && document.location.href !== parseLink) {
            dLog('yellow', bkHere, `We got direct link: ${parseLink}`);
    ...
    const getBetElement = async bet => {
        ...
        dLog('green', bkHere, ['Final market is:', m]);
    ...
    const openCoupon = async paramData => {
        dLog('green', bkHere, ['openCoupon, paramData:', paramData]);
        const saveIdx = await bMess('couponIndex')
            .check(1888, true)
            .catch(() => 0);
        dLog('blue', bkHere, ['saveIdx:', saveIdx]);
        for (let i = saveIdx; i < paramData.length; i++) {
            await bMess('couponIndex').set(i);
            const data = paramData[i];
            dLog('green', bkHere, [`openCoupon ITERATION ${i}- we using data ${(typeof data)}:`, data]);
            await openEvent(data);
            dLog('green', bkHere, 'Event must be opened!');
    ...
    const proceedBetSport = async (data, balance) => {
        ...
        do {
            ...
            dLog('green', bkHere, `Will place (performBet): ${place}, balance: ${balance}`);
            ...
            if (parseFloat($input().val()) !== parseFloat(place)) {
                ...
                await delayPromise(777);
                dLog('green', bkHere, `STAKE entered ${willPlace}`);
            }
            let entered = parseFloat($input().val());
            dLog('green', bkHere, `After enter stake check: willPlace = ${willPlace}, entered: ${entered}`);
            if (isNaN(entered) || willPlace !== entered) {
                dLog('red', bkHere, 'Entered !== willPlace - try to reenter!');
                continue;
            }
            await delayPromise(555);
        ...
        const $firstBetRes = await waitForElement('div.mybets-list__item:first',
            333, 9999, true);
        const extID = $firstBetRes.find('div.mybets-list__item-betid').trt().replace(/[^-\d.]/g, '');
        await delayPromise(777);
        // close my bets
    ...
    const collectBetResultsSports = async (inD) => {
        ...
        if ($('div.modal div.modal__box-mybets').length === 0) {
            ...
            dLog('green', bkHere, 'My bets clicked!');
        } else {
            dLog('green', bkHere, 'Already on my bets page!');
        }
        await delayPromise(555);
        let checked = 0;
        ...
        for (const tab of ['Unsettled', 'Settled']) {
            ...
            if ($tab.attr('class').indexOf('_active') === -1) {
                ...
                dLog('green', bkHere, `Tab ${tab} clicked!`);
                await delayPromise(777);
            } else {
                dLog('green', bkHere, `Tab ${tab} already selected!`);
                await delayPromise(777);
            }
            const
            ...
            if ($(noResSel).length > 0) {
                dLog('red', bkHere, `No results in the tab ${tab}!`);
                continue;
            }
            ...
            dLog('', bkHere, [`${tab}, checked: ${checked}, collected:`, collected]);
        }
    ...
    const getRegistered = async (data, command, balance, testing) => {
        ...
        if (testing) {
            dLog('green', bkHere, `getRegistered testing!`);
    ...
    const sportCommands = new class SportCommands {
        ...
        async execute(command, data, balance) {
            dLog('green', bkHere, [`SportProcessor execute: ${command}`, data]);
            ...
            dLog(res.success ? 'green' : 'red', bkHere, [`${command} sports result was set:`, res]);
            await bMess(bkIndex + 'Result').set(res);
            return res;
        }
    };

    const sportProcessor = command => {
        dLog('green', bkHere, `SportProcessor: ${command.action}`);
        if (sportCommands.exists(command.action)) {
            ...
            dLog('red', bkHere, ['Unknown Sport command:', command]);
        }
    };

    const executeSportCommand = async (data, command, balance, testing) => {
        dLog('green', bkHere, `executeSportCommand: ${command}, ${balance}`);
        ...
        if (command === 'BET' && !!currentBetData.data[0].betFromParser) {
            ...
            if (!await eventsWork(bkHere, settings, eventName, false, true)) {
        ...
        if (!!testing) {
            dLog('orange', bkHere, [`execute command:`, command, `settings:`, settings]);
            currentBetData.data[0].betFromParser = true;
            dLog('orange', bkHere, 'TESTING - simple wait 10 secs');
            await delayPromise(10000);
            return {
                success: true,
                message: {
        ...
        await bMess(bkIndex + 'Sport').set({action: command, data, balance});
        dLog('yellow', bkHere, [`Start waiting for command: ${command}`, data, balance]);
        return await bMess(bkIndex + 'Result')
    ...
    const commands = new class commands {
        constructor() {
            ...
            if (this.testing) {
                dLog('bigger-red', bkHere, [`TESTING MODE ${window.self === window.top} at:`,
        ...
        async execute(command, data) {
            ...
            if (command === 'REGISTER_NEW') {
                ...
                dLog('', bkHere, [`execute REGISTER_NEW, ${this.wasRegister}:`,
                    this.register]);
            }
            ...
            dLog(res.success ? 'green' : 'red', bkHere, [`${command} result:`, res]);
            this.prepareResult(command, res)
                .then(m => {
                    dLog('orange', bkHere, ['Prepared result: ', m]);
                    port.postMessage(m);
                })
            if (!res.success) {
                throw res.message;
        ...
        async prepareResult(command, res) {
            dLog('orange', bkHere, [`prepareResult for ${command}`, res]);
            if (['BET', 'EXPRESS_BET', 'FORK_BET'].indexOf(command) > -1) {
                ...
                if (!!currentBetData.data[0].betFromParser) {
                    ...
                    resultData.bookmaker = bkHere.toUpperCase();
    ...
    const messageProcessor = message => {
        dLog('green', bkHere, [`messageProcessor (${busy})`, message]);
    ...
    addEventListener("unload", function () {
        if (ourCommand.isSet()) {
            let idx = document.location.href.indexOf('hub88b-ytuoyswg.betsy.gg') > -1
                ? bkIndex + 'Sport' : bkIndex + 'Command';
            bMess(idx)
                .set(ourCommand.get(), increaseDelay ? 150000 : 0)
                .then(() => {
                    dLog('green', bkHere, [`Command (${idx}) was set till unload:`,
                        ourCommand.get()]);
                });
        }
    }, true);
    ...
    function afterDOMLoaded() {
        ...
        const sportPage = bkHere === 'fairspin' ? 'fairspin-2-cei938sj.sportbook.work/?' : 'weiss-dk37ncgy.sportbook.work';
        if (window.self === window.top) {
            // Hint: main window (outer with auth, menu, balance, etc.)
            pageLoadedAt = Date.now();
            bMess(bkIndex + 'Command').check(40000, true)
                .then(currentCommand => {
                    dLog('orange', bkHere, [
            ...
            dLog('green', bkHere, 'Sport processor initialized!');
        } else {
            dLog('', bkHere, `afterDOMLoaded ${document.location.href}`);
        }
    }
})();
