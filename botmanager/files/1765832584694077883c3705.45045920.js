(() => {
    ...
    const port = isMain ? chrome.runtime.connect({name: 'port_cryptocasino'})
        : {postMessage: (...args) => console.log(args)};
    const settings = {
        restartEvery: 1800000,
        authCheckInterval: 2000,
        url: 'https://www.cryptocasino.com/sports',
    ...
    const
        ...
        shadowRoot = () => $(btSel) && $(btSel).length > 0
            ? $(btSel)[0].shadowRoot
            : null,
        loginLink = 'button:textEquals("Log in"):visible';
    ...
    const authCheck = () => {
        ...
        (async () => {
            ...
            if ($('pre').length > 0) {
                window.location.href = 'https://www.cryptocasino.com/sports';
            }

            const $loginLink = await waitForElement(loginLink, 300, 2333, true).catch(() => $([]));
            if (!limited && $loginLink.length > 0) {
                ...
                if (settings.lastScoreBasketball === '999') {
                    if (Date.now() - settings.sourceDate >= 300000) {
                        ...
                        dLog('blue', 'CRYPTOCASINO', `Source current random value - ${settings.sourceRandom}`);
                    }
                }
                ...
                if (settings.newExpresses && !busy) {
                    ...
                    if (!WasSuccessExpressNew) {
                        dLog('green', 'CRYPTOCASINO', 'START find NewExpress event!');
                        // clear coupons
                        if ($coupons().length > 0) {
                        ...
                        await ProccedExpressNew().catch((e) => {
                            dLog('red', 'CRYPTOCASINO', 'ProccedExpressNew Error - ' + e);
            ...
            .catch(e => dLog('red', 'CRYPTOCASINO', `authCheck: ${e}`))
    ...
    const tryToLogin = async () => {
        lang = document.location.href.indexOf('cryptocasino.com/sports/ru') > -1 ? 'ru' : 'eng';
        if (Date.now() - authClickedTime < 60000) {
            throw 'too soon!';
        }
        const profile = `div[class*="BalanceComponent-module_wrapper_"]:visible`;
        ...
        await clearAndSimulate($('input[name="email"]')[0],
            settings.login);
        await delayPromise(777);
        await clearAndSimulate($('input[name="password"]')[0],
            settings.password);
        await delayPromise(777);
        await mouseChain({
        ...
        dLog('green', 'CRYPTOCASINO', `Auth clicked ${authClicked}!`);
        if ($profile.length === 0) {
            port.postMessage({
                answered: "auth_error",
                status: "ERROR",
    ...
    const checkEventName = (eventNameIn, controlNameIn) => {
        ...
        dLog('green', 'CRYPTOCASINO', `Event name '${eventName}' ${res ? '==' : '!='} '${controlName}'`);
        return res;
    };

    const openEvent = async data => {
        dLog('red', 'CRYPTOCASINO', ['openEvent', data]);
        ...
        const findEvent = async () => {
            ...
            $events(timeCnd).each(function () {
                ...
                dLog('blue', 'CRYPTOCASINO', `Check: '${checkEvent}' === '${eventName}'`);
    ...
    const openCoupon = async paramData => {
        dLog('green', 'CRYPTOCASINO', ['openCoupon, paramData:', paramData]);
        for (let i = 0; i < paramData.length; i++) {
            const data = paramData[i];
            dLog('green', 'CRYPTOCASINO', [`openCoupon ITERATION ${i}- we using data ${(typeof data)}:`, data]);
            await openEvent(data);
            dLog('green', 'CRYPTOCASINO', 'Event must be opened!');
    ...
    const getBetElement = async betIn => {
        ...
        dLog('green', 'CRYPTOCASINO', ['Final market is:', m]);
    ...
    const ProccedExpressNew = async () => {
        ...
        do {
            ...
            if (eventName.length < 5) {
                dLog('red', 'CRYPTOCASINO', `'${eventName}' is too short - ${eventName.length}`);
                continue;
            }

            if (used[eventName] >= 1) {
                dLog('big-yellow', 'CRYPTOCASINO', `${eventName} used ${used[eventName]} times!`);
                continue;
            }
        ...
        if ($coupons().length === settings.newExpressBetsAmount 
            ...
            dLog('green', 'CRYPTOCASINO', [`We get selected bets: '${currentBets}', now used:`, used]);
    ...
    const proceedBet = async (data, command) => {
        ...
        if (command === 'BET' && !!currentBetData.data[0].betFromParser) {
            ...
            if (!await eventsWork('cryptocasino', settings, eventName, false, true)) {
                throw `Max amount of bets ${settings.eventMaxBets} to ${eventName} already done!`;
            } else {
                dLog('big-blue', 'CRYPTOCASINO',
       ...
       if (settings.lastScoreBasketball === '999') {
        ...
        do {
            ...
            dLog('green', 'CRYPTOCASINO', `Will place (performBet): ${place}`);
            ...
            if (parseFloat($input().val()) !== parseFloat(place)) {
                ...
                await delayPromise(555);
                dLog('green', 'CRYPTOCASINO', `STAKE entered ${willPlace}`);
            }

            const entered = parseFloat($input().val());
            dLog('green', 'CRYPTOCASINO', `After enter stake check: willPlace = ${willPlace}, entered: ${entered}`);

            if (isNaN(entered) || willPlace !== entered) {
                dLog('red', 'CRYPTOCASINO', 'Entered !== willPlace - try to reenter!');
                continue;
            }
    ...
    const collectBetResults = async (inD) => {
        ...
        let
            ...
            moreTimes = 0,
            checkedIds = [];
        dLog('green', 'CRYPTOCASINO', [`collectBetResults, limit: ${limit}, data:`, data]);
        await goDirection(3);
        ...
        dLog('', 'CRYPTOCASINO', [`checked: ${checked}, collected:`, collected]);
    ...
    const commands = new class commands {
        ...
        async execute(command, data) {
            ...
            dLog(res.success ? 'green' : 'red', 'CRYPTOCASINO',
        ...
        async prepareResult(command, res) {
            dLog('', 'CRYPTOCASINO', [`prepareResult '${command}':`, res, currentBetData,]);
            if (['BET', 'EXPRESS_BET'].indexOf(command) > -1) {
                if (res.success) {
                    await eventsWork('cryptocasino', settings,
    ...
    const messageProcessor = (message, direct) => {
        ...
        if (limited) {
            ...
            dLog(`yellow`, 'CRYPTOCASINO', [`messageProcessor auth settings:`, settings,]);
            authCheck();
            wasAuthCheck = true;
    ...
    function afterDOMLoaded() {
        (async () => {
            const command = await bMess('CRYPTOCASINO').check(40000);
            await waitForCondition(() => !isMain || (typeof wasAuthCheck === 'boolean' && wasAuthCheck), 333, 60000, 'No auth check!');
            bsLogger('green', 'CRYPTOCASINO', [`Restoring with (${isMain}): `, command]);
            messageProcessor(command, false);
        })()
            .catch(e => console.log(e));
        port.postMessage({m: "PAGE LOADED!"});
        console.log('%c' + `CRYPTOCASINO loaded and message sent! (${isMain}) ${document.location.href}`,
    ...
    addEventListener("unload", async () => {
        await ourCommand.save('CRYPTOCASINO', increaseDelay ? 150000 : 0);
        bsLogger('green', 'CRYPTOCASINO', [`Command was set till unload (${isMain}):`, ourCommand.get()]);
    }, true);
