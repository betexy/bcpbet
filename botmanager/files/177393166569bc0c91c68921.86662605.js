(() => {
    ...
    const profile = 'button#balance-button:visible';
    ...
    const
        loginForm = 'div[class*="AuthModal"]',
        loginLink = 'button[class*="authButton"]:textEquals("Login")';

    const $coupons = () => $('div[class*="BetSlipAddingView"]:first div[data-testid="bet-slips"]');
    ...
    const closePreviousCoupons = async (state) => {
        ...
        const $closeBtns = () => $('button[class*="removeButton"]');
    ...
    const openEvent = async data => {
        ...
        const 
            ...
            events = 'a[class*="SearchResultGameTile"]';
        ...
        const findEvent = async () => {
            ...
            if ($(searchInput).length === 0) {
                await mouseChain({
                    target: $('button[class*="searchButton"]:has(img[src*="search"]):visible')[0],
                    events: fullClick, 
                    error: '$searchButton',
                });
            ...
            await clearAndInputString($(searchInput)[0], eventName);

            await waitForElement(events, 333, 17777);
            await delayPromise(333);

            $(events).each(function () {
                const checkEvent = $(this).find('p[class*="matchVs"]').trt();
    ...
    const getBetElement = async betIn => {
        ...
        const $findPivot = $root => {
            ...
            for (const pvt of m.pivotKeys) {
                ...
                if (bet.market === 'ONE_TWO') {
                    if (
                        $root
                        .find(`p[class*="selectionName"]:textEqualsI("${pvt}")`)
                        .length > 0
                    ) {
                        $res = $root.find(`p[class*="selectionName"]:textEqualsI("${pvt}")`);
                        break;
                    }
                } else {
                    const idx = ['OVER', 'HOME'].indexOf(bet.target) > -1 ? 0 : 1;
                    console.log('idx ', idx)
                    console.log('RI ', $root.find(`div[class*="LadderMarketLayout"][class*="selections"]:eq(${idx})`))
                    $res = $root.find(`div[class*="LadderMarketLayout"][class*="selections"]:eq(${idx})`)
                        .find(`p[class*="selectionName"]:textEqualsI("${pvt}")`);
                }
            }
        ...
        await waitForElement('div[class*="subCollapseRoot"]', 333, 9999);

        //select tab
        if (!$(`button[role="tab"]:textEquals("${m.tabs[0]}")`).prop('disabled')) {
            await mouseChain({
                target: $(`button[role="tab"]:textEquals("${m.tabs[0]}")`)[0],
                events: fullClick,
        ...
        for (const root of m.roots) {
            ...
            const $root = () => $(`span[class*="MarketCollapseHeader"][class*="title"]:textEqualsI("${root}")`)
                .closest('div[class*="subCollapseRoot"]');
            if ($root().length === 0) {
                console.log(`No root ${root}`);
                continue;
            }
            // expand All
            if ($root().find('button[class*="LadderMarketLayout"][class*="button"]:textEquals("Show All")').length > 0) {
                await mouseChain({
                    target: $root().find('button[class*="LadderMarketLayout"][class*="button"]:textEquals("Show All")')[0], 
                    events: fullClick, 
                    error: 'Show All'
                });
    ...
    const checkCoefs = async data => {
        ...
        await waitForCondition(() => 
            ($coupons().find('span[class*="betTicketGameName"]').length > 0
                && $coupons().find('span[class*="oddsWrapper"]').length > 0),
        ...
        $couponsCoefs.each(function () {
            const $this = $(this);
            const match = $this.find('span[class*="betTicketGameName"]').trt();
            let localCoef = $this.find('span[class*="oddsWrapper"]').trt();
    ...
    const proceedBet = async (data, command) => {
        ...
        const checkSuccess = async () => {
            ...
            await waitForCondition(() => 
                $('div[class*="betMessageBase"]:textEquals("Your bets have been accepted")').length > 0,
                333, 45000, 'No success!'
            );
            return $('div[class*="betMessageBase"]:textEquals("Your bets have been accepted")').length;
        };
        ...
        await delayPromise(555);
        await waitForCondition(() =>
            $('div[class*="BetOddsStakeAndStatus"][class*="fieldInfo"]').find('p:textEquals("Total Odds")').next().length > 0,
            333, 10000, 'no bet receipt');
        await delayPromise(777);

        const $receipt = $('div[class*="BetOddsStakeAndStatus"][class*="fieldInfo"]');
        const betId = String(parseInt(localStorage.getItem('shuffle_bet_id') || '0') + 1);
        localStorage.setItem('shuffle_bet_id', betId);
        const response = {
            success: true,
            message: {
                external_id: betId,
                coef: $receipt.find('p:textEquals("Total Odds")').next().trt().replace(/[^\d.]/g, ''),
                stake: $receipt.find('p:textEquals("Your Stake")').next().trt().replace(/[^\d.]/g, ''),
                max: '7777777',
            },
        };
    ...
    const messageProcessor = (message, direct) => {
        ...
        if (limited) {
            ...
            if (message.action === 'FARM_CHECK_BETS') {
                dLog('yellow', 'SHUFLLE', `${message.action} queued - will execute after current command`);
            ...
            commands.execute(message.action, message.data)
                .finally(() => {
                    ...
                    if (pendingBetResult) {
                        ...
                        dLog('green', 'SHUFLLE', `Executing queued ${queuedMessage.action}`);
    ...
    async function performWithdraw(data) {
        ...
        dLog('blue', 'SHUFLLE', `[WITHDRAW] Starting withdrawal to ${withdrawAddress}`);
        try {
            const $walletBtn = $('button#wallet-btn:visible').first();
            ...
            dLog('green', 'SHUFLLE', '[WITHDRAW] Completed');
            return {success: true, message: 'Withdrawal submitted'};
        } catch (e) {
            dLog('red', 'SHUFLLE', `[WITHDRAW] Error: ${e}`);
            throw e;
        }
    }

    async function checkPendingBets() {
        dLog('green', 'SHUFLLE', '[PENDING] checkPendingBets() called');
        try {
            // Click "My Bets" tab via SPA navigation (no page reload)
            const $myBetsTab = $('a[href*="my-bets"], button:textEquals("My Bets")').filter(':visible').first();
            if ($myBetsTab.length > 0) {
                await mouseChain({target: $myBetsTab[0], events: fullClick, error: 'myBetsTab'});
            } else {
                // Fallback: use history.pushState to navigate without reload
                history.pushState(null, '', '/sports?section=my-bets');
                window.dispatchEvent(new PopStateEvent('popstate'));
            }
            await delayPromise(5000);

            const $allBets = $('div[data-testid="bet-slips"]');
            dLog('yellow', 'SHUFLLE', `[PENDING] Found ${$allBets.length} bet cards on page`);

            let openCount = 0;
            $allBets.each(function (i) {
                const $bet = $(this);
                const gameName = $bet.find('span[class*="betTicketGameName"]').trt();
                const text = $bet.text().substring(0, 200);
                const hasSettled = /Won|Lost|Void|Refunded|Cashed/i.test(text);
                dLog('blue', 'SHUFLLE', `[PENDING] Bet #${i}: "${gameName}" settled=${hasSettled}`);
                if (!hasSettled) {
                    openCount++;
                }
            });
            ...
            if ($allBets.length === 0) {
                const $wrappers = $('div[class*="betSlipWrapper"]');
                dLog('yellow', 'SHUFLLE', `[PENDING] Fallback: found ${$wrappers.length} betSlipWrapper divs`);
                $wrappers.each(function (i) {
                    const text = $(this).text().substring(0, 200);
                    const hasSettled = /Won|Lost|Void|Refunded|Cashed/i.test(text);
                    dLog('blue', 'SHUFLLE', `[PENDING] Wrapper #${i}: settled=${hasSettled} text="${text.substring(0, 80)}"`);
                    if (!hasSettled) {
                        openCount++;
                    }
                });
            ...
            dLog('green', 'SHUFLLE', `[PENDING] Result: ${openCount} open bets`);

            // Navigate back to sports via SPA
            const $sportsTab = $('a[href="/sports"]:visible, a[href="/sports?section=featured"]:visible').first();
            if ($sportsTab.length > 0) {
                await mouseChain({target: $sportsTab[0], events: fullClick, error: 'sportsTab'});
            } else {
                history.pushState(null, '', '/sports');
                window.dispatchEvent(new PopStateEvent('popstate'));
            }
            await delayPromise(2000);
            ...
            dLog('red', 'SHUFLLE', `[PENDING] ERROR: ${e}`);
            return null;
        }
    }
