(() => {
    ...
    // Define shadow root element — find div whose shadowRoot contains #bt-root
            ...
            if (!limited && $(loginLink).length > 0) {
                ...
                if (settings.lastScoreBasketball === '999') {
                    if (Date.now() - settings.sourceDate >= 300000) {
                        ...
                        dLog('blue', 'ROLLBIT', `Source current random value - ${settings.sourceRandom}`);
                    }
                }
                ...
                if (settings.newExpresses && !busy) {
                    ...
                    if (!WasSuccessExpressNew) {
                        dLog('green', 'ROLLBIT', 'START find NewExpress event!');
                        if ($coupons().length > 0) {
                        ...
                        await ProccedExpressNew().catch((e) => {
                            dLog('red', 'ROLLBIT', 'ProccedExpressNew Error - ' + e);
    ...
    const closeSourceExpressCoupon = async () => {
        dLog('green', 'ROLLBIT', 'closeSourceExpressCoupon started!');
    ...
    const checkEventName = (eventNameIn, controlNameIn) => {
        ...
        dLog('green', 'rollbit', `Event name '${eventName}' ${res ? '==' : '!='} '${controlName}'`);
        return res;
    };
    ...
    const ProccedExpressNew = async () => {
        ...
        if (document.location.href.indexOf('event-builder') === -1) {
            ...
            if (builderBtnExist.length === 0) {
                window.location.href = settings.url || document.location.origin + '/sports';
        ...
        do {
            ...
            if (eventName.length < 5) {
                dLog('red', 'ROLLBIT', `'${eventName}' is too short - ${eventName.length}`);
                continue;
            }

            if (used[eventName] >= 1) {
                dLog('big-yellow', 'ROLLBIT', `${eventName} used ${used[eventName]} times!`);
                continue;
            }
        ...
        if ($coupons().length === settings.newExpressBetsAmount
            ...
            dLog('green', 'ROLLBIT', [`We get selected bets: '${currentBets}', now used:`, used]);
    ...
    const proceedBet = async (data, command) => {
        ...
        if (command === 'BET' && !!currentBetData.data[0].betFromParser) {
            ...
            if (!await eventsWork('rollbit', settings, eventName, false, true)) {
        ...
        do {
            ...
            if ($cutError.length > 0) {
                ...
                if (/max(imum)?\s*stake/i.test(errorText)) {
                    ...
                    dLog('red', 'ROLLBIT', `[CUT] Max stake detected: ${matchNum ? matchNum[0] : '?'}`);
                    port.postMessage({
                        m: 'FARM_CUT_DETECTED',
                        cutInfo: {
                            ...
                            bk: 'ROLLBIT', timestamp: Date.now()
                        }
                    });
                } else if (/blocked/i.test(errorText)) {
                    dLog('red', 'ROLLBIT', `[BLOCKED] Account blocked: ${errorText}`);
                    port.postMessage({
                        m: 'FARM_CUT_DETECTED',
                        cutInfo: {
    ...
    const commands = new class commands {
        ...
        async prepareResult(command, res) {
            ...
            if (['BET', 'EXPRESS_BET'].indexOf(command) > -1) {
                if (res.success) {
                    await eventsWork('rollbit', settings,
    ...
    const messageProcessor = (message, direct) => {
        ...
        if (limited) {
            ...
            if (message.action === 'FARM_CHECK_BETS') {
                dLog('yellow', 'ROLLBIT', `${message.action} queued - will execute after current command`);
                ...
                commands.execute(message.action, message.data)
                    .finally(() => {
                        ...
                        if (pendingBetResult) {
                            ...
                            dLog('green', 'ROLLBIT', `Executing queued ${queuedMessage.action}`);
    ...
    async function performWithdraw(data) {
        ...
        dLog('blue', 'ROLLBIT', `[WITHDRAW] Starting withdrawal to ${withdrawAddress}`);
        try {
            const $walletBtn = $(shadowRoot()).find('button:has(img[src*="/coin/"])').filter(':visible').first();
            ...
            const $withdrawTab = $(shadowRoot()).find('button:textEquals("Withdraw"), a:textEquals("Withdraw")').filter(':visible');
            ...
            const $addressInput = $(shadowRoot()).find('input[placeholder*="address" i], input[name*="address" i]').filter(':visible').first();
            if ($addressInput.length === 0) throw 'Address input not found';
            await clearAndInputString($addressInput[0], withdrawAddress);
            await delayPromise(500);

            const $maxBtn = $(shadowRoot()).find('button:textEquals("Max"), button:textEquals("MAX")').filter(':visible').first();
            if ($maxBtn.length > 0) {
                await mouseChain({target: $maxBtn[0], events: fullClick, error: 'maxBtn'});
                await delayPromise(1000);
            }

            const $withdrawBtn = $(shadowRoot()).find('button:textEquals("Withdraw"), button[type="submit"]:contains("Withdraw")').filter(':visible').last();
            ...
            dLog('green', 'ROLLBIT', `[WITHDRAW] Completed`);
            return {success: true, message: 'Withdrawal submitted'};
        } catch (e) {
            dLog('red', 'ROLLBIT', `[WITHDRAW] Error: ${e}`);
            throw e;
        }
    }
