(() => {
    ...
    const port = isMain ? chrome.runtime.connect({name: 'port_crashino'})
        : {postMessage: (...args) => console.log(args)};
    const settings = {
        restartEvery: 1800000,
        authCheckInterval: 2000,
        url: 'https://www.crashino.com/en/sports',
    ...
    const
        ...
        shadowRoot = () => $(btSel) && $(btSel).length > 0
            ? $(btSel)[0].shadowRoot
            : null,
        $loginForm = () => $('div.MuiModal-root'),
        loginLink = 'button#topbar-login-button';

    const $coupons = () => $(shadowRoot()).find('div[data-editor-id="betslipSelection"]');

    function getBalance(returnNull) {
        const balanceEl = findSel(['span[class*="CollapsedBalanceAtom-module_balance_"]', 'div[class*="CollapsedBalanceAtom-module_item_"]']);
        return $(balanceEl).length === 0
            ? (typeof returnNull === 'boolean' && returnNull ? null : 0)
            : parseFloat($(balanceEl).trt().replace(/[^\d.]/g, ''));
    };
    ...
    const authCheck = () => {
        ...
        (async () => {
            ...
            await closeAllWeNeed({
                ...
                'button[class*="CookieBannerOverlay-module_button_"]': 'button[class*="CookieBannerOverlay-module_button_"]',
            });
            ...
            if (!limited && $loginLink.length > 0) {
                ...
                if (settings.lastScoreBasketball === '999') {
                    if (Date.now() - settings.sourceDate >= 300000) {
                        ...
                        dLog('blue', 'CRASHINO', `Source current random value - ${settings.sourceRandom}`);
                    }
                }
                ...
                if (settings.newExpresses && !busy) {
                    ...
                    if (!WasSuccessExpressNew) {
                        dLog('green', 'CRASHINO', 'START find NewExpress event!');
                        // clear coupons
                        if ($coupons().length > 0) {
                        ...
                        await ProccedExpressNew().catch((e) => {
                            dLog('red', 'CRASHINO', 'ProccedExpressNew Error - ' + e);
            ...
            .catch(e => dLog('red', 'CRASHINO', `authCheck: ${e}`))
    ...
    const tryToLogin = async () => {
        ...
        const profile = 'div[class*="BalanceComponent-module_wrapper_"]';
        if ($loginForm().length === 0) {
            await mouseChain({target: $(loginLink)[0], events: fullClick, error: 'Log in'});
            await delayPromise(333);
        }
        await waitForCondition(() => $loginForm().find('div:textEquals("Login")').length > 0,
            333, 10000, 'No login form');
        await clearAndInputEmail($loginForm().find('input[name="email"]')[0],
            settings.login);
        ...
        await delayPromise(777);
        await mouseChain({
            target: $loginForm().find('button#login-form-submit-button')[0],
            events: fullClick,
            error: 'submit form'
        });
        ...
        dLog('green', 'crashino', `Auth clicked ${authClicked}!`);
        if ($profile.length === 0) {
            port.postMessage({
                answered: "auth_error",
                status: "ERROR",
    ...
    const checkEventName = (eventNameIn, controlNameIn) => {
        ...
        dLog('green', 'crashino', `Event name '${eventName}' ${res ? '==' : '!='} '${controlName}'`);
        return res;
    };

    const openEvent = async data => {
        dLog('red', 'crashino', ['openEvent', data]);
        ...
        const findEvent = async () => {
            ...
            $events(timeCnd).each(function () {
                ...
                dLog('blue', 'crashino', `Check: '${checkEvent}' === '${eventName}'`);
    ...
    const openCoupon = async paramData => {
        dLog('green', 'crashino', ['openCoupon, paramData:', paramData]);
        for (let i = 0; i < paramData.length; i++) {
            const data = paramData[i];
            dLog('green', 'crashino', [`openCoupon ITERATION ${i}- we using data ${(typeof data)}:`, data]);
            await openEvent(data);
            dLog('green', 'crashino', 'Event must be opened!');
    ...
    const getBetElement = async betIn => {
        ...
        dLog('green', 'crashino', ['Final market is:', m]);
   ...
   const ProccedExpressNew = async () => {
        ...
        if (document.location.href.indexOf('event-builder') === -1) {
            ...
            if (builderBtnExist.length === 0) {
                window.location.href = 'https://crashino.com/en/sports';
        ...
        do {
            ...
            if (eventName.length < 5) {
                dLog('red', 'CRASHINO', `'${eventName}' is too short - ${eventName.length}`);
                continue;
            }

            if (used[eventName] >= 1) {
                dLog('big-yellow', 'CRASHINO', `${eventName} used ${used[eventName]} times!`);
                continue;
            }
        ...
        if ($coupons().length === settings.newExpressBetsAmount 
            ...
            dLog('green', 'CRASHINO', [`We get selected bets: '${currentBets}', now used:`, used]);
    ...
    const proceedBet = async (data, command) => {
        lang = document.location.href.indexOf('crashino.com/en/sports') > -1 ? 'ru' : 'eng';
        const
        ...
        if (command === 'BET' && !!currentBetData.data[0].betFromParser) {
            ...
            if (!await eventsWork('crashino', settings, eventName, false, true)) {
                throw `Max amount of bets ${settings.eventMaxBets} to ${eventName} already done!`;
            } else {
                dLog('big-blue', 'crashino',
     ...
     if (settings.lastScoreBasketball === '999') {
        ...
        do {
            ...
            dLog('green', 'crashino', `Will place (performBet): ${place}`);
            ...
            if (parseFloat($input().val()) !== parseFloat(place)) {
                ...
                await delayPromise(555);
                dLog('green', 'crashino', `STAKE entered ${willPlace}`);
            }

            const entered = parseFloat($input().val());
            dLog('green', 'crashino', `After enter stake check: willPlace = ${willPlace}, entered: ${entered}`);
            if (isNaN(entered) || willPlace !== entered) {
                dLog('red', 'crashino', 'Entered !== willPlace - try to reenter!');
                continue;
            }
    ...
    const collectBetResults = async (inD) => {
        ...
        let
            ...
            moreTimes = 0,
            checkedIds = [];
        dLog('green', 'crashino', [`collectBetResults, limit: ${limit}, data:`, data]);
        await goDirection(3);
        ...
        dLog('', 'crashino', [`checked: ${checked}, collected:`, collected]);
    ...
    const commands = new class commands {
        ...
        async execute(command, data) {
            ...
            dLog(res.success ? 'green' : 'red', 'crashino',
        ...
        async prepareResult(command, res) {
            dLog('', 'crashino', [`prepareResult '${command}':`, res, currentBetData,]);
            if (['BET', 'EXPRESS_BET'].indexOf(command) > -1) {
                if (res.success) {
                    await eventsWork('crashino', settings,
    ...
    const messageProcessor = (message, direct) => {
        ...
        if (limited) {
            ...
            dLog(`yellow`, 'crashino', [`messageProcessor auth settings:`, settings,]);
            authCheck();
            wasAuthCheck = true;
    ...
    function afterDOMLoaded() {
        (async () => {
            const command = await bMess('crashino').check(40000);
            await waitForCondition(() => !isMain || (typeof wasAuthCheck === 'boolean' && wasAuthCheck), 333, 60000, 'No auth check!');
            bsLogger('green', 'crashino', [`Restoring with (${isMain}): `, command]);
            messageProcessor(command, false);
        })()
            .catch(e => console.log(e));
        port.postMessage({m: "PAGE LOADED!"});
        console.log('%c' + `crashino loaded and message sent! (${isMain}) ${document.location.href}`,
    ...
    addEventListener("unload", async () => {
        await ourCommand.save('crashino', increaseDelay ? 150000 : 0);
        bsLogger('green', 'crashino', [`Command was set till unload (${isMain}):`, ourCommand.get()]);
    }, true);
