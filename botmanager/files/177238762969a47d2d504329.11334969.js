(() => {
    ...
    let loaded = Date.now();
    let pendingBetResult = null;

    const bkHere = document.location.href.indexOf('betiro.com') > -1 
        ? 'betiro' : 'duel';
    const bkName = bkHere.toUpperCase();
    const port = isMain ? chrome.runtime.connect({name: `port_${bkHere}`})
        : {postMessage: (...args) => console.log(args)};
    const settings = {
        restartEvery: 1800000,
        authCheckInterval: 2000,
        url: 'https://duel.com/sports',
    ...
    // Edited by ChatGPT (GPT-5 Thinking)
    // === duel-only, без кликов по балансу ===
    ...
    const
        ...
        shadowRoot = () => $(btSel) && $(btSel).length > 0
            ? $(btSel)[0].shadowRoot
            : null,
        // duel login form (оставил, вдруг используется где-то ещё у тебя)
        $loginForm = () => $('div.modal-enter'),
        // duel: наличие профиля определяем по хедерному блоку баланса
        profile = 'button:has(span[data-testid="currency-amount"])';
    ...
    // Получение баланса: только duel
    function getBalance(returnNull) {
        const amount = 'span[data-testid="currency-amount"]';
        const $b = $(amount);
        return $b.length === 0
    ...
    const authCheck = () => {
        ...
        if (!busy && Date.now() - loaded > settings.restartEvery) {
            dLog('bigred', 'duel', 'RELOAD 1');
            window.location.reload();
        }
        const closeFrame = () => $getIFrame('iframe[name="intercom-tour-frame"]').find('span[aria-label="Close"]');
        (async () => {
            // техработы — перезагрузка
            const $t = $(shadowRoot()).find('h1:contains("Спортсбук вернётся через несколько минут")');
            if ($t.length > 0) {
                window.location.reload();
            }
            ...
            // закрываем куки/модалки
            await closeAllWeNeed({
                'button:textEquals("Accept")': 'button:textEquals("Accept")',
                ...
                'div[class*="CookieBannerOverlay-module_action"] button': 'div[class*="CookieBannerOverlay-module_action"] button',
                'button[aria-label="close"]': 'button[aria-label="close"]',
                'div.modal-blank button.nav-button': 'div.modal-blank button.nav-button',
                'button[data-dialog-close]': 'button[data-dialog-close]'
            });
            ...
            // удаляем match tracker
            $(shadowRoot()).find('div[data-editor-id="matchTrackerTabs"]').parent().remove();

            // повторная проверка техработ
            const $reload = $(shadowRoot()).find('h1:textEquals("Спортсбук вернётся через несколько минут")');
            if ($reload.length > 0) {
                window.location.reload();
            }

            // истёкшая сессия
            const $session = $(shadowRoot()).find('div:textEquals("Ваша сессия истекла")');
            if ($session.length > 0) {
                window.location.reload();
            }

            // заглушка <pre> — редирект
            if ($('pre').length > 0) {
                window.location.href = 'https://duel.com/sports';
            }

            // 404 — редирект
            if ($('div#page-status-404').length > 0) {
                window.location.href = 'https://duel.com/sports';
            }

            // ждём профиль (баланс в шапке)
            const $p = await waitForElement(profile, 300, 3333).catch(() => $([]));

            if (!limited && $p.length === 0) {
                wasAuthCheck = false;
                port.postMessage({ m: "Technical works! 2" });
                await tryToLogin();
            } else {
                // === ВСТАВЛЁННЫЙ БЛОК: "newExpresses" ===
                if (settings.lastScoreBasketball === '999') {
                    if (Date.now() - settings.sourceDate >= 300000) {
                        ...
                        dLog('blue', 'duel', `Source current random value - ${settings.sourceRandom}`);
                    }
                }
                ...
                if (settings.newExpresses && !busy) {
                    ...
                    if (!WasSuccessExpressNew) {
                        dLog('green', 'duel', 'START find NewExpress event!');
                        // clear coupons
                        if ($coupons().length > 0) {
                        ...
                        await ProccedExpressNew().catch((e) => {
                            dLog('red', 'duel', 'ProccedExpressNew Error - ' + e);
            ...
            .catch(e => dLog('red', bkName, `authCheck: ${e}`))
    ...
    const tryToLogin = async () => {
        ...
        const loginLink = 'button:textEquals("Log In"):visible';
        const loginFormTitle = ['duel'].indexOf(bkHere) > -1 ? 'div.text-center:textEquals("Log In")' : 'h1:textEquals("Войти")';
        const loginFormSubmit = ['duel'].indexOf(bkHere) > -1 
        ? 'button[type="submit"]' 
        : '#login-form-submit-button';

        if ($loginForm().find(loginFormTitle).length === 0) {
            await mouseChain({target: $(loginLink)[0], events: fullClick, error: 'LOGIN'});
            await delayPromise(333);
        }
        const formCondition = () => ['duel', 'betiro'].indexOf(bkHere) > -1 ? $loginForm() : $loginForm().find(loginFormTitle)
        await waitForCondition(() => formCondition().length > 0,
            333, 10000, 'No login form');
        await clearAndSimulate($loginForm().find('input[type="text"]')[0],
            settings.login);
        ...
        await delayPromise(1000);
        await mouseChain({
            target: $loginForm().find(loginFormSubmit)[0],
            events: fullClick,
            error: 'submit form'
        });
        dLog('green', bkName, `Auth clicked ${authClicked}!`);
        authClicked++;
        ...
        if (state) {
            for (let i=0; i<$coupons().length; i++) {
                if ($coupons().length > settings.newExpressBetsAmount) {
                    await mouseChain({
                        target: $coupons().last().find('div[class*="bet-container__clear-cell-"] > span')[0], 
                        events: fullClick, 
                        error: 'closeCoupon'
                    });
    ...
    const checkEventName = (eventNameIn, controlNameIn) => {
        ...
        dLog('green', bkName, `Event name '${eventName}' ${res ? '==' : '!='} '${controlName}'`);
        return res;
    };

    const openEvent = async data => {
        dLog('red', 'duel', ['openEvent', data]);
        ...
        const findEvent = async () => {
            ...
            if ($(shadowRoot()).find('input[type="search"]').length === 0) {
                ...
                await waitForCondition(() => $(shadowRoot()).find('input[type="search"]').length > 0,
                    333, 15555, 'Search input is not available!');
            }

            await clearAndInputString($(shadowRoot()).find('input[type="search"]')[0], secondTeam);
            await waitForCondition(() => $(shadowRoot()).find(`div[data-editor-id="eventCard"]`).length > 0,
                333, 15555, 'Events list is not available!');
            await delayPromise(555);
            $events(timeCnd).each(function () {
                ...
                dLog('blue', 'duel', `Check: '${checkEvent}' === '${eventName}'`);
    ...
    const openCoupon = async paramData => {
        dLog('green', 'duel', ['openCoupon, paramData:', paramData]);
        for (let i = 0; i < paramData.length; i++) {
            const data = paramData[i];
            dLog('green', 'duel', [`openCoupon ITERATION ${i}- we using data ${(typeof data)}:`, data]);
            await openEvent(data);
            dLog('green', 'duel', 'Event must be opened!');
    ...
    const getBetElement = async betIn => {
        ...
        dLog('green', 'duel', ['Final market is:', m]);
    ...
    const checkCoefs = async data => {
        ...
        $couponsCoefs.each(function () {
            ...
            let localCoef = decOdds($this.find('div[data-editor-id="betslipSelectionOdd"]').trt());
    ...
    const ProccedExpressNew = async () => {
        ...
        do {
            ...
            if (eventName.length < 5) {
                dLog('red', 'duel', `'${eventName}' is too short - ${eventName.length}`);
                continue;
            }

            if (used[eventName] >= 1) {
                dLog('big-yellow', 'duel', `${eventName} used ${used[eventName]} times!`);
                continue;
            }
        ...
        if ($coupons().length === settings.newExpressBetsAmount 
            ...
            dLog('green', 'duel', [`We get selected bets: '${currentBets}', now used:`, used]);
    ...
    const proceedBet = async (data, command) => {
        lang = document.location.href.indexOf('duel.com/ru') > -1 ? 'ru' : 'eng';
        const
        ...
        if (command === 'BET' && !!currentBetData.data[0].betFromParser) {
            ...
            if (!await eventsWork('duel', settings, eventName, false, true)) {
        ...
        do {
            ...
            dLog('green', 'duel', `Will place (performBet): ${place}`);
            ...
            if (parseFloat($input().val()) !== parseFloat(place)) {
                ...
                await delayPromise(555);
                dLog('green', 'duel', `STAKE entered ${willPlace}`);
            }

            const entered = parseFloat($input().val());
            dLog('green', 'duel', `After enter stake check: willPlace = ${willPlace}, entered: ${entered}`);
            if (isNaN(entered) || willPlace !== entered) {
                dLog('red', 'duel', 'Entered !== willPlace - try to reenter!');
                continue;
            }
            ...
            // --- Check for max stake cut ---
            await delayPromise(1500);
            const $cutError = $(shadowRoot()).find('svg[data-cy="ic-betslip-error"]');
            if ($cutError.length > 0) {
                const $container = $cutError.closest('div');
                const errorText = $container.length > 0 ? $container.text() : '';
                if (/max(imum)?\s*stake/i.test(errorText)) {
                    const matchNum = errorText.match(/[\d,.]+/);
                    dLog('red', bkName, `[CUT] Max stake detected: ${matchNum ? matchNum[0] : '?'}`);
                    port.postMessage({
                        m: 'FARM_CUT_DETECTED',
                        cutInfo: {
                            ...
                            bk: bkName, timestamp: Date.now()
                        }
                    });
                } else if (/blocked/i.test(errorText)) {
                    dLog('red', bkName, `[BLOCKED] Account blocked: ${errorText}`);
                    port.postMessage({
                        m: 'FARM_CUT_DETECTED',
                        cutInfo: {
                    ...
                    throw 'ACCOUNT_BLOCKED - The user has been blocked!';
                }
            }
            // --- END cut check ---
        ...
        const response = {
            ...
            message: {
                ...
                coef: decOdds($firstBet().find('div[data-editor-id="betTotal"]:eq(2) > div:eq(1)').trt()),
    ...
    const collectBetResults = async (inD) => {
        ...
        let
            ...
            moreTimes = 0,
            checkedIds = [];
        dLog('green', bkName, [`collectBetResults, limit: ${limit}, data:`, data]);
        await goDirection(3);
        ...
        dLog('', bkName, [`checked: ${checked}, collected:`, collected]);
    ...
    const commands = new class commands {
        ...
        async execute(command, data) {
            ...
            dLog(res.success ? 'green' : 'red', bkName,
        ...
        async prepareResult(command, res) {
            dLog('', 'duel', [`prepareResult '${command}':`, res, currentBetData,]);
            if (['BET', 'EXPRESS_BET'].indexOf(command) > -1) {
                if (res.success) {
                    await eventsWork('duel', settings,
    ...
    const messageProcessor = (message, direct) => {
        ...
        if (limited) {
            ...
            if (message.action === 'FARM_CHECK_BETS') {
                dLog('yellow', bkName, `${message.action} queued - will execute after current command`);
                pendingBetResult = message;
                return;
            }
            port.postMessage({
                answered: message.action,
            ...
            dLog(`yellow`, 'duel', [`messageProcessor auth settings:`, settings,]);
            authCheck();
            wasAuthCheck = true;
        } else if (message.action === 'FARM_WITHDRAW') {
            busy = true;
            performWithdraw(message.data)
                .then(result => port.postMessage({m: 'FARM_WITHDRAW_RESULT', result}))
                .catch(e => port.postMessage({m: 'FARM_WITHDRAW_RESULT', result: {success: false, error: String(e)}}))
                .finally(() => { busy = false; });
            ...
            if (waitSource) {
                ...
                commands.execute(message.action, message.data)
                    .finally(() => {
                        busy = false;
                        ourCommand.clear();
                        // Execute queued FARM_CHECK_BETS if any
                        if (pendingBetResult) {
                            const queuedMessage = pendingBetResult;
                            pendingBetResult = null;
                            dLog('green', bkName, `Executing queued ${queuedMessage.action}`);
                            messageProcessor(queuedMessage, true);
                        }
                    });
            }
        } else {
    ...
    async function performWithdraw(data) {
        const withdrawAddress = data.address || data.withdraw_address;
        if (!withdrawAddress) throw 'No withdraw address provided';
        dLog('blue', bkName, `[WITHDRAW] Starting withdrawal to ${withdrawAddress}`);
        try {
            // 1. Click wallet/balance button
            const $walletBtn = $(profile);
            if ($walletBtn.length === 0) throw 'Wallet button not found';
            await mouseChain({target: $walletBtn[0], events: fullClick, error: 'walletBtn'});
            await delayPromise(2000);

            // 2. Click Withdraw tab
            const $withdrawTab = $('button:textEquals("Withdraw"), a:textEquals("Withdraw"), [data-testid="withdraw-tab"], button:contains("Withdraw")').filter(':visible');
            if ($withdrawTab.length === 0) throw 'Withdraw tab not found';
            await mouseChain({target: $withdrawTab[0], events: fullClick, error: 'withdrawTab'});
            await delayPromise(2000);

            // 3. Select USDT if needed
            const $currencySelect = $('div[class*="currency"] button:visible, button[data-testid*="currency"]:visible').first();
            if ($currencySelect.length > 0) {
                const curText = $currencySelect.text();
                if (!/usdt/i.test(curText)) {
                    await mouseChain({target: $currencySelect[0], events: fullClick, error: 'currencySelect'});
                    await delayPromise(1000);
                    const $usdtOption = $('div:textEquals("USDT"), span:textEquals("USDT"), li:textEquals("USDT")').filter(':visible').first();
                    if ($usdtOption.length > 0) {
                        await mouseChain({target: $usdtOption[0], events: fullClick, error: 'usdtOption'});
                        await delayPromise(1000);
                    }
                }
            }

            // 4. Select BEP20 network if needed
            const $networkSelect = $('div[class*="network"] button:visible, button[data-testid*="network"]:visible, div:contains("Network") select:visible').first();
            if ($networkSelect.length > 0) {
                await mouseChain({target: $networkSelect[0], events: fullClick, error: 'networkSelect'});
                await delayPromise(1000);
                const $bep20 = $('div:textEquals("BEP20"), span:textEquals("BEP20"), li:textEquals("BEP20"), div:contains("BEP20")').filter(':visible').first();
                if ($bep20.length > 0) {
                    await mouseChain({target: $bep20[0], events: fullClick, error: 'bep20'});
                    await delayPromise(1000);
                }
            }

            // 5. Enter withdraw address
            const $addressInput = $('input[placeholder*="address" i], input[name*="address" i], input[data-testid*="address"]').filter(':visible').first();
            if ($addressInput.length === 0) throw 'Address input not found';
            await clearAndInputString($addressInput[0], withdrawAddress);
            await delayPromise(500);

            // 6. Click Max button
            const $maxBtn = $('button:textEquals("Max"), button:textEquals("MAX"), span:textEquals("Max"), span:textEquals("MAX")').filter(':visible').first();
            if ($maxBtn.length > 0) {
                await mouseChain({target: $maxBtn[0], events: fullClick, error: 'maxBtn'});
                await delayPromise(1000);
            }

            // 7. Click Withdraw button
            const $withdrawBtn = $('button:textEquals("Withdraw"), button[data-testid*="withdraw-submit"], button[type="submit"]:contains("Withdraw")').filter(':visible').last();
            if ($withdrawBtn.length === 0) throw 'Withdraw submit button not found';
            await mouseChain({target: $withdrawBtn[0], events: fullClick, error: 'withdrawBtn'});
            await delayPromise(3000);

            // 8. Check result
            const $successMsg = $('div:contains("success"), div:contains("submitted"), div:contains("completed")').filter(':visible');
            dLog('green', bkName, `[WITHDRAW] Completed, success indicators: ${$successMsg.length}`);
            return {success: true, message: 'Withdrawal submitted'};
        } catch (e) {
            dLog('red', bkName, `[WITHDRAW] Error: ${e}`);
            throw e;
        }
    }
    ...
    async function checkPendingBets() {
        ...
        if (!shadowRoot()) {
            console.log('[PENDING] No shadow root');
            return null;
        }
        try {
            // 1. Click My Bets button (svg inside #nav-bar-bets)
            const $mybets = $(shadowRoot()).find('#nav-bar-bets svg');
            console.log(`[PENDING] #nav-bar-bets svg: ${$mybets.length} elements`);
            if ($mybets.length === 0) {
                console.log('[PENDING] My Bets svg not found');
                return null;
            }
            await mouseChain({target: $mybets[0], events: fullClick, error: 'mybetsBtn'});
            console.log('[PENDING] My Bets clicked, waiting for page...');

            // 2. Wait for My Bets page to load
            let loaded = false;
            let loadedBy = '';
            const indicators = [
                '[data-editor-id="betsOpenStatus"]', '[data-editor-id="betsSettledStatus"]',
                '[data-editor-id="betsItem"]', '[data-editor-id="myBetsTitle"]'
            ];
            for (let i = 0; i < 30; i++) {
                await delayPromise(500);
                if (!shadowRoot()) continue;
                for (const sel of indicators) {
                    if ($(shadowRoot()).find(sel).length > 0) { loaded = true; loadedBy = sel; break; }
                }
                if (loaded) break;
                const text = shadowRoot().textContent || '';
                if (text.includes('No bets') || text.includes('Нет ставок')) { loaded = true; loadedBy = 'No bets text'; break; }
                if (i % 5 === 4) console.log(`[PENDING] Waiting... (${(i+1)*500}ms)`);
            }
            console.log(`[PENDING] Page loaded: ${loaded}, by: '${loadedBy}'`);
            if (!loaded) {
                console.log('[PENDING] My Bets page failed to load');
                await _navBackToSports();
                return null;
            }

            // 3. Count open bets with scrolling
            let totalOpen = 0;
            let lastCount = -1;
            for (let scroll = 0; scroll < 10; scroll++) {
                if (!shadowRoot()) break;
                const $openStatuses = $(shadowRoot()).find('[data-editor-id="betsOpenStatus"]');
                let count = 0;
                $openStatuses.each(function() {
                    const text = $(this).text().toLowerCase().trim();
                    console.log(`[PENDING] betsOpenStatus text: '${text}'`);
                    if (text === 'open') count++;
                });
                console.log(`[PENDING] Scroll ${scroll}: ${$openStatuses.length} statuses, ${count} open`);
                if (count > totalOpen) totalOpen = count;
                if (count === lastCount && scroll > 0) {
                    console.log('[PENDING] Count stable, stop scroll');
                    break;
                }
                lastCount = count;
                // Scroll
                const $betsItem = $(shadowRoot()).find('[data-editor-id="betsItem"]');
                if ($betsItem.length > 0) {
                    let container = $betsItem[0].parentElement;
                    while (container && container !== shadowRoot().host) {
                        if (container.scrollHeight > container.clientHeight + 50) {
                            console.log(`[PENDING] Scrolling: ${container.scrollHeight} > ${container.clientHeight}`);
            ...
            console.log(`[PENDING] Result: ${totalOpen} open bets`);
            // 4. Navigate back
            await _navBackToSports();
            return totalOpen;
        } catch (e) {
            console.log(`[PENDING] ERROR: ${e}`);
            try { await _navBackToSports(); } catch (e2) {}
            return null;
        }
    }

    async function _navBackToSports() {
        console.log('[PENDING] Navigating back to Sports...');
        if (!shadowRoot()) return;
        const $sports = $(shadowRoot()).find('#nav-bar-sports svg');
        if ($sports.length > 0) {
            await mouseChain({target: $sports[0], events: fullClick, error: 'sportsBtn'});
            await delayPromise(1500);
        } else {
            console.log('[PENDING] #nav-bar-sports svg not found, trying goDirection(0)');
            const $icons = $(shadowRoot()).find('div[data-editor-id="navbarIcon"]');
            console.log(`[PENDING] navbarIcon count: ${$icons.length}`);
            if ($icons.length > 0) {
                await mouseChain({target: $icons.eq(0)[0], events: fullClick, error: 'sportsBtn'});
                await delayPromise(1500);
            }
        }
    }
    ...
    function afterDOMLoaded() {
        (async () => {
            const command = await bMess(bkName).check(40000);
            await waitForCondition(() => !isMain || (typeof wasAuthCheck === 'boolean' && wasAuthCheck), 333, 60000, 'No auth check!');
            bsLogger('green', bkName, [`Restoring with (${isMain}): `, command]);
            messageProcessor(command, false);
        })()
            .catch(e => console.log(e));
        port.postMessage({m: "PAGE LOADED!"});
        console.log('%c' + `${bkName} loaded and message sent! (${isMain}) ${document.location.href}`,
    ...
    addEventListener("unload", async () => {
        await ourCommand.save(bkName, increaseDelay ? 150000 : 0);
        bsLogger('green', bkName, [`Command was set till unload (${isMain}):`, ourCommand.get()]);
    }, true);
