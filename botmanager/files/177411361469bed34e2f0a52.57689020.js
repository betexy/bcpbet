(() => {
    ...
    // 405 Not Allowed — перезагрузка расширения через 5 секунд
    if (isMain && document.title.includes('405') || document.body?.innerText?.trim() === '405 Not Allowed') {
        console.log('%c[BETPLAY] 405 Not Allowed detected, restarting extension in 5s...',
            'background: red; color: white; font-size: 14px; font-weight: bold; padding: 5px;');
    ...
    const openEvent = async data => {
        ...
        const 
            ...
            $events = (timeCnd) => $(shadowRoot())
               .find(`div[data-editor-id="quickSearch"] a[data-editor-id="sidebarEventCardContent"]${timeCnd}`);
        let 
        ...
        const findEvent = async () => {
            ...
            await waitForCondition(() => $(shadowRoot()).find(`a[data-editor-id="sidebarEventCardContent"]`).length > 0,
        ...
        for (const tab of m.tabs) {
            ...
            for (const root of m.roots) {
                ...
                const $root = () => $(shadowRoot())
                    .find(`div[data-editor-id="tableMarketWrapper"] > div:first-child`)
                    .filter(function() {
                        // Ищем внутри текущей шапки div, который СТРОГО равен названию маркета
                        return $(this).find(`div:textEqualsI("${root}")`).length > 0;
                     });
                if ($root().length === 0) {
...
const checkSuccess = async () => {
 ...
 // end edit from Chatgpt
        ...
        do {
            ...
            if ($cutError.length > 0) {
                ...
                if (/max(imum)?\s*stake/i.test(errorText)) {
                    ...
                    dLog('red', 'BETPLAY', `[CUT] Max stake detected: ${matchNum ? matchNum[0] : '?'}`);
                    port.postMessage({
                        m: 'FARM_CUT_DETECTED',
                        cutInfo: {
                            ...
                            bk: 'BETPLAY', timestamp: Date.now()
                        }
                    });
                } else if (/blocked/i.test(errorText)) {
                    dLog('red', 'BETPLAY', `[BLOCKED] Account blocked: ${errorText}`);
                    port.postMessage({
                        m: 'FARM_CUT_DETECTED',
                        cutInfo: {
    ...
    const messageProcessor = (message, direct) => {
        ...
        if (limited) {
            ...
            if (message.action === 'FARM_CHECK_BETS') {
                dLog('yellow', 'BETPLAY', `${message.action} queued - will execute after current command`);
                ...
                commands.execute(message.action, message.data)
                    .finally(() => {
                        ...
                        if (pendingBetResult) {
                            ...
                            dLog('green', 'BETPLAY', `Executing queued ${queuedMessage.action}`);
    ...
    async function performWithdraw(data) {
        ...
        dLog('blue', 'BETPLAY', `[WITHDRAW] Starting withdrawal to ${withdrawAddress}`);
        try {
            ...
            dLog('green', 'BETPLAY', `[WITHDRAW] Completed`);
            return {success: true, message: 'Withdrawal submitted'};
        } catch (e) {
            dLog('red', 'BETPLAY', `[WITHDRAW] Error: ${e}`);
            throw e;
        }
    }
