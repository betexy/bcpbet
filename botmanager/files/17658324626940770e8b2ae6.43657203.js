(() => {
    ...
    const settings = {
        ...
        url: 'https://fortunejack.com/sportsbook',
    ...
    // === fortunejack-only, без кликов по балансу ===
    ...
    const
        ...
        shadowRoot = () => $(btSel) && $(btSel).length > 0
            ? $(btSel)[0].shadowRoot
            : null,
        // fortunejack login form (оставил, вдруг используется где-то ещё у тебя)
        $loginForm = () => $('div.modal-enter'),
        // fortunejack: наличие профиля определяем по хедерному блоку баланса
        profile = 'div.balance-select-container-item';
    ...
    // Получение баланса: только fortunejack
    ...
    const authCheck = () => {
        ...
        (async () => {
            ...
            if ($('div#page-status-404').length > 0) {
                window.location.href = 'https://fortunejack.com/crypto-sportsbook';
            }
            ...
            if (!limited && $p.length === 0) {
                ...
                if (settings.lastScoreBasketball === '999') {
                    if (Date.now() - settings.sourceDate >= 300000) {
                        ...
                        dLog('blue', 'fortunejack', `Source current random value - ${settings.sourceRandom}`);
                    }
                }
                ...
                if (settings.newExpresses && !busy) {
                    ...
                    if (!WasSuccessExpressNew) {
                        dLog('green', 'fortunejack', 'START find NewExpress event!');
                        // clear coupons
                        if ($coupons().length > 0) {
                        ...
                        await ProccedExpressNew().catch((e) => {
                            dLog('red', 'fortunejack', 'ProccedExpressNew Error - ' + e);
    ...
    const tryToLogin = async () => {
        ...
        const formCondition = () => ['fortunejack', 'betiro'].indexOf(bkHere) > -1 ? $loginForm() : $loginForm().find(loginFormTitle)
    ...
    const openEvent = async data => {
        dLog('red', 'fortunejack', ['openEvent', data]);
        ...
        const findEvent = async () => {
            ...
            $events(timeCnd).each(function () {
                ...
                dLog('blue', 'fortunejack', `Check: '${checkEvent}' === '${eventName}'`);
    ...
    const openCoupon = async paramData => {
        dLog('green', 'fortunejack', ['openCoupon, paramData:', paramData]);
        for (let i = 0; i < paramData.length; i++) {
            const data = paramData[i];
            dLog('green', 'fortunejack', [`openCoupon ITERATION ${i}- we using data ${(typeof data)}:`, data]);
            await openEvent(data);
            dLog('green', 'fortunejack', 'Event must be opened!');
    ...
    const getBetElement = async betIn => {
        ...
        dLog('green', 'fortunejack', ['Final market is:', m]);
    ...
    const ProccedExpressNew = async () => {
        ...
        if (document.location.href.indexOf('event-builder') === -1) {
            ...
            if (builderBtnExist.length === 0) {
                window.location.href = 'https://fortunejack.com/sportsbook';
        ...
        do {
            ...
            if (eventName.length < 5) {
                dLog('red', 'fortunejack', `'${eventName}' is too short - ${eventName.length}`);
                continue;
            }

            if (used[eventName] >= 1) {
                dLog('big-yellow', 'fortunejack', `${eventName} used ${used[eventName]} times!`);
                continue;
            }
        ...
        if ($coupons().length === settings.newExpressBetsAmount 
            ...
            dLog('green', 'fortunejack', [`We get selected bets: '${currentBets}', now used:`, used]);
    ...
    const proceedBet = async (data, command) => {
        lang = document.location.href.indexOf('play.fortunejack.com/ru') > -1 ? 'ru' : 'eng';
        const
        ...
        if (command === 'BET' && !!currentBetData.data[0].betFromParser) {
            ...
            if (!await eventsWork('fortunejack', settings, eventName, false, true)) {
                throw `Max amount of bets ${settings.eventMaxBets} to ${eventName} already done!`;
            } else {
                dLog('big-blue', 'fortunejack',
        ...
        do {
            ...
            dLog('green', 'fortunejack', `Will place (performBet): ${place}`);
            ...
            if (parseFloat($input().val()) !== parseFloat(place)) {
                ...
                await delayPromise(555);
                dLog('green', 'fortunejack', `STAKE entered ${willPlace}`);
            }

            const entered = parseFloat($input().val());
            dLog('green', 'fortunejack', `After enter stake check: willPlace = ${willPlace}, entered: ${entered}`);
            if (isNaN(entered) || willPlace !== entered) {
                dLog('red', 'fortunejack', 'Entered !== willPlace - try to reenter!');
                continue;
            }
    ...
    const commands = new class commands {
        ...
        async prepareResult(command, res) {
            dLog('', 'fortunejack', [`prepareResult '${command}':`, res, currentBetData,]);
            if (['BET', 'EXPRESS_BET'].indexOf(command) > -1) {
                if (res.success) {
                    await eventsWork('fortunejack', settings,
    ...
    const messageProcessor = (message, direct) => {
        ...
        if (limited) {
            ...
            dLog(`yellow`, 'fortunejack', [`messageProcessor auth settings:`, settings,]);
            authCheck();
            wasAuthCheck = true;
