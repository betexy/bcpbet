(function () {
    ...
    const port = window.self === window.top
        ? chrome.runtime.connect({name: `port_sol249`})
    ...
    const getBalance = returnNull => {
        const $b = $('span.user-top__curr-val:visible');
        if ($b.length > 0) {
    ...
    const authCheck = function () {
        ...
        (async () => {
            await closeAllWeNeed({
                'div.payment-modal__close': 'div.payment-modal__close',
                'button.notification-block__button': 'button.notification-block__button',
            });
            const $logLink = $('#user-top__login-link');
            if ($logLink.length > 0) {
                // Hint: Log In
            ...
            .catch(e => dLog('red', 'sol249', `authCheck error: ${e}`))
            .then(delayFunction(settings.authCheckInterval))
            .then(authCheck);
    };

    const submitLoginForm = async () => {
        await clearAndSimulate($('input#email')[0], settings.login);
        await delayPromise(1555);
        await clearAndSimulate($('input#password')[0], settings.password);
        await delayPromise(1555);
        await mouseChain({target: $('button[type="submit"]')[0], events: fullClick, error: 'form submit'});
        await delayPromise(555);
        authClicked = Date.now();
        const $checkBalance = await waitForElement('span.user-top__curr-val:visible', 333, 7777).catch(() => $([]));
    ...
    const tryToLogIn = async () => {
        ...
        const $buttonLogin = $('#user-top__login-link');
        const $formLogin = $('div.registration-form__wrapper');
        if ($formLogin.length > 0) {
            //check and switch login
            if ($formLogin.find('div.registration-form__body-title:textEquals("Continue with")').length === 0) {
                await mouseChain({target: $formLogin.find('a[href="/signin"]')[0], events: fullClick, error: 'login tab'});
                await delayPromise(1555);
            }
            ...
            await delayPromise(555);
            await waitForElement('div.registration-form__wrapper', 333, 4444);
            await delayPromise(555);
            
            await submitLoginForm();
        }
        dLog('', 'sol249', 'Auth clicked!');
        return "auth_clicked";
    };
    ...
    const openEvent = async data => {
        dLog('red', 'sol249', ['openEvent', data]);
        ...
        const switchToSport = async () => {
            dLog('red', 'sol249', `switchToSport: ${sport}`);
        ...
        if (checkWeAreThere()) {
            dLog('green', 'sol249', 'We probably on event page!');
            return 'We probably on event page!'
        }
        const $goStart = await waitForElement([
            'li.tg__submenu__item:textEquals("Live Event List")',
        ], 200, 10000)
            .catch(() => $([]));
        if ($goStart.length > 0) {
        ...
        const withCountriesSel = 'div.tg__live_filter_item div.tg__checkbox_ico[title="Country"]';
        ...
        if ($el.length === 1) {
            await mouseChain({
                target: $el.find('div[title="Available odds"]')[0],
                events: fullClick,
                scroll: true,
                error: 'EVENT'
            });
    ...
    const openCoupon = async paramData => {
        dLog('green', 'sol249', ['openCoupon, paramData:', paramData]);
        for (let i = 0; i < paramData.length; i++) {
            const data = paramData[i];
            dLog('green', 'sol249', [`openCoupon ITERATION ${i}- we using data ${(typeof data)}:`, data]);
            await openEvent(data);
            dLog('green', 'sol249', 'Event must be opened!');
            const $element = await getBetElement(data);
            console.log($element);
            let coefWeWaitFor = $element.find('div.coef').trt();
            dLog('green', 'sol249', 'We got element! Coef: ' + coefWeWaitFor);
            const waitForCouponVisibleStarted = Date.now();
            let elementWasClicked = 0;
            let performElementClick = async function () {
                dLog('green', 'sol249', 'performElementClick ' + elementWasClicked + ' (' + coefWeWaitFor
            ...
            if (paramData.length > 1) {
                dLog('red', 'sol249', `Express here! ${i}/${(paramData.length - 1)}`);
    ...
    const proceedBetSport = async (data, balance) => {
        ...
        if (!!currentBetData.data[0].betFromParser) {
            const checkRes = await eventsWorkAll('solcasino',
                settings.eventMaxBets, settings.eventTimeLimit,
                currentBetData.data, false, true);
            if (checkRes !== 'OK') {
                dLog('red', 'sol249', `We got errors: ${checkRes}`);
                throw checkRes;
            } else {
                dLog('big-blue', 'sol249',
                ...
                for (const d of currentBetData.data) {
                    ...
                    dLog('blue', 'sol249', `${settings.eventMaxBets} for ${eventName} not reached`);
                }
            }
        }
        ...
        do {
            ...
            dLog('green', 'sol249', `Will place (performBet): ${willPlace}, balance: ${balance}`);
            ...
            dLog('green', 'sol249', `STAKE entered ${willPlace}`);
            let entered = parseFloat($input.val().replaceAll(' ', ''));
            dLog('green', 'sol249', `After enter stake check: willPlace = ${willPlace}, entered: ${entered}`);
            if (isNaN(entered) || willPlace !== entered) {
                dLog('red', 'sol249', 'Entered !== willPlace - try to reenter!');
                continue;
            }
    ...
    const collectBetResultsSports = async (inD, command, balance, inplay) => {
        ...
        dLog('green', 'sol249', [`collectBetResults, limit: ${limit}, inplay: ${inplay}, data: `, data]);
        const $expandCoupon = () => $couponFrame().find('div#tstSlnBetSlipHeaderClosed');
            ...
            if (data.length === 0 || data.indexOf(external_id) > -1) {
                ...
                while (!opened && tries < 5) {
                    ...
                    await waitForCondition(() => $(rowSel).eq(i).next().attr('class') === '',
                        333, 10000, 'Bad next!')
                        .catch(() => (opened = false, tries++, dLog('red', 'sol249', `NOT OPENED! ${tries}`)));
                    await delayPromise(1000);
                    opened = true;
                }
        ...
        dLog('green', 'sol249', ['Collected', collected]);
        await mouseChain({target: $('#maPageCloseButton')[0], events: fullClick, error: ''})
            .catch(() => dLog('red', 'sol249', 'No close btn'));
        return {success: true, message: collected};
    };
    ...
    const sportCommands = new class SportCommands {
        ...
        async execute(command, data, balance) {
            ...
            dLog('green', 'sol249', [command, data]);
            const res = await this.cLinks[command](data, balance)
                .catch(e => ({
                    success: false,
                    message: `${command}: ${e}, ${formatStack(e.stack)}`
                }));
            dLog(res.success ? 'green' : 'red', 'sol249', [`${command} sports result was set:`, res]);
            await bMess('SolSportResult').set(res);
            return res;
        }
    };

    const sportProcessor = command => {
        dLog('green', 'sol249', `SportProcessor: ${command.action}`);
        if (sportCommands.exists(command.action)) {
            ...
            dLog('red', 'sol249', ['Unknown Sport command:', command]);
        }
    };

    const executeSportCommand = async (data, command, balance) => {
        dLog('green', 'sol249', `executeSportCommand: ${command}, ${balance}`);
        await bMess('SolSportCommand').set({action: command, data, balance});
        const res = await bMess('SolSportResult')
    ...
    const commands = new class commands {
        ...
        async execute(command, data) {
            currentBetData.init(data);
            //dLog('green', 'sol249', [`commands execute ${command}`, data, formatStack((new Error()).stack)]);
            const res = await this.cLinks[command](data, command, getBalance()).catch(e => ({
                success: false,
                message: `${command}: ${e}, ${formatStack(e.stack)}`
            }));
            dLog(res.success ? 'green' : 'red', 'sol249', [`${command} result:`, res]);
        ...
        async prepareResult(command, res) {
            if (['BET', 'EXPRESS_BET'].indexOf(command) > -1) {
                ...
                if (!!currentBetData.data[0].betFromParser && resultData.status !== 'LIMITED') {
                    ...
                    resultData.bookmaker = 'SOLCASINO';
    ...
    const messageProcessor = message => {
        dLog('green', 'sol249', [`messageProcessor (${busy})`, message]);
    ...
    addEventListener("unload", function () {
        if (ourCommand.isSet()) {
            dLog('green', 'sol249', ['Command was set till unload:', ourCommand.get()]);
            // Hint: We allow payment to be done in 120 seconds for DEPOSIT
            bMess('SOL249_COMMAND', true).set(ourCommand.get(), increaseDelay ? 130000 : 0);
        }
    }, true);
    ...
    function afterDOMLoaded() {
        ...
        if (window.self === window.top) {
            ...
            bMess('SOL249_COMMAND', true).check(40000, true)
                .then(currentCommand => {
                    dLog('orange', 'sol249', [
                ...
                .catch(() => dLog('color: darkgray;', 'sol249', 'No command!'));
        } else if (document.location.href.indexOf('sport.sol.dgtnsptpub.com/SportsBook/Home') > -1) {
            // Hint: sports frame (here the most of work)
            (async () => {
                while (!stopSports) {
                    const csc = await bMess('SolSportCommand').check(10000, true)
                        .catch(() => null);
                    if (csc !== null) {
                        sportProcessor(csc);
                    }
            ...
            dLog('green', 'sol249', 'Sport processor initialized!');
        }
    }
