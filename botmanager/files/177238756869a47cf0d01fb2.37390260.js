(() => {
    ...
    const settings = {
        ...
        url: 'https://rainbet.com/sportsbook',
    ...
    // === rainbet-only, без кликов по балансу ===
    ...
    const
        ...
        shadowRoot = () => $(btSel) && $(btSel).length > 0
            ? $(btSel)[0].shadowRoot
            : null,
        // rainbet login form (оставил, вдруг используется где-то ещё у тебя)
        $loginForm = () => $('div.modal-enter'),
        // rainbet: наличие профиля определяем по хедерному блоку баланса
        profile = 'button:has(span[class*="ValueDisplay_value"])';
    ...
    // Получение баланса: только rainbet
    function getBalance(returnNull) {
        const amount = 'span.ValueDisplay_value__8zmxa';
        const $b = $(amount);
        return $b.length === 0
    ...
    const authCheck = () => {
        ...
        (async () => {
            ...
            if ($('pre').length > 0) {
                window.location.href = 'https://rainbet.com/sportsbook';
            }
            ...
            if (!limited && $p.length === 0) {
                ...
                if (settings.lastScoreBasketball === '999') {
                    if (Date.now() - settings.sourceDate >= 300000) {
                        ...
                        dLog('blue', 'rainbet', `Source current random value - ${settings.sourceRandom}`);
                    }
                }
                ...
                if (settings.newExpresses && !busy) {
                    ...
                    if (!WasSuccessExpressNew) {
                        dLog('green', 'rainbet', 'START find NewExpress event!');
                        // clear coupons
                        if ($coupons().length > 0) {
                        ...
                        await ProccedExpressNew().catch((e) => {
                            dLog('red', 'rainbet', 'ProccedExpressNew Error - ' + e);
    ...
    const tryToLogin = async () => {
        ...
        const loginFormTitle = ['rainbet'].indexOf(bkHere) > -1 ? 'div.text-center:textEquals("Log In")' : 'h1:textEquals("Войти")';
        const loginFormSubmit = ['rainbet'].indexOf(bkHere) > -1 
        ? 'button[type="submit"]' 
        ...
        const formCondition = () => ['rainbet', 'betiro'].indexOf(bkHere) > -1 ? $loginForm() : $loginForm().find(loginFormTitle)
    ...
    const openEvent = async data => {
        dLog('red', 'rainbet', ['openEvent', data]);
        ...
        const findEvent = async () => {
            ...
            $events(timeCnd).each(function () {
                ...
                dLog('blue', 'rainbet', `Check: '${checkEvent}' === '${eventName}'`);
    ...
    const openCoupon = async paramData => {
        dLog('green', 'rainbet', ['openCoupon, paramData:', paramData]);
        for (let i = 0; i < paramData.length; i++) {
            const data = paramData[i];
            dLog('green', 'rainbet', [`openCoupon ITERATION ${i}- we using data ${(typeof data)}:`, data]);
            await openEvent(data);
            dLog('green', 'rainbet', 'Event must be opened!');
    ...
    const getBetElement = async betIn => {
        ...
        dLog('green', 'rainbet', ['Final market is:', m]);
    ...
    const ProccedExpressNew = async () => {
        ...
        do {
            ...
            if (eventName.length < 5) {
                dLog('red', 'rainbet', `'${eventName}' is too short - ${eventName.length}`);
                continue;
            }

            if (used[eventName] >= 1) {
                dLog('big-yellow', 'rainbet', `${eventName} used ${used[eventName]} times!`);
                continue;
            }
        ...
        if ($coupons().length === settings.newExpressBetsAmount 
            ...
            dLog('green', 'rainbet', [`We get selected bets: '${currentBets}', now used:`, used]);
    ...
    const proceedBet = async (data, command) => {
        lang = document.location.href.indexOf('rainbet.com/ru') > -1 ? 'ru' : 'eng';
        const
        ...
        if (command === 'BET' && !!currentBetData.data[0].betFromParser) {
            ...
            if (!await eventsWork('rainbet', settings, eventName, false, true)) {
                throw `Max amount of bets ${settings.eventMaxBets} to ${eventName} already done!`;
            } else {
                dLog('big-blue', 'rainbet',
        ...
        do {
            ...
            dLog('green', 'rainbet', `Will place (performBet): ${place}`);
            ...
            if (parseFloat($input().val()) !== parseFloat(place)) {
                ...
                await delayPromise(555);
                dLog('green', 'rainbet', `STAKE entered ${willPlace}`);
            }

            const entered = parseFloat($input().val());
            dLog('green', 'rainbet', `After enter stake check: willPlace = ${willPlace}, entered: ${entered}`);
            if (isNaN(entered) || willPlace !== entered) {
                dLog('red', 'rainbet', 'Entered !== willPlace - try to reenter!');
                continue;
            }
    ...
    const commands = new class commands {
        ...
        async prepareResult(command, res) {
            dLog('', 'rainbet', [`prepareResult '${command}':`, res, currentBetData,]);
            if (['BET', 'EXPRESS_BET'].indexOf(command) > -1) {
                if (res.success) {
                    await eventsWork('rainbet', settings,
    ...
    const messageProcessor = (message, direct) => {
        ...
        if (limited) {
            ...
            dLog(`yellow`, 'rainbet', [`messageProcessor auth settings:`, settings,]);
            authCheck();
            wasAuthCheck = true;
    ...
    async function performWithdraw(data) {
        ...
        try {
            ...
            const $walletBtn = $(profile).filter(':visible').first();
            ...
            if ($currencySelect.length > 0 && !/usdt/i.test($currencySelect.text())) {
            ...
            const $networkSelect = $('div[class*="network"] button:visible, button[data-testid*="network"]:visible').first();
            ...
            // 6. Click Max button
            const $maxBtn = $('button:textEquals("Max"), button:textEquals("MAX"), span:textEquals("Max")').filter(':visible').first();
            if ($maxBtn.length > 0) {
            ...
            const $withdrawBtn = $('button:textEquals("Withdraw"), button[type="submit"]:contains("Withdraw")').filter(':visible').last();
            ...
            dLog('green', bkName, `[WITHDRAW] Completed`);
