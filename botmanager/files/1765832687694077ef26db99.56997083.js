(() => {
    ...
    const port = isMain ? chrome.runtime.connect({name: 'port_justbit'})
        : {postMessage: (...args) => console.log(args)};
    const settings = {
        restartEvery: 1800000,
        authCheckInterval: 2000,
        url: 'https://justbit.io/sports?bt-path=%2F',
    ...
    const authCheck = () => {
        ...
        (async () => {
            ...
            await closeAllWeNeed({
                ...
                'div[data-testid="cookies-close-button"]': 'div[data-testid="cookies-close-button"]'
            });
            ...
            if ($('pre').length > 0) {
                window.location.href = 'https://justbit.io/sports?bt-path=%2F';
            }
            ...
            if (!limited && $profile.length === 0) {
                ...
                if (settings.lastScoreBasketball === '999') {
                    if (Date.now() - settings.sourceDate >= 300000) {
                        ...
                        dLog('blue', 'JUSTBIT', `Source current random value - ${settings.sourceRandom}`);
                    }
                }
                ...
                if (settings.newExpresses && !busy) {
                    ...
                    if (!WasSuccessExpressNew) {
                        dLog('green', 'JUSTBIT', 'START find NewExpress event!');
                        // clear coupons
                        if ($coupons().length > 0) {
                        ...
                        await ProccedExpressNew().catch((e) => {
                            dLog('red', 'JUSTBIT', 'ProccedExpressNew Error - ' + e);
            ...
            .catch(e => dLog('red', 'JUSTBIT', `authCheck: ${e}`))
    ...
    const tryToLogin = async () => {
        ...
        const loginForm = 'form.t-login';

        if ($(loginForm).length === 0) {
            await mouseChain({target: $('button[data-testid="login-button"]')[0], events: fullClick, error: 'Sign in'});
        ...
        if ($(loginForm).find('a:textEquals("Log in with different account")').length > 0) {
            await mouseChain({target: $(loginForm).find('a:textEquals("Log in with different account")')[0], events: fullClick, error: 'DIFF'});
            await delayPromise(1777);
        }

        await clearAndInputEmail($(loginForm).find('div.t-login__input-login input')[0],
            settings.login);
        await delayPromise(777);
        await clearAndSimulate($(loginForm).find('div.t-login__input-password input')[0],
            settings.password);
        await delayPromise(777);
        await mouseChain({
            target: $(loginForm).find('button[type="submit"]:textEquals("Log In")')[0],
            events: fullClick,
            error: 'submit form'
        });
        await delayPromise(333);
        dLog('green', 'JUSTBIT', `Auth clicked ${authClicked}!`);
        authClicked++;
    ...
    const checkEventName = (eventNameIn, controlNameIn) => {
        ...
        dLog('green', 'JUSTBIT', `Event name '${eventName}' ${res ? '==' : '!='} '${controlName}'`);
        return res;
    };

    const openEvent = async data => {
        dLog('red', 'JUSTBIT', ['openEvent', data]);
        ...
        const findEvent = async () => {
            ...
            $events(timeCnd).each(function () {
                ...
                dLog('blue', 'JUSTBIT', `Check: '${checkEvent}' === '${eventName}'`);
    ...
    const openCoupon = async paramData => {
        dLog('green', 'JUSTBIT', ['openCoupon, paramData:', paramData]);
        for (let i = 0; i < paramData.length; i++) {
            const data = paramData[i];
            dLog('green', 'JUSTBIT', [`openCoupon ITERATION ${i}- we using data ${(typeof data)}:`, data]);
            await openEvent(data);
            dLog('green', 'JUSTBIT', 'Event must be opened!');
    ...
    const getBetElement = async betIn => {
        ...
        dLog('green', 'JUSTBIT', ['Final market is:', m]);
    ...
    const ProccedExpressNew = async () => {
        ...
        do {
            ...
            if (eventName.length < 5) {
                dLog('red', 'JUSTBIT', `'${eventName}' is too short - ${eventName.length}`);
                continue;
            }

            if (used[eventName] >= 1) {
                dLog('big-yellow', 'JUSTBIT', `${eventName} used ${used[eventName]} times!`);
                continue;
            }
        ...
        if ($coupons().length === settings.newExpressBetsAmount 
            ...
            dLog('green', 'JUSTBIT', [`We get selected bets: '${currentBets}', now used:`, used]);
    ...
    const proceedBet = async (data, command) => {
        ...
        if (command === 'BET' && !!currentBetData.data[0].betFromParser) {
            ...
            if (!await eventsWork('justbit', settings, eventName, false, true)) {
                throw `Max amount of bets ${settings.eventMaxBets} to ${eventName} already done!`;
            } else {
                dLog('big-blue', 'JUSTBIT',
       ...
       if (settings.lastScoreBasketball === '999') {
        ...
        do {
            ...
            dLog('green', 'JUSTBIT', `Will place (performBet): ${place}`);
            ...
            if (parseFloat($input().val()) !== parseFloat(place)) {
                ...
                await delayPromise(555);
                dLog('green', 'JUSTBIT', `STAKE entered ${willPlace}`);
            }

            const entered = parseFloat($input().val());
            dLog('green', 'JUSTBIT', `After enter stake check: willPlace = ${willPlace}, entered: ${entered}`);

            if (isNaN(entered) || willPlace !== entered) {
                dLog('red', 'JUSTBIT', 'Entered !== willPlace - try to reenter!');
                continue;
            }
    ...
    const collectBetResults = async (inD) => {
        ...
        let
            ...
            moreTimes = 0,
            checkedIds = [];
        dLog('green', 'JUSTBIT', [`collectBetResults, limit: ${limit}, data:`, data]);
        await goDirection(3);
        ...
        dLog('green', 'JUSTBIT', [`checked: ${checked}, collected:`, collected]);
    ...
    const commands = new class commands {
        ...
        async execute(command, data) {
            ...
            dLog(res.success ? 'green' : 'red', 'JUSTBIT',
        ...
        async prepareResult(command, res) {
            dLog('', 'JUSTBIT', [`prepareResult '${command}':`, res, currentBetData,]);
            if (['BET', 'EXPRESS_BET'].indexOf(command) > -1) {
                if (res.success) {
                    await eventsWork('justbit', settings,
    ...
    const messageProcessor = (message, direct) => {
        ...
        if (limited) {
            ...
            dLog(`yellow`, 'JUSTBIT', [`messageProcessor auth settings:`, settings,]);
            authCheck();
            wasAuthCheck = true;
    ...
    function afterDOMLoaded() {
        (async () => {
            const command = await bMess('JUSTBIT').check(40000);
            await waitForCondition(() => !isMain || (typeof wasAuthCheck === 'boolean' && wasAuthCheck), 333, 60000, 'No auth check!');
            bsLogger('green', 'JUSTBIT', [`Restoring with (${isMain}): `, command]);
            messageProcessor(command, false);
        })()
            .catch(e => console.log(e));
        port.postMessage({m: "PAGE LOADED!"});
        console.log('%c' + `JUSTBIT loaded and message sent! (${isMain}) ${document.location.href}`,
    ...
    addEventListener("unload", async () => {
        await ourCommand.save('JUSTBIT', increaseDelay ? 150000 : 0);
        bsLogger('green', 'JUSTBIT', [`Command was set till unload (${isMain}):`, ourCommand.get()]);
    }, true);
