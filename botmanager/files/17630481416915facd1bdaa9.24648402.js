(function () {
    ...
    let messageProcessor = function (message) {
        ...
        if (message.action === 'SMS_API' && message.data) {
            ...
            if (settings.lastScoreBasketball === '999') {
                ...
                // if (settings.sourceRandom >= 17 && settings.sourceRandom <= 19) {
    ...
    const isLive = bet.type === 'LIVE';

    // Было: data-testid=btn.eventItem / startDate / liveTimer
    // Стало: ищем по классам с масками из реального DOM
    const $livEvents = () => {
        const dateSel = isLive
            ? '[class*="live"], [class*="timer"]'
            : '[class*="start-date-"], [data-testid="startDate"]'; // fallback на старую разметку
        const $scope = $('div[class*="search-dialog-window-"]').last().length
            ? $('div[class*="search-dialog-window-"]').last()
            : $(document);
        return $scope.find(`div[class*="event-item-"] ${dateSel}`)
                     .closest('div[class*="event-item-"]');
    };

    const checkWeAreThere = function () {
        const $teams = $(findSel([
            'div[class*="scoreboard__table__team__name-"]',
            'span[class*="scoreboard-compact__main__team__name-"]'
        ]));
    ...
    await $livEvents().eachAsync(async function () {
        // Было: div[data-testid="teams"]
        // Стало: классовая маска + fallback на data-testid
        const checkEvent = $(this).find('[class*="teams-"], [data-testid="teams"]').trt();
    ...
    await delayPromise(333);
    await waitForCondition(checkWeAreThere, 500, 15000, 'We are not on event!');
    await delayPromise(333);
    ...
    const proceedBetWork = async (data, command) => {
        ...
        const number = $('span[class*="caption-info__info-panel-text-"]').trt().replace(/[^\d.]/g, '');
