(() => {
    ...
    const profile = 'div#balance';

    const port = isMain ? chrome.runtime.connect({name: 'port_roobet'})
        : {postMessage: (...args) => console.log(args)};
    const settings = {
        restartEvery: 1800000,
        authCheckInterval: 2000,
        url: 'https://roobet.com/sports/live',
    ...
    const
        ...
        shadowRoot = () => $(btSel) && $(btSel).length > 0
            ? $(btSel)[0].shadowRoot
            : null,
        $loginForm = () => $(findSel(['p:textEquals("Login to your account")', 'p:textEquals("Войти в учетную запись")'])).closest('form'),
        loginLink = () => findSel(['button:textEquals("Login")', 'button:textEquals("Вход")']);
    ...
    const findBalance = async () => {
        let selected = false;
        const balanceMenu = () => findSel(['div#balanceMenu ul a', 'div.MuiModal-root ul a']);
        const defaultLTC = () => findSel(['div#balanceMenu ul a span:textEquals("LTC")', 'div.MuiModal-root ul a span:textEquals("LTC")']);
        await mouseChain({target: $('div#balance button#balance-action-icon')[0], events: fullClick, error: 'Button balance bottom'});
        await delayPromise(888);
        await $(balanceMenu()).eachAsync(async function () {
            const currentAcc = parseFloat($(this).find('span:eq(1)').trt().replace(/[^\d.]/g, ''));
            if (currentAcc > 0) {
                selected = true;
                await mouseChain({target: $(this)[0], events: fullClick, error: 'SELECTED BALANCE'});
            }
        });
        if (!selected) {
            await mouseChain({target: $(defaultLTC())[0], events: fullClick, error: 'LTC'});
        }
    };

    const selectCurrencyMega = async () => {
        if ($('#balance svg[aria-label="usdt"]').length > 0) {
            if (getBalance(false) === 0) {
                await findBalance();
            }
        } else {
            await findBalance();
    ...
    const authCheck = () => {
        ...
        (async () => {
            ...
            await closeAllWeNeed({
                ...
                'button:textEquals("Cancel")': 'button:textEquals("Cancel")',
            });
            ...
            const $session = $(shadowRoot())
                .find('div:textEquals("Ваша сессия истекла")');
            if ($session.length > 0) {
                window.location.reload();
            }
            const $profile = await waitForElement(profile, 222, 7777).catch(() => $([]));
            if (!limited && $profile.length === 0) {
                ...
                if (settings.lastScoreBasketball === '999') {
                    if (Date.now() - settings.sourceDate >= 300000) {
                        ...
                        dLog('blue', 'ROOBET', `Source current random value - ${settings.sourceRandom}`);
                    }
                }
                ...
                if (settings.newExpresses && !busy) {
                    ...
                    if (!WasSuccessExpressNew) {
                        dLog('green', 'ROOBET', 'START find NewExpress event!');
                        // clear coupons
                        if ($coupons().length > 0) {
                        ...
                        await ProccedExpressNew().catch((e) => {
                            dLog('red', 'ROOBET', 'ProccedExpressNew Error - ' + e);
            ...
            .catch(e => dLog('red', 'ROOBET', `authCheck: ${e}`))
            .then(delayFunction(settings.authCheckInterval))
            .then(() => authCheck());
    };

    const goDirection = async (idx) => {
        const navIcon = await waitForCondition(() => $(shadowRoot()).find('div[data-editor-id="navbarIcon"]').length > 0,
            333, 2777, `No direction ${idx} link`).catch(() => $([]));

        if (navIcon.length > 0) {
            await mouseChain({
                target: $(shadowRoot()).find(`div[data-editor-id="navbarIcon"]:eq(${idx})`)[0],
                events: fullClick,
                error: `direction icon ${idx} click`
            });
        } else {
            const links = ['Featured', 'In-Play', 'All Sports', 'My Bets'];
            await mouseChain({
                target: $(shadowRoot()).find(`a:contains("${links[idx]}")`)[0],
                events: fullClick,
    ...
    const tryToLogin = async () => {
        ...
        await clearAndInputEmail($loginForm().find('input[name="username"]')[0],
            settings.login);
        ...
        dLog('green', 'ROOBET', `Auth clicked ${authClicked}!`);
        if ($profile.length === 0) {
            port.postMessage({
                answered: "auth_error",
                status: "ERROR",
    ...
    const checkEventName = (eventNameIn, controlNameIn) => {
        ...
        dLog('green', 'ROOBET', `Event name '${eventName}' ${res ? '==' : '!='} '${controlName}'`);
        return res;
    };

    const openEvent = async data => {
        dLog('red', 'ROOBET', ['openEvent', data]);
        ...
        const 
            ...
            $events = (main) => $(shadowRoot())
                .find(`${main}div[data-editor-id="eventCard"]`);
        let 
            sport = accordance[data.sport],
            eventLink = 'span[data-editor-id="searchResultHighlightItem"]:eq(0)',
            mainEventBlock = 'div[data-editor-id="subHeaderDropDown"] ',
            eventIndex = -1;
        ...
        const findEvent = async () => {
            let idx = 0;
            let searchInput = 'div[data-editor-id="quickSearchField"] input';
            let resultItem1 = 'div[style="width: 24px; height: 24px;"] + div:eq(0)';
            let resultItem2 = 'div[style="width: 24px; height: 24px;"] + div:eq(1)';

            if ($(shadowRoot()).find(searchInput).length === 0) {
                searchInput = 'input[type="search"]';
                resultItem1 = 'div[style="height: 24px; line-height: 24px;"]:eq(0)';
                resultItem2 = 'div[style="height: 24px; line-height: 24px;"]:eq(1)';
                eventLink = 'a';
                mainEventBlock = '';

                if ($(shadowRoot()).find(searchInput).length === 0) {
                    await mouseChain({target: $(shadowRoot()).find('svg[data-cy="ic-search"]')[0], events: fullClick, error: '$searchButton', scroll: true,});
                    await waitForCondition(() => $(shadowRoot()).find(searchInput).length > 0,
                        333, 5555, 'Search input is not available!');
                }
            }

            await ($(shadowRoot()).find(searchInput)[0].dispatchEvent(new ClipboardEvent('paste',{bubbles:true,cancelable:true,clipboardData:(()=>{try{const d=new DataTransfer();d.setData('text/plain',String(secondTeam));return d;}catch(_){return {getData:()=>String(secondTeam)}}})()})),($(shadowRoot()).find(searchInput)[0].value!==String(secondTeam))&&(($(shadowRoot()).find(searchInput)[0].value=String(secondTeam)),$(shadowRoot()).find(searchInput)[0].dispatchEvent(new Event('input',{bubbles:true})),$(shadowRoot()).find(searchInput)[0].dispatchEvent(new Event('change',{bubbles:true}))),Promise.resolve());
            await waitForCondition(() => $(shadowRoot()).find(`${mainEventBlock}div[data-editor-id="eventCard"]`).length > 0,
                333, 5555, 'Events list is not available!');
            await delayPromise(777);
            $events(mainEventBlock).each(function () {
                const $this = $(this);
                const team1 = $this.find(resultItem1).trt();
                const team2 = $this.find(resultItem2).trt();
                const checkEvent = `${team1} - ${team2}`.toLowerCase();
                dLog('blue', 'ROOBET', `Check: '${checkEvent}' === '${eventName}'`);
        ...
        // go to event page
        const $el = $events(mainEventBlock).eq(eventIndex).find(eventLink);
    ...
    const openCoupon = async paramData => {
        dLog('green', 'ROOBET', ['openCoupon, paramData:', paramData]);
        for (let i = 0; i < paramData.length; i++) {
            const data = paramData[i];
            dLog('green', 'ROOBET', [`openCoupon ITERATION ${i}- we using data ${(typeof data)}:`, data]);
            await openEvent(data);
            dLog('green', 'ROOBET', 'Event must be opened!');
    ...
    const getBetElement = async betIn => {
        ...
        dLog('green', 'ROOBET', ['Final market is:', m]);
    ...
    const ProccedExpressNew = async () => {
        ...
        do {
            ...
            if (eventName.length < 5) {
                dLog('red', 'ROOBET', `'${eventName}' is too short - ${eventName.length}`);
                continue;
            }

            if (used[eventName] >= 1) {
                dLog('big-yellow', 'ROOBET', `${eventName} used ${used[eventName]} times!`);
                continue;
            }
        ...
        if ($coupons().length === settings.newExpressBetsAmount 
            ...
            dLog('green', 'ROOBET', [`We get selected bets: '${currentBets}', now used:`, used]);
    ...
    const proceedBet = async (data, command) => {
        lang = document.location.href.indexOf('chips.gg/ru') > -1 ? 'ru' : 'eng';
        const
        ...
        if (command === 'BET' && !!currentBetData.data[0].betFromParser) {
            ...
            if (!await eventsWork('ROOBET', settings, eventName, false, true)) {
                throw `Max amount of bets ${settings.eventMaxBets} to ${eventName} already done!`;
            } else {
                dLog('big-blue', 'ROOBET',
        ...
        do {
            ...
            dLog('green', 'ROOBET', `Will place (performBet): ${place}`);
            ...
            if (parseFloat($input().val()) !== parseFloat(place)) {
                ...
                await delayPromise(555);
                dLog('green', 'ROOBET', `STAKE entered ${willPlace}`);
            }

            const entered = parseFloat($input().val());
            dLog('green', 'ROOBET', `After enter stake check: willPlace = ${willPlace}, entered: ${entered}`);
            if (isNaN(entered) || willPlace !== entered) {
                dLog('red', 'ROOBET', 'Entered !== willPlace - try to reenter!');
                continue;
            }
            ...
            for (let _cutTry = 0; _cutTry < 5; _cutTry++) {
                ...
                if ($cutError.length > 0) {
                    ...
                    if (/max(imum)?\s*stake/i.test(errorText)) {
                        ...
                        dLog('red', 'ROOBET', `[CUT] Max stake detected: ${matchNum ? matchNum[0] : '?'}`);
                        port.postMessage({
                            m: 'FARM_CUT_DETECTED',
                            cutInfo: {
                                ...
                                bk: 'ROOBET', timestamp: Date.now()
                            }
                        });
                    } else if (/blocked/i.test(errorText)) {
                        dLog('red', 'ROOBET', `[BLOCKED] Account blocked: ${errorText}`);
                        port.postMessage({
                            m: 'FARM_CUT_DETECTED',
                            cutInfo: {
        ...
        await delayPromise(333);
        await waitForCondition(() => $firstBet().length > 0, 333, 10000, 'no open bets');
    ...
    const collectBetResults = async (inD) => {
        ...
        let
            ...
            moreTimes = 0,
            checkedIds = [];
        dLog('green', 'ROOBET', [`collectBetResults, limit: ${limit}, data:`, data]);
        await goDirection(3);
        ...
        dLog('', 'ROOBET', [`checked: ${checked}, collected:`, collected]);
    ...
    const commands = new class commands {
        ...
        async execute(command, data) {
            ...
            dLog(res.success ? 'green' : 'red', 'ROOBET',
        ...
        async prepareResult(command, res) {
            dLog('', 'ROOBET', [`prepareResult '${command}':`, res, currentBetData,]);
            if (['BET', 'EXPRESS_BET'].indexOf(command) > -1) {
                if (res.success) {
                    await eventsWork('ROOBET', settings,
    ...
    const messageProcessor = (message, direct) => {
        ...
        if (limited) {
            ...
            && message.action !== 'FARM_CHECK_BETS'
            && message.action !== 'FARM_WITHDRAW'
            && busy) {
            port.postMessage({
                answered: message.action,
                status: "error",
            ...
            dLog(`yellow`, 'ROOBET', [`messageProcessor auth settings:`, settings,]);
            authCheck();
            wasAuthCheck = true;
    ...
    async function performWithdraw(data) {
        ...
        dLog('blue', 'ROOBET', `[WITHDRAW] Starting withdrawal to ${withdrawAddress}`);
        try {
            // 1. Click wallet/balance button
            const $walletBtn = $('button:has(span[data-testid="currency-amount"]), a[href*="wallet"], button[class*="balance"]').filter(':visible').first();
            ...
            dLog('green', 'ROOBET', `[WITHDRAW] Completed`);
            return {success: true, message: 'Withdrawal submitted'};
        } catch (e) {
            dLog('red', 'ROOBET', `[WITHDRAW] Error: ${e}`);
            throw e;
        }
    }
    ...
    async function checkPendingBets() {
        ...
        try {
            // 1. Click My Bets button
            const $mybets = $(shadowRoot()).find('a#nav-bar-bets');
            console.log(`[PENDING] a#nav-bar-bets: ${$mybets.length} elements`);
            if ($mybets.length === 0) {
                console.log('[PENDING] My Bets link not found');
                return null;
            }
    ...
    async function _navBackToSports() {
        ...
        const $inplay = $(shadowRoot()).find('a[href="/sports/live"]');
        if ($inplay.length > 0) {
            await mouseChain({target: $inplay[0], events: fullClick, error: 'inPlayBtn'});
            await delayPromise(1500);
        } else {
            console.log('[PENDING] a[href="/sports/live"] not found, trying goDirection(1)');
            await goDirection(1);
        }
    }
    ...
    function afterDOMLoaded() {
        (async () => {
            const command = await bMess('ROOBET').check(40000);
            await waitForCondition(() => !isMain || (typeof wasAuthCheck === 'boolean' && wasAuthCheck), 333, 60000, 'No auth check!');
            bsLogger('green', 'ROOBET', [`Restoring with (${isMain}): `, command]);
            messageProcessor(command, false);
        })()
            .catch(e => console.log(e));
        port.postMessage({m: "PAGE LOADED!"});
        console.log('%c' + `ROOBET loaded and message sent! (${isMain}) ${document.location.href}`,
    ...
    addEventListener("unload", async () => {
        await ourCommand.save('ROOBET', increaseDelay ? 150000 : 0);
        bsLogger('green', 'ROOBET', [`Command was set till unload (${isMain}):`, ourCommand.get()]);
    }, true);
