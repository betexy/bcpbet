(() => {
    ...
    const port = isMain ? chrome.runtime.connect({name: 'port_luckyblock'})
        : {postMessage: (...args) => console.log(args)};
    const settings = {
        restartEvery: 1800000,
        authCheckInterval: 2000,
        url: 'https://luckyblock.top/sports',
    ...
    const
        btSel = '#betby > div',
        usdtLink = 'div[class*="CollapsedBalanceAtom-module_item_"]',
    ...
    const authCheck = () => {
        ...
        (async () => {
            ...
            if ($('pre').length > 0) {
                window.location.href = 'https://luckyblock.top/sports';
            }

            const $loginLink = await waitForElement(loginLink, 300, 2999, true).catch(() => $([]));
            if (!limited && $loginLink.length > 0) {
                ...
                if (settings.lastScoreBasketball === '999') {
                    if (Date.now() - settings.sourceDate >= 300000) {
                        ...
                        dLog('blue', 'LUCKYBLOCK', `Source current random value - ${settings.sourceRandom}`);
                    }
                }
                ...
                if (settings.newExpresses && !busy) {
                    ...
                    if (!WasSuccessExpressNew) {
                        dLog('green', 'LUCKYBLOCK', 'START find NewExpress event!');
                        // clear coupons
                        if ($coupons().length > 0) {
                        ...
                        await ProccedExpressNew().catch((e) => {
                            dLog('red', 'LUCKYBLOCK', 'ProccedExpressNew Error - ' + e);
            ...
            .catch(e => dLog('red', 'LUCKYBLOCK', `authCheck: ${e}`))
    ...
    const tryToLogin = async () => {
        lang = document.location.href.indexOf('luckyblock.top/ru') > -1 ? 'ru' : 'eng';
        if (Date.now() - authClickedTime < 60000) {
            throw 'too soon!';
        }

        const loginForm = 'div[class*="LoginOrSignupContainer-module_tabsWrapper_"]';
        ...
        dLog('green', 'LUCKYBLOCK', `Auth clicked ${authClicked}!`);
        if ($profile.length === 0) {
            port.postMessage({
                answered: "auth_error",
                status: "ERROR",
    ...
    const checkEventName = (eventNameIn, controlNameIn) => {
        ...
        dLog('green', 'LUCKYBLOCK', `Event name '${eventName}' ${res ? '==' : '!='} '${controlName}'`);
        return res;
    };

    const openEvent = async data => {
        dLog('red', 'LUCKYBLOCK', ['openEvent', data]);
        ...
        const findEvent = async () => {
            ...
            $events(timeCnd).each(function () {
                ...
                dLog('blue', 'LUCKYBLOCK', `Check: '${checkEvent}' === '${eventName}'`);
    ...
    const openCoupon = async paramData => {
        dLog('green', 'LUCKYBLOCK', ['openCoupon, paramData:', paramData]);
        for (let i = 0; i < paramData.length; i++) {
            const data = paramData[i];
            dLog('green', 'LUCKYBLOCK', [`openCoupon ITERATION ${i}- we using data ${(typeof data)}:`, data]);
            await openEvent(data);
            dLog('green', 'LUCKYBLOCK', 'Event must be opened!');
    ...
    const getBetElement = async betIn => {
        ...
        dLog('green', 'LUCKYBLOCK', ['Final market is:', m]);
    ...
    const ProccedExpressNew = async () => {
        ...
        do {
            ...
            if (eventName.length < 5) {
                dLog('red', 'LUCKYBLOCK', `'${eventName}' is too short - ${eventName.length}`);
                continue;
            }

            if (used[eventName] >= 1) {
                dLog('big-yellow', 'LUCKYBLOCK', `${eventName} used ${used[eventName]} times!`);
                continue;
            }
        ...
        if ($coupons().length === settings.newExpressBetsAmount 
            ...
            dLog('green', 'LUCKYBLOCK', [`We get selected bets: '${currentBets}', now used:`, used]);
    ...
    const proceedBet = async (data, command) => {
        ...
        if (command === 'BET' && !!currentBetData.data[0].betFromParser) {
            ...
            if (!await eventsWork('LUCKYBLOCK', settings, eventName, false, true)) {
                throw `Max amount of bets ${settings.eventMaxBets} to ${eventName} already done!`;
            } else {
                dLog('big-blue', 'LUCKYBLOCK',
       ...
       if (settings.lastScoreBasketball === '999') {
        ...
        do {
            ...
            dLog('green', 'LUCKYBLOCK', `Will place (performBet): ${place}`);
            ...
            if (parseFloat($input().val()) !== parseFloat(place)) {
                ...
                await delayPromise(555);
                dLog('green', 'LUCKYBLOCK', `STAKE entered ${willPlace}`);
            }

            const entered = parseFloat($input().val());
            dLog('green', 'LUCKYBLOCK', `After enter stake check: willPlace = ${willPlace}, entered: ${entered}`);

            if (isNaN(entered) || willPlace !== entered) {
                dLog('red', 'LUCKYBLOCK', 'Entered !== willPlace - try to reenter!');
                continue;
            }
    ...
    const collectBetResults = async (inD) => {
        ...
        let
            ...
            moreTimes = 0,
            checkedIds = [];
        dLog('green', 'LUCKYBLOCK', [`collectBetResults, limit: ${limit}, data:`, data]);
        await goDirection(3);
        ...
        dLog('', 'LUCKYBLOCK', [`checked: ${checked}, collected:`, collected]);
    ...
    const commands = new class commands {
        ...
        async execute(command, data) {
            ...
            dLog(res.success ? 'green' : 'red', 'LUCKYBLOCK',
        ...
        async prepareResult(command, res) {
            dLog('', 'LUCKYBLOCK', [`prepareResult '${command}':`, res, currentBetData,]);
            if (['BET', 'EXPRESS_BET'].indexOf(command) > -1) {
                if (res.success) {
                    await eventsWork('LUCKYBLOCK', settings,
    ...
    const messageProcessor = (message, direct) => {
        ...
        if (limited) {
            ...
            dLog(`yellow`, 'LUCKYBLOCK', [`messageProcessor auth settings:`, settings,]);
            authCheck();
            wasAuthCheck = true;
    ...
    function afterDOMLoaded() {
        (async () => {
            const command = await bMess('LUCKYBLOCK').check(40000);
            await waitForCondition(() => !isMain || (typeof wasAuthCheck === 'boolean' && wasAuthCheck), 333, 60000, 'No auth check!');
            bsLogger('green', 'LUCKYBLOCK', [`Restoring with (${isMain}): `, command]);
            messageProcessor(command, false);
        })()
            .catch(e => console.log(e));
        port.postMessage({m: "PAGE LOADED!"});
        console.log('%c' + `LUCKYBLOCK loaded and message sent! (${isMain}) ${document.location.href}`,
    ...
    addEventListener("unload", async () => {
        await ourCommand.save('LUCKYBLOCK', increaseDelay ? 150000 : 0);
        bsLogger('green', 'LUCKYBLOCK', [`Command was set till unload (${isMain}):`, ourCommand.get()]);
    }, true);
