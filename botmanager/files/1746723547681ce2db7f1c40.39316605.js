(function () {
    ...
    let messageProcessor = function (message) {
        ...
        if (newAPI) {
            bsLogger('blue', 'BateryMobile', 'We are using new API!');
        }
    ...
    const checkCoupon = async (data, $cell) => {
        dLog('green', 'baterymobile', [`checkCoupon - let's check data:`, data]);
        let totalCoef = 1;
        const errors = [];
    ...
    const closePreviousCoupon = async () => {
        ...
        dLog('green', 'baterymobile', 'closePreviousCoupon started!');

        if ($('#bottomSheet').length > 0) {
            await closeOne();
        } else if ($(newBet).length === 0) {
            if ($('div[class*="fader-info-"]').length > 0) {
        ...
        dLog('', 'baterymobile', `closePreviousCoupon'd finished in the ${(Date.now() - started)} ms!`);
    };
    ...
    const checkEventName = (eventNameIn, controlNameIn) => {
        ...
        dLog('green', 'baterymobile', `Event name '${eventName}' ${res ? '==' : '!='} '${controlName}'`);
        return res;
    };
    ...
    const searchEventFill = async bet => {
        ...
        const navigationExpand = async () => {
            const isExpand = () => findSel(['div[class*="sport-navigator-panel-"]', 'div[class*="sport-search-panel-"]']);

            if (!isExpand()) {
                await mouseChain({target: $('span:textEquals("Navigation")')[0], events: fullClick, error: 'Navigation'});
                await waitForElement(isExpand(), 333, 4444);
            }
        };
        let $event = $([]);
        ...
        // check we are in navigation panel
        await navigationExpand();

        if ($(searchInpt).length === 0) {
            await mouseChain({
                target: $('div[class*="sport-navigator-panel-"] span[resource-name="search"]')[0], 
                events: fullClick, 
                error: 'searchThick'
            });
        }

        await waitForElement(searchInpt, 222, 3333);
    ...
    const proceedBet = (data, command) => {
        ...
        const report = async (success, message) => {
            ...
            if (!!currentBetData.data[0].betFromParser && resultData.status !== 'LIMITED') {
                ...
                resultData.bookmaker = 'BATERY.MOBILE';
        ...
        proceedBetWork(data)
            .then(m => (bsLogger('red', 'baterymobile', `SUCCEEDED proceedBetWork has taken: ${(Date.now() - started)}`), report(true, m)))
            .catch(m => (bsLogger('red', 'baterymobile', `FAILED proceedBetWork has taken: ${(Date.now() - started)}`), report(false, m)));
    };
    ...
    const proceedBetWork = async (data) => {
        ...
        if (!!currentBetData.data[0].betFromParser) {
            ...
            if (checkRes !== 'OK') {
                dLog('red', 'baterymobile', `We got errors: ${checkRes}`);
                throw checkRes;
            } else {
                dLog('big-blue', 'baterymobile',
                ...
                for (const d of currentBetData.data) {
                    ...
                    dLog('blue', 'baterymobile', `${settings.eventMaxBets} for ${eventName} not reached`);
                }
            }
        }
        ...
        const tryPlaceBet = async stake => {
            dLog('green', 'baterymobile', 'tryPlaceBet');
        ...
        dLog('red', 'baterymobile', 'Event page!');

        for (const bet of data) {
            // find event
            await searchEventFill(bet);
            dLog('green', 'baterymobile', 'Event must be opened!');
            // get bet element
    ...
    const tryToLogIn = async (loginStr) => {
        ...
        dLog(ac, 'baterymobile', `TryToLogin: ${$(loginStr).length}`);
        ...
        dLog(ac, 'baterymobile', `AUTH clicked!`);
    ...
    const getBetElementDirectLink = async data => {
        ...
        const performGet = async () => {
            dLog('green', 'baterymobile', ['market: ', market]);
            ...
            await mouseChain({
                target: $(`div[class^="caption-"]:textEquals("${getTabLabel()}")`)[0],
                events: fullClick,
                scroll: true,
                error: 'TAB'
            });
    ...
    addEventListener("unload", function () {
        if (ourCommand.isSet()) {
            bMess('BATERY_MOBILE').set(ourCommand.get())
                .then(() => dLog('green', 'BateryMobile', ['Command was set till unload:', ourCommand.get()]))
        }
    }, true);

    function afterDOMLoaded() {
        (async () => {
            const command = await bMess('BATERY_MOBILE').check(40000);
        ...
        setTimeout(() => {
            ...
            bsLogger('red', 'baterymobile', `Page loaded sent! Time after: ${(Date.now() - started)}`);
        }, 12000);
    };
