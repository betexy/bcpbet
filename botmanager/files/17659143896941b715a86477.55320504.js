(() => {
    ...
    const profile = 'button:textEquals("Wallet"):visible';

    const port = isMain ? chrome.runtime.connect({name: 'port_realbet'})
        : {postMessage: (...args) => console.log(args)};
    const settings = {
        restartEvery: 1800000,
        authCheckInterval: 2000,
        url: 'https://realbet.io/sports',
    ...
    const
        btSel = '#sports-game-content > div',
        shadowRoot = () => $(btSel) && $(btSel).length > 0
            ? $(btSel)[0].shadowRoot
            : null,
        loginForm = 'div[aria-labelledby="modal"] span:textEquals("Sign in")';
    ...
    function getBalance(returnNull) {
        const $b = $(profile).prev().find('span:first');
        return $b.length === 0
    ...
    const findBalance = async () => {
        // open curency window
        if ($(profile).next().find('ul:visible').length === 0) {
            await mouseChain({target: $(profile).prev()[0], events: fullClick, error: 'Open curency window'});
            await delayPromise(777);
            await waitForCondition(() => $(profile).next().find('ul:visible').length > 0,
                222, 3333, 'No currency modal');
        }

        const currencyEl = $('button:textEquals("Wallet"):visible').next().find('ul:visible li').toArray().find(
            (el) => parseFloat($(el).find('span:first').trt().replace(/[^\d.]/g, '')) > 0
        )

        if ($(currencyEl).length > 0) {
            await mouseChain({target: $(currencyEl)[0], events: fullClick, error: 'Select curency'});
        }
    };
    ...
    const authCheck = () => {
        ...
        (async () => {
            ...
            if (!limited && $profile.length === 0) {
                ...
                if (settings.lastScoreBasketball === '999') {
                    if (Date.now() - settings.sourceDate >= 300000) {
                        ...
                        dLog('blue', 'REALBET', `Source current random value - ${settings.sourceRandom}`);
                    }
                }
                ...
                if (settings.newExpresses && !busy) {
                    ...
                    if (!WasSuccessExpressNew) {
                        dLog('green', 'REALBET', 'START find NewExpress event!');
                        // clear coupons
                        if ($coupons().length > 0) {
                        ...
                        await ProccedExpressNew().catch((e) => {
                            dLog('red', 'REALBET', 'ProccedExpressNew Error - ' + e);
            ...
            .catch(e => dLog('red', 'REALBET', `authCheck: ${e}`))
    ...
    const tryToLogin = async () => {
        ...
        if ($(loginForm).length === 0) {
            await mouseChain({
                target: $('div.navbar-height button:textEquals("Sign in")')[0], 
                events: fullClick, 
                error: 'Log in'
            });
        ...
        await clearAndSimulate($('input[name="email"]:visible')[0],
            settings.login);
        await delayPromise(777);
        await clearAndSimulate($('input[name="password"]:visible')[0],
            settings.password);
        await delayPromise(777);
        await mouseChain({
            target: $('button[type="submit"]:textEquals("Sign in"):visible')[0],
            events: fullClick,
            error: 'submit form'
        });
        ...
        dLog('green', 'REALBET', `Auth clicked ${authClicked}!`);
        if ($profile.length === 0) {
            port.postMessage({
                answered: "auth_error",
                status: "ERROR",
    ...
    const checkEventName = (eventNameIn, controlNameIn) => {
        ...
        dLog('green', 'REALBET', `Event name '${eventName}' ${res ? '==' : '!='} '${controlName}'`);
        return res;
    };

    const openEvent = async data => {
        dLog('red', 'REALBET', ['openEvent', data]);
        ...
        const findEvent = async () => {
            ...
            $events(timeCnd).each(function () {
                ...
                dLog('blue', 'REALBET', `Check: '${checkEvent}' === '${eventName}'`);
    ...
    const openCoupon = async paramData => {
        dLog('green', 'REALBET', ['openCoupon, paramData:', paramData]);
        for (let i = 0; i < paramData.length; i++) {
            const data = paramData[i];
            dLog('green', 'REALBET', [`openCoupon ITERATION ${i}- we using data ${(typeof data)}:`, data]);
            await openEvent(data);
            dLog('green', 'REALBET', 'Event must be opened!');
    ...
    const getBetElement = async betIn => {
        ...
        dLog('green', 'REALBET', ['Final market is:', m]);
    ...
    const ProccedExpressNew = async () => {
        ...
        if (document.location.href.indexOf('event-builder') === -1) {
            ...
            if (builderBtnExist.length === 0) {
                window.location.href = 'https://realbet.io/sports';
        ...
        do {
            ...
            if (eventName.length < 5) {
                dLog('red', 'REALBET', `'${eventName}' is too short - ${eventName.length}`);
                continue;
            }

            if (used[eventName] >= 1) {
                dLog('big-yellow', 'REALBET', `${eventName} used ${used[eventName]} times!`);
                continue;
            }
        ...
        if ($coupons().length === settings.newExpressBetsAmount 
            ...
            dLog('green', 'REALBET', [`We get selected bets: '${currentBets}', now used:`, used]);
    ...
    const proceedBet = async (data, command) => {
        ...
        if (command === 'BET' && !!currentBetData.data[0].betFromParser) {
            ...
            if (!await eventsWork('REALBET', settings, eventName, false, true)) {
                throw `Max amount of bets ${settings.eventMaxBets} to ${eventName} already done!`;
            } else {
                dLog('big-blue', 'REALBET',
        ...
        do {
            ...
            dLog('green', 'REALBET', `Will place (performBet): ${place}`);
            ...
            if (parseFloat($input().val()) !== parseFloat(place)) {
                ...
                await delayPromise(555);
                dLog('green', 'REALBET', `STAKE entered ${willPlace}`);
            }

            const entered = parseFloat($input().val());
            dLog('green', 'REALBET', `After enter stake check: willPlace = ${willPlace}, entered: ${entered}`);
            if (isNaN(entered) || willPlace !== entered) {
                dLog('red', 'REALBET', 'Entered !== willPlace - try to reenter!');
                continue;
            }
    ...
    const collectBetResults = async (inD) => {
        ...
        let
            ...
            moreTimes = 0,
            checkedIds = [];
        dLog('green', 'REALBET', [`collectBetResults, limit: ${limit}, data:`, data]);
        await goDirection(3);
        ...
        dLog('', 'REALBET', [`checked: ${checked}, collected:`, collected]);
    ...
    const commands = new class commands {
        ...
        async execute(command, data) {
            ...
            dLog(res.success ? 'green' : 'red', 'REALBET',
        ...
        async prepareResult(command, res) {
            dLog('', 'REALBET', [`prepareResult '${command}':`, res, currentBetData,]);
            if (['BET', 'EXPRESS_BET'].indexOf(command) > -1) {
                if (res.success) {
                    await eventsWork('REALBET', settings,
    ...
    const messageProcessor = (message, direct) => {
        ...
        if (limited) {
            ...
            dLog(`yellow`, 'REALBET', [`messageProcessor auth settings:`, settings,]);
            authCheck();
            wasAuthCheck = true;
    ...
    function afterDOMLoaded() {
        (async () => {
            const command = await bMess('REALBET').check(40000);
            await waitForCondition(() => !isMain || (typeof wasAuthCheck === 'boolean' && wasAuthCheck), 333, 60000, 'No auth check!');
            bsLogger('green', 'REALBET', [`Restoring with (${isMain}): `, command]);
            messageProcessor(command, false);
        })()
            .catch(e => console.log(e));
        port.postMessage({m: "PAGE LOADED!"});
        console.log('%c' + `REALBET loaded and message sent! (${isMain}) ${document.location.href}`,
    ...
    addEventListener("unload", async () => {
        await ourCommand.save('REALBET', increaseDelay ? 150000 : 0);
        bsLogger('green', 'REALBET', [`Command was set till unload (${isMain}):`, ourCommand.get()]);
    }, true);
