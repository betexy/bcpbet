(() => {
    ...
    const port = isMain ? chrome.runtime.connect({name: 'port_jetton'})
        : {postMessage: (...args) => console.log(args)};
    const settings = {
        restartEvery: 1800000,
        authCheckInterval: 2000,
        url: 'https://jetvegas.xyz/en/sportsbook',
    ...
    const
        btSel = 'div.min-h-dvh > div',
        shadowRoot = () => $(btSel) && $(btSel).length > 0
            ? $(btSel)[0].shadowRoot
            : null;
    let usdtLink = 'button:textEquals("Пополнить"):visible';
    let loginLink = 'button:textEquals("Вход"):visible';
...
function getBalance(returnNull) {
    const $b = $('button[data-mixpanel*="View balance"] p.text-sm');
    if ($b.length === 0) {
        return (typeof returnNull === 'boolean' && returnNull ? null : 0);
    }
    return parseFloat($b.text().replace(/[^\d.]/g, ''));
}
...
//    const getLang = async () => {
//        await waitForCondition(() => $(findSel([
//            'div[data-sidebar="sidebar"] span:textEquals("Русский")',
//            'div[data-sidebar="sidebar"] span:textEquals("English")',
//        ])).length > 0, 222, 5555, 'Language not found!');
//        lang = $('div[data-sidebar="sidebar"] span:textEquals("Русский")').length > 0 ? 'ru' : 'eng';
//
//        if (lang === 'eng') {
//            Registration = 'Registration';
//            Login = 'Login';
//
//            usdtLink = 'button:textEquals("Top up"):visible';
//            loginLink = 'button:textEquals("Login"):visible';
//        } else {
//            usdtLink = 'button:textEquals("Пополнить"):visible';
//            loginLink = 'button:textEquals("Вход"):visible';
//        }
//    };
    ...
    const authCheck = () => {
        ...
        (async () => {
            ...
            if ($('pre').length > 0) {
                window.location.href = 'https://jetvegas.xyz/sportsbook';
            }
  ...
  //          await getLang();
            ...
            if (!limited && $loginLink.length > 0) {
                ...
                if (settings.lastScoreBasketball === '999') {
                    if (Date.now() - settings.sourceDate >= 300000) {
                        ...
                        dLog('blue', 'JETTON', `Source current random value - ${settings.sourceRandom}`);
                    }
                }
                ...
                if (settings.newExpresses && !busy) {
                    ...
                    if (!WasSuccessExpressNew) {
                        dLog('green', 'JETTON', 'START find NewExpress event!');
                        // clear coupons
                        if ($coupons().length > 0) {
                        ...
                        await ProccedExpressNew().catch((e) => {
                            dLog('red', 'JETTON', 'ProccedExpressNew Error - ' + e);
            ...
            .catch(e => dLog('red', 'JETTON', `authCheck: ${e}`))
    ...
    const tryToLogin = async () => {
        const authForm = () => $(`div[role="dialog"]:visible`);

        if (Date.now() - authClickedTime < 60000) {
            throw 'too soon!';
        }

        if (authForm().find(`p:textEquals(${Login})`).length === 0) {
            await mouseChain({target: $(loginLink)[0], events: fullClick, error: 'Login'});
            await delayPromise(333);
        }

        await waitForCondition(() => authForm().find(`p:textEquals(${Login})`).length > 0,
            333, 10000, 'No login form');
        await clearAndSimulate(authForm().find('input[name="email"]')[0],
            settings.login);
        await delayPromise(777);
        await clearAndSimulate(authForm().find('input[type="password"]')[0],
            settings.password);
        await delayPromise(777);
        await mouseChain({
            target: authForm().find('button[type="submit"]')[0],
            events: fullClick,
            error: 'submit form'
        });
        await delayPromise(333);
        ...
        dLog('green', 'JETTON', `Auth clicked ${authClicked}!`);
        const $profile = await waitForElement(usdtLink,
            333, 37000).catch(() => $([]));
    ...
    const checkEventName = (eventNameIn, controlNameIn) => {
        ...
        dLog('green', 'JETTON', `Event name '${eventName}' ${res ? '==' : '!='} '${controlName}'`);
        return res;
    };

    const openEvent = async data => {
        dLog('red', 'JETTON', ['openEvent', data]);
        ...
        const findEvent = async () => {
            ...
            $events(timeCnd).each(function () {
                ...
                dLog('blue', 'JETTON', `Check: '${checkEvent}' === '${eventName}'`);
    ...
    const openCoupon = async paramData => {
        dLog('green', 'JETTON', ['openCoupon, paramData:', paramData]);
        for (let i = 0; i < paramData.length; i++) {
            const data = paramData[i];
            dLog('green', 'JETTON', [`openCoupon ITERATION ${i}- we using data ${(typeof data)}:`, data]);
            await openEvent(data);
            dLog('green', 'JETTON', 'Event must be opened!');
    ...
    const getBetElement = async betIn => {
        ...
        const markets = {
            'ONE_TWO': {
                ...
                'ONE_DRAW': {
                    tabs: ['Main', 'Основные'],
                    roots: ['Double chance', 'Двойной шанс'],
        ...
        dLog('green', 'JETTON', ['Final market is:', m]);
    ...
    const ProccedExpressNew = async () => {
        ...
        if (document.location.href.indexOf('event-builder') === -1) {
            ...
            if (builderBtnExist.length === 0) {
                window.location.href = 'https://jetvegas.xyz/en/sportsbook';
        ...
        do {
            ...
            if (eventName.length < 5) {
                dLog('red', 'JETTON', `'${eventName}' is too short - ${eventName.length}`);
                continue;
            }

            if (used[eventName] >= 1) {
                dLog('big-yellow', 'JETTON', `${eventName} used ${used[eventName]} times!`);
                continue;
            }
        ...
        if ($coupons().length === settings.newExpressBetsAmount 
            ...
            dLog('green', 'JETTON', [`We get selected bets: '${currentBets}', now used:`, used]);
    ...
    const proceedBet = async (data, command) => {
        dLog('green', 'JETTON', ['proceedBet:', data, command]);
        if (data.length === 0) {
            throw 'No data to bet!';
        }
        const
        ...
        if (command === 'BET' && !!currentBetData.data[0].betFromParser) {
            ...
            if (!await eventsWork('JETTON', settings, eventName, false, true)) {
                throw `Max amount of bets ${settings.eventMaxBets} to ${eventName} already done!`;
            } else {
                dLog('big-blue', 'JETTON',
        ...
        do {
            ...
            dLog('green', 'JETTON', `Will place (performBet): ${place}`);
            ...
            if (parseFloat($input().val()) !== parseFloat(place)) {
                ...
                await delayPromise(555);
                dLog('green', 'JETTON', `STAKE entered ${willPlace}`);
            }

            const entered = parseFloat($input().val());
            dLog('green', 'JETTON', `After enter stake check: willPlace = ${willPlace}, entered: ${entered}`);

            if (isNaN(entered) || willPlace !== entered) {
                dLog('red', 'JETTON', 'Entered !== willPlace - try to reenter!');
                continue;
            }
    ...
    const collectBetResults = async (inD) => {
        ...
        let
            ...
            moreTimes = 0,
            checkedIds = [];
        dLog('green', 'JETTON', [`collectBetResults, limit: ${limit}, data:`, data]);
        await goDirection(3);
        ...
        dLog('', 'JETTON', [`checked: ${checked}, collected:`, collected]);
    ...
    const commands = new class commands {
        ...
        async execute(command, data) {
            ...
            dLog(res.success ? 'green' : 'red', 'JETTON',
        ...
        async prepareResult(command, res) {
            dLog('', 'JETTON', [`prepareResult '${command}':`, res, currentBetData,]);
            if (['BET', 'EXPRESS_BET'].indexOf(command) > -1) {
                if (res.success) {
                    await eventsWork('JETTON', settings,
    ...
    const messageProcessor = (message, direct) => {
        ...
        if (limited) {
            ...
            dLog(`yellow`, 'JETTON', [`messageProcessor auth settings:`, settings,]);
            authCheck();
            wasAuthCheck = true;
    ...
    function afterDOMLoaded() {
        (async () => {
            const command = await bMess('JETTON').check(40000);
            await waitForCondition(() => !isMain || (typeof wasAuthCheck === 'boolean' && wasAuthCheck), 333, 60000, 'No auth check!');
            bsLogger('green', 'JETTON', [`Restoring with (${isMain}): `, command]);
            messageProcessor(command, false);
        })()
            .catch(e => console.log(e));
        port.postMessage({m: "PAGE LOADED!"});
        console.log('%c' + `JETTON loaded and message sent! (${isMain}) ${document.location.href}`,
    ...
    addEventListener("unload", async () => {
        await ourCommand.save('JETTON', increaseDelay ? 150000 : 0);
        bsLogger('green', 'JETTON', [`Command was set till unload (${isMain}):`, ourCommand.get()]);
    }, true);
