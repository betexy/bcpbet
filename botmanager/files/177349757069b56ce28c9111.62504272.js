(() => {
    ...
    const port = isMain ? chrome.runtime.connect({name: 'port_gamdom'})
    ...
    // Shadow root: find div whose shadowRoot contains #bt-root
    let _cachedShadowHost = null;
    const
        shadowRoot = () => {
            if (_cachedShadowHost && _cachedShadowHost.shadowRoot) return _cachedShadowHost.shadowRoot;
            const divs = document.querySelectorAll('div');
            for (const div of divs) {
                if (div.shadowRoot && div.shadowRoot.querySelector('#bt-root')) {
                    _cachedShadowHost = div;
                    return div.shadowRoot;
                }
            }
            return null;
        },
        loginSelector = 'button:textEquals("Sign in")';
    ...
    const goToSport = async () => {
        if (document.location.href.indexOf('/sports') === -1) {
            const $sb = $('a[href="/sports"], button:textEquals("Sports Betting")').filter(':visible').first();
            if ($sb.length > 0) {
                await mouseChain({target: $sb[0], events: fullClick, error: 'goToSport'});
                await delayPromise(1000);
            }
        }
    };
    ...
    const authCheck = () => {
        ...
        (async () => {
            ...
            // Close chat, notifications, popups
            ...
            const $profile = $('a[href="/profile"]');
            if ($(loginSelector).length > 0 && Date.now() - pageLoadedAt < 60000) {
                dLog('', 'gamdom', `Still loading: ${Date.now() - pageLoadedAt}`);
                ...
                if (settings.lastScoreBasketball === '999') {
                    if (Date.now() - settings.sourceDate >= 300000) {
                        ...
                        dLog('blue', 'GAMDOM', `Source current random value - ${settings.sourceRandom}`);
                    }
                }

                if (settings.newExpresses && !busy) {
                    //dLog('green', 'GAMDOM', 'START ProccedExpressNew!');
                    busy = true;
                    ...
                    if (!WasSuccessExpressNew) {
                        dLog('green', 'GAMDOM', 'START find NewExpress event!');
                        // clear coupons
                        if ($coupons().length > 0) {
                        ...
                        await ProccedExpressNew().catch((e) => {
                            dLog('red', 'GAMDOM', 'ProccedExpressNew Error - ' + e);
    ...
    const closeSourceExpressCoupon = async () => {
        dLog('green', 'GAMDOM', 'closeSourceExpressCoupon started!');
    ...
    const openEvent = async data => {
        ...
        const findEvent = async () => {
            ...
            $events(timeCnd).each(function () {
                ...
                dLog('blue', 'gamdom', `Check: '${checkEvent}' === '${eventName}'`);
    ...
    const ProccedExpressNew = async () => {
        ...
        do {
            ...
            if (eventName.length < 5) {
                dLog('red', 'GAMDOM', `'${eventName}' is too short - ${eventName.length}`);
                continue;
            }

            if (used[eventName] >= 1) {
                dLog('big-yellow', 'GAMDOM', `${eventName} used ${used[eventName]} times!`);
                continue;
            }
        ...
        if ($coupons().length === settings.newExpressBetsAmount
            ...
            dLog('green', 'GAMDOM', [`We get selected bets: '${currentBets}', now used:`, used]);
    ...
    const proceedBet = async (data, command) => {
        ...
        const checkSuccess = async () => {
            const successText = "Your bets have been successfully placed!";
        ...
        do {
            ...
            dLog('green', 'GAMDOM', `Will place (performBet): ${place}`);
            ...
            if ($cutError.length > 0) {
                ...
                if (/max(imum)?\s*stake/i.test(errorText)) {
                    ...
                    dLog('red', 'GAMDOM', `[CUT] Max stake detected: ${matchNum ? matchNum[0] : '?'}`);
                    port.postMessage({
                        m: 'FARM_CUT_DETECTED',
                        cutInfo: {
                            ...
                            bk: 'GAMDOM', timestamp: Date.now()
                        }
                    });
                } else if (/blocked/i.test(errorText)) {
                    dLog('red', 'GAMDOM', `[BLOCKED] Account blocked: ${errorText}`);
                    port.postMessage({
                        m: 'FARM_CUT_DETECTED',
                        cutInfo: {
        ...
        await waitForCondition(() => $(shadowRoot()).find('div[data-editor-id="pillButton"]:eq(1)').length > 0, 333, 30000, 'no open bets button');
        await delayPromise(333);
        await mouseChain({
        ...
        await delayPromise(333);
        await waitForCondition(() => $(shadowRoot()).find('div[data-editor-id="bet"]:first').length > 0, 333, 30000, 'no open bets');
    ...
    const collectBetResults = async (inD) => {
        ...
        let
            ...
            moreTimes = 0,
            checkedIds = [];
        dLog('green', 'GAMDOM', [`collectBetResults, limit: ${limit}, data:`, data]);
        await goDirection(3);
    ...
    const commands = new class commands {
        ...
        async execute(command, data) {
            ...
            dLog(res.success ? 'green' : 'red', 'GAMDOM',
        ...
        async prepareResult(command, res) {
            dLog('', 'GAMDOM', [`prepareResult '${command}':`, res, currentBetData,]);
    ...
    const messageProcessor = (message, direct) => {
        ...
        if (limited) {
            ...
            // Queue FARM_CHECK_BETS to execute after current command finishes
            if (message.action === 'FARM_CHECK_BETS') {
                dLog('yellow', 'GAMDOM', `${message.action} queued - will execute after current command`);
            ...
            dLog('yellow', 'gamdom', ['messageProcessor auth settings:', settings]);
            authCheck();
            wasAuthCheck = true;
                ...
                commands.execute(message.action, message.data)
                    .finally(() => {
                        ...
                        if (pendingBetResult) {
                            ...
                            dLog('green', 'GAMDOM', `Executing queued ${queuedMessage.action}`);
    ...
    async function performWithdraw(data) {
        ...
        dLog('blue', 'GAMDOM', `[WITHDRAW] Starting withdrawal to ${withdrawAddress}`);
        try {
            const $walletBtn = $('a[href="/wallet"]').filter(':visible').first();
            ...
            const $withdrawTab = $('button:textEquals("Withdraw"), a:textEquals("Withdraw")').filter(':visible');
            ...
            const $addressInput = $('input[placeholder*="address" i], input[name*="address" i]').filter(':visible').first();
            if ($addressInput.length === 0) throw 'Address input not found';
            await clearAndSimulate($addressInput[0], withdrawAddress);
            await delayPromise(500);

            const $maxBtn = $('button:textEquals("Max"), button:textEquals("MAX")').filter(':visible').first();
            if ($maxBtn.length > 0) {
            ...
            dLog('green', 'GAMDOM', '[WITHDRAW] Completed');
            return {success: true, message: 'Withdrawal submitted'};
        } catch (e) {
            dLog('red', 'GAMDOM', `[WITHDRAW] Error: ${e}`);
            throw e;
        }
    }
    ...
    async function checkPendingBets() {
        ...
        try {
            ...
            for (let i = 0; i < 30; i++) {
                ...
                if (text.includes('No bets')) { loaded = true; loadedBy = 'No bets text'; break; }
            }
            if (!loaded) {
            ...
            for (let scroll = 0; scroll < 10; scroll++) {
                ...
                $openStatuses.each(function() {
                    if ($(this).text().toLowerCase().trim() === 'open') count++;
                });
                if (count > totalOpen) totalOpen = count;
                if (count === lastCount && scroll > 0) break;
                lastCount = count;
    ...
    function afterDOMLoaded() {
        (async () => {
            const command = await bMess('GAMDOM').check(40000);
            await waitForCondition(() => !isMain || (typeof wasAuthCheck === 'boolean' && wasAuthCheck), 333, 60000, 'No auth check!');
            bsLogger('green', 'gamdom', [`Restoring with (${isMain}): `, command]);
            messageProcessor(command, false);
        })()
            .catch(e => console.log(e));
        port.postMessage({m: "PAGE LOADED!"});
        pageLoadedAt = Date.now();
        console.log('%c' + `GAMDOM loaded and message sent! (${isMain}) ${document.location.href}`,
    ...
    addEventListener("unload", async () => {
        await ourCommand.save('GAMDOM', increaseDelay ? 150000 : 0);
    }, true);
