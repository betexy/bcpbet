(() => {
    ...
    function getBalance(returnNull) {
        const $b = $('div.currency__data span.balance');
        return $b.length === 0
    ...
    const selectCurrency = async () => {
        const img = 'img[src="https://coinsgame1.com/assets/coins/USDT.svg"]';
        const amount = 'div.balance__inner > div';
        const currencyList = 'div.balance-dropdown:visible';
        const imgList = 'div.balance-dropdown:visible img[src="/assets/coins/USDT.svg"]';
        const $coin = await waitForElement(img,
            333, 4444).catch(() => $([]));
        if ($coin.length === 0) {
            await mouseChain({target: $(amount)[0], events: fullClick, error: '$coin'});
            await waitForElement(currencyList, 333, 3555);
            await mouseChain({
                target: $(imgList)[0],
                events: fullClick,
                error: '$coin list'
    ...
    const authCheck = () => {
        ...
        (async () => {
            ...
            await closeAllWeNeed({
                'button.modal__btn-close': 'button.modal__btn-close',
                'button.push-notification__btn-close': 'button.push-notification__btn-close'
            });

            const $profile = await waitForElement(profile, 300, 3333).catch(() => $([]));
            if (!limited && $profile.length === 0) {
                ...
                if (settings.lastScoreBasketball === '999') {
                    if (Date.now() - settings.sourceDate >= 300000) {
                        ...
                        dLog('blue', bkName, `Source current random value - ${settings.sourceRandom}`);
                    }
                }
                ...
                if (settings.newExpresses && !busy) {
                    ...
                    if (!WasSuccessExpressNew) {
                        dLog('green', bkName, 'START find NewExpress event!');
                        // clear coupons
                        if ($coupons().length > 0) {
                        ...
                        await ProccedExpressNew().catch((e) => {
                            dLog('red', bkName, 'ProccedExpressNew Error - ' + e);
    ...
    const tryToLogin = async () => {
        ...
        lang = document.location.href.indexOf('/ru') > -1 ? 'ru' : 'eng';
        const logLabel = lang === 'eng' ? 'Log in' : 'Войти';
        const loginLink = `button:textEquals("${logLabel}"):visible`;
        const loginForm = 'div.login-signup-modal';
        ...
        await waitForCondition(() => $(loginForm).length > 0,
            333, 7777, 'No login form');
        await clearAndSimulate($(loginForm).find('input.input__tag:eq(0)')[0],
            settings.login);
        await delayPromise(888);
        await clearAndSimulate($(loginForm).find('input.input__tag:eq(1)')[0],
            settings.password);
        await delayPromise(888);
        await mouseChain({
            target: $(loginForm).find('button.login-form__submit-btn')[0],
            events: fullClick,
            error: 'submit form'
        });
    ...
    const openEvent = async data => {
        dLog('red', bkName, ['openEvent', data]);
        ...
        const findEvent = async () => {
            ...
            $events(timeCnd).each(function () {
                ...
                dLog('blue', bkName, `Check: '${checkEvent}' === '${eventName}'`);
    ...
    const openCoupon = async paramData => {
        dLog('green', bkName, ['openCoupon, paramData:', paramData]);
        for (let i = 0; i < paramData.length; i++) {
            const data = paramData[i];
            dLog('green', bkName, [`openCoupon ITERATION ${i}- we using data ${(typeof data)}:`, data]);
            await openEvent(data);
            dLog('green', bkName, 'Event must be opened!');
    ...
    const getBetElement = async betIn => {
        ...
        params.proceed_football = function (bet) {
            if (bet.market === 'HDP' && parseFloat(bet.pivot) === 0) {
                const draw = lang === 'eng' ? 'Draw no bet' : 'Ничья ставки нет';
                const draw1 = lang === 'eng' ? '1st half - draw no bet' : '1-й тайм - ничья ставки нет';
                this.addTo('roots', this.full ? draw : draw1);
                this.addTo('pivotKeys', bet.target === 'HOME' ? '#TEAM1#' : '#TEAM2#');
            }
            if (this.full) {
                return;
            }
            this.addTo('tabs', 'Halves', 'Половины');
            switch (bet.market) {
                case 'ONE_TWO':
                    this.addReplIn('roots', '1x2', '1st half - 1x2', '1x2', '1-й тайм - 1x2');
                    break;
                case 'HDP':
                    this.addReplIn('roots', 'Handicap', '1st half - handicap', 'Фора', '1-й тайм - Фора');
                    this.addReplIn('roots', 'Handicap (Asian)', '1st half - handicap (Asian)', 'Фора (Азиатская)', '1-й тайм - фора (Азиатская)');
                    break;
                case 'TOTAL':
                    this.addReplIn('roots', 'Total', '1st half - total', 'Тотал', '1-й тайм - тотал');
                    this.addReplIn('roots', 'Total (Asian)', '1st half - total (Asian)', 'Тотал (Азиатский)', '1-й тайм - тотал (Азиатский)');
                    break;
                case 'T1_TOTAL':
                    this.addReplIn('roots', '#TEAM1# total', '1st half - #TEAM1# total', '#TEAM1# тотал', '1-й тайм - #TEAM1# тотал');
                    break;
                case 'T2_TOTAL':
                    this.addReplIn('roots', '#TEAM2# total', '1st half - #TEAM2# total', '#TEAM2# тотал', '1-й тайм - #TEAM2# тотал');
                    break;
            }
        };
        params.proceed_tennis = function (bet) {
            const 
                firstSet = lang === 'eng' ? 'First set' : 'Первый сет',
                secondSet = lang === 'eng' ? 'Second set' : 'Второй сет',
                thirdSet = lang === 'eng' ? 'Third set' : 'Третий сет',
                fourthSet = lang === 'eng' ? 'Fourth set' : 'Четвертый сет';
            const 
                winner = lang === 'eng' ? 'Winner' : 'Победитель',
                fora = lang === 'eng' ? 'Game handicap' : 'Фора по геймам',
                total = lang === 'eng' ? 'Total games' : 'Тотал геймов';
            const
                sets = [firstSet, secondSet, thirdSet, fourthSet],
                repls = {
                    'ONE_TWO': winner,
                    'HDP': fora,
                    'TOTAL': total,
            ...
            if (!this.full) {
                ...
                if (bet.time_value.indexOf('SET') > -1 && bet.time_value.indexOf('GAME') > -1) {
                    ...
                    if (Object.keys(repls).indexOf(bet.market) > -1) {
                        this.addTotal('roots', [`${s} гейм ${parts[1]} - ${repls[bet.market].toLowerCase()}`]);
                    }
                } else {
                    if (Object.keys(repls).indexOf(bet.market) > -1) {
                ...
                if (Object.keys(repls).indexOf(bet.market) > -1) {
                    this.addTotal('roots', [repls[bet.market]]);
                }
                if (bet.market === 'T1_TOTAL') {
                    this.addReplIn('roots', '#TEAM1# total', '#TEAM1# total games', '#TEAM1# тотал', '#TEAM1# тотал геймов');
                } else if (bet.market === 'T2_TOTAL') {
                    this.addReplIn('roots', '#TEAM2# total', '#TEAM2# total games', '#TEAM2# тотал', '#TEAM2# тотал геймов');
                }
            }
        };
        params.proceed_basketball = function (bet) {
            const
                quarters = ['First quarter - ', 'Second quarter - ', 'Third quarter - ', 'Fourth quarter - ', 'Первая четверть - ', 'Вторая четверть - ', 'Третья четверть - ', 'Четвертая четверть - '],
                quarter = quarters[this.tDigit - 1];
            this.addTotal('tabs', ['ALL', 'Все']);
            if (!this.full) {
                if (bet.market === 'ONE_TWO') {
                    this.addReplIn('roots', '1x2', quarter + '1x2', '1x2', quarter + '1x2');
                } else if (bet.market === 'HDP') {
                    this.addReplIn('roots', 'Handicap', quarter + 'handicap', 'Фора', quarter + 'фора');
                } else if (bet.market === 'TOTAL') {
                    this.addReplIn('roots', 'Total', quarter + 'total', 'Тотал', quarter + 'тотал');
                }
            } else {
                if (bet.market === 'HDP') {
                    this.addReplIn('roots', 'Handicap', 'Handicap (incl. overtime)', 'Фора', 'Фора (вкл. овертайм)');
                } else if (bet.market === 'TOTAL') {
                    this.addReplIn('roots', 'Total', 'Total (incl. overtime)', 'Тотал', 'Тотал (вкл. овертайм)');
                } else if (bet.market === 'T1_TOTAL') {
                    this.addReplIn('roots', '#TEAM1# total', '#TEAM1# total (incl. overtime)', '#TEAM1# тотал', '#TEAM1# тотал (вкл. овертайм)');
                } else if (bet.market === 'T2_TOTAL') {
                    this.addReplIn('roots', '#TEAM2# total', '#TEAM2# total (incl. overtime)', '#TEAM2# тотал', '#TEAM2# тотал (вкл. овертайм)');
                }
            }
        };
        params.proceed_hockey = function (bet) {
            const
                periods = ['First period - ', 'Second period - ', 'Third period - ', 'Первый период - ', 'Второй период - ', 'Третий период - '],
                period = periods[this.tDigit - 1];
            console.log(this.tDigit);
            if (bet.market === 'HDP' && parseFloat(bet.pivot) === 0) {
                this.addTo('roots', 'Draw no bet', 'Ничья ставки нет');
                this.addTo('pivotKeys', bet.target === 'HOME' ? '#TEAM1#' : '#TEAM2#');
            }
            ...
            if (!this.full) {
                ...
                if (bet.market === 'ONE_TWO') {
                    ...
                    this.addTotal('roots', [period + draw]);
                    this.addTotal('pivotKeys', [bet.target === 'HOME' ? '#TEAM1#' : '#TEAM2#']);
                } else if (bet.market === 'HDP') {
                    this.addReplIn('roots', 'Handicap', period + 'handicap', 'Фора', period + 'фора');
                } else if (bet.market === 'TOTAL') {
                    this.addReplIn('roots', 'Total', period + 'total', 'Тотал', period + 'тотал');
                } 
            }
        };
        params.proceed_cybersport = function (bet) {
            const maps = ['First map - ', 'Second map - ', 'Third map - ', 'Первая карта - ', 'Вторая карта - ', 'Третья карта - ']
            const map = maps[this.tDigit - 1];
            if (!this.full) {
                if (bet.market === 'ONE_TWO') {
                    this.addReplIn('roots', '1x2', map + 'winner (incl. overtime)', '1x2', map + 'победитель (вкл. овертайм)');
                } else if (bet.market === 'HDP') {
                ...
                if (bet.market === 'ONE_TWO') {
                    this.addReplIn('roots', '1x2', 'Winner', '1x2', 'Победитель');
                } else if (bet.market === 'HDP') {
                    this.addReplIn('roots', 'Handicap', 'Map handicap');
                } else if (bet.market === 'TOTAL') {
                    this.addReplIn('roots', 'Total', 'Total maps', 'Тотал', 'Тотал карт');
                }
            }
        };
        ...
        const $findPivot = $root => {
            ...
            for (const pvt of m.pivotKeys) {
                ...
                let $pivot = $([]);
                $pivot = $root.find(`div[data-editor-id="tableOutcomePlateName"]:textEqualsI("${pvt}")`);
                if ($pivot.length === 1) {
                    $res = $pivot;
                    break;
                } else {
                    console.log(`Not found! (pivot length ${$pivot.length} for ${pvt})`);
                }
            }
            return $res;
        };
        ...
        dLog('green', bkName, ['Final market is:', m]);
        ...
        for (const tab of m.tabs) {
            ...
            for (const root of m.roots) {
                ...
                const $root = () => $(shadowRoot())
                    .find('div[data-editor-id="tableMarketWrapper"]')
                    .toArray()
                    .find(el => 
                        $(el).find(`div:first-child > div:first-child > div:eq(1):textEqualsI("${root}")`).length > 0
                    );
                if ($($root()).length === 0) {
    ...
    const ProccedExpressNew = async () => {
        ...
        if (document.location.href.indexOf('event-builder') === -1) {
            ...
            if (builderBtnExist.length === 0) {
                window.location.href = 'https://betfury.ac/sports';
        ...
        const findOption = coef => {
            return isNaN(coef) ? false : coef >= 1.15 && coef <= 1.7;
        };
        ...
        do {
            ...
            if (eventName.length < 5) {
                dLog('red', bkName, `'${eventName}' is too short - ${eventName.length}`);
                continue;
            }

            if (used[eventName] >= 1) {
                dLog('big-yellow', bkName, `${eventName} used ${used[eventName]} times!`);
                continue;
            }
        ...
        if ($coupons().length === settings.newExpressBetsAmount 
            ...
            dLog('green', bkName, [`We get selected bets: '${currentBets}', now used:`, used]);
    ...
    const proceedBet = async (data, command) => {
        ...
        if (command === 'BET' && !!currentBetData.data[0].betFromParser) {
            ...
            if (!await eventsWork('betfury', settings, eventName, false, true)) {
                throw `Max amount of bets ${settings.eventMaxBets} to ${eventName} already done!`;
            } else {
                dLog('big-blue', bkName,
        ...
        do {
            ...
            dLog('green', bkName, `Will place (performBet): ${place}`);
            ...
            if (parseFloat($input().val()) !== parseFloat(place)) {
                ...
                await delayPromise(555);
                dLog('green', bkName, `STAKE entered ${willPlace}`);
            }

            const entered = parseFloat($input().val());
            dLog('green', bkName, `After enter stake check: willPlace = ${willPlace}, entered: ${entered}`);
            if (isNaN(entered) || willPlace !== entered) {
                dLog('red', bkName, 'Entered !== willPlace - try to reenter!');
                continue;
            }
    ...
    const commands = new class commands {
        ...
        async prepareResult(command, res) {
            dLog('', bkName, [`prepareResult '${command}':`, res, currentBetData,]);
            if (['BET', 'EXPRESS_BET'].indexOf(command) > -1) {
                if (res.success) {
                    await eventsWork(bkHere, settings,
    ...
    const messageProcessor = (message, direct) => {
        ...
        if (limited) {
            ...
            dLog(`yellow`, bkName, [`messageProcessor auth settings:`, settings,]);
            authCheck();
            wasAuthCheck = true;
