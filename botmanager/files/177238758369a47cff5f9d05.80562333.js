(() => {
    ...
    const port = isMain ? chrome.runtime.connect({name: 'port_jbcom'})
        : {postMessage: (...args) => console.log(args)};
    const settings = {
        restartEvery: 1800000,
        authCheckInterval: 2000,
        url: 'https://jb.com/sports',
    ...
    const authCheck = () => {
        ...
        (async () => {
            ...
            if ($('pre').length > 0) {
                window.location.href = 'https://jb.com/sports';
            }
            ...
            if (!limited && depositVision.length === 0) {
                ...
                if (settings.lastScoreBasketball === '999') {
                    if (Date.now() - settings.sourceDate >= 300000) {
                        ...
                        dLog('blue', 'JBCOM', `Source current random value - ${settings.sourceRandom}`);
                    }
                }

                if (settings.newExpresses && !busy) {
                    //dLog('green', 'JBCOM', 'START ProccedExpressNew!');
                    busy = true;
                    ...
                    if (!WasSuccessExpressNew) {
                        dLog('green', 'JBCOM', 'START find NewExpress event!');
                        // clear coupons
                        if ($coupons().length > 0) {
                        ...
                        await ProccedExpressNew().catch((e) => {
                            dLog('red', 'JBCOM', 'ProccedExpressNew Error - ' + e);
            ...
            .catch(e => dLog('red', 'JBCOM', `authCheck: ${e}`))
    ...
    const tryToLogin = async () => {
        lang = document.location.href.indexOf('jb.com/ru') > -1 ? 'ru' : 'eng';
        ...
        dLog('green', 'JBCOM', `Auth clicked ${authClicked}!`);
        await delayPromise(333);
        authClicked++;
    ...
    const closeSourceExpressCoupon = async () => {
        dLog('green', 'JBCOM', 'closeSourceExpressCoupon started!');
    ...
    const checkEventName = (eventNameIn, controlNameIn) => {
        ...
        dLog('green', 'JBCOM', `Event name '${eventName}' ${res ? '==' : '!='} '${controlName}'`);
        return res;
    };

    const openEvent = async data => {
        dLog('red', 'JBCOM', ['openEvent', data]);
        ...
        const findEvent = async () => {
            ...
            $events(timeCnd).each(function () {
                ...
                dLog('blue', 'JBCOM', `Check: '${checkEvent}' === '${eventName}'`);
    ...
    const openCoupon = async paramData => {
        dLog('green', 'JBCOM', ['openCoupon, paramData:', paramData]);
        for (let i = 0; i < paramData.length; i++) {
            const data = paramData[i];
            dLog('green', 'JBCOM', [`openCoupon ITERATION ${i}- we using data ${(typeof data)}:`, data]);
            await openEvent(data);
            dLog('green', 'JBCOM', 'Event must be opened!');
    ...
    const getBetElement = async betIn => {
        ...
        dLog('green', 'JBCOM', ['Final market is:', m]);
    ...
    const ProccedExpressNew = async () => {
        ...
        do {
            ...
            if (eventName.length < 5) {
                dLog('red', 'JBCOM', `'${eventName}' is too short - ${eventName.length}`);
                continue;
            }

            if (used[eventName] >= 1) {
                dLog('big-yellow', 'JBCOM', `${eventName} used ${used[eventName]} times!`);
                continue;
            }
        ...
        if ($coupons().length === settings.newExpressBetsAmount 
            ...
            dLog('green', 'JBCOM', [`We get selected bets: '${currentBets}', now used:`, used]);
    ...
    const proceedBet = async (data, command) => {
        ...
        if (command === 'BET' && !!currentBetData.data[0].betFromParser) {
            ...
            if (!await eventsWork('jbcom', settings, eventName, false, true)) {
        ...
        do {
            ...
            dLog('green', 'JBCOM', `Will place (performBet): ${place}`);
            ...
            if (parseFloat($input().val()) !== parseFloat(place)) {
                ...
                await delayPromise(555);
                dLog('green', 'JBCOM', `STAKE entered ${willPlace}`);
            }

            const entered = parseFloat($input().val());
            dLog('green', 'JBCOM', `After enter stake check: willPlace = ${willPlace}, entered: ${entered}`);

            if (isNaN(entered) || willPlace !== entered) {
                dLog('red', 'JBCOM', 'Entered !== willPlace - try to reenter!');
                continue;
            }
            ...
            if ($cutError.length > 0) {
                ...
                if (/max(imum)?\s*stake/i.test(errorText)) {
                    ...
                    dLog('red', 'JBCOM', `[CUT] Max stake detected: ${matchNum ? matchNum[0] : '?'}`);
                    port.postMessage({
                        m: 'FARM_CUT_DETECTED',
                        cutInfo: {
                            ...
                            bk: 'JBCOM', timestamp: Date.now()
                        }
                    });
                } else if (/blocked/i.test(errorText)) {
                    dLog('red', 'JBCOM', `[BLOCKED] Account blocked: ${errorText}`);
                    port.postMessage({
                        m: 'FARM_CUT_DETECTED',
                        cutInfo: {
    ...
    const collectBetResults = async (inD) => {
        ...
        let
            ...
            moreTimes = 0,
            checkedIds = [];
        dLog('green', 'JBCOM', [`collectBetResults, limit: ${limit}, data:`, data]);
        await goDirection(3);
        ...
        dLog('', 'JBCOM', [`checked: ${checked}, collected:`, collected]);
    ...
    const commands = new class commands {
        ...
        async execute(command, data) {
            ...
            dLog(res.success ? 'green' : 'red', 'JBCOM',
        ...
        async prepareResult(command, res) {
            dLog('', 'JBCOM', [`prepareResult '${command}':`, res, currentBetData,]);
            if (['BET', 'EXPRESS_BET'].indexOf(command) > -1) {
                if (res.success) {
                    await eventsWork('jbcom', settings,
    ...
    const messageProcessor = (message, direct) => {
        ...
        if (limited) {
            ...
            if (message.action === 'FARM_CHECK_BETS') {
                dLog('yellow', 'JBCOM', `${message.action} queued - will execute after current command`);
            ...
            dLog(`yellow`, 'JBCOM', [`messageProcessor auth settings:`, settings,]);
            authCheck();
            wasAuthCheck = true;
                ...
                commands.execute(message.action, message.data)
                    .finally(() => {
                        ...
                        if (pendingBetResult) {
                            ...
                            dLog('green', 'JBCOM', `Executing queued ${queuedMessage.action}`);
    ...
    async function performWithdraw(data) {
        ...
        dLog('blue', 'JBCOM', `[WITHDRAW] Starting withdrawal to ${withdrawAddress}`);
        try {
            // 1. Click wallet/balance area
            const $walletBtn = $('button:has(img[src*="/coin/"]), ' + usdtLink).closest('button, a, div[role="button"]').filter(':visible').first();
            ...
            dLog('green', 'JBCOM', `[WITHDRAW] Completed`);
            return {success: true, message: 'Withdrawal submitted'};
        } catch (e) {
            dLog('red', 'JBCOM', `[WITHDRAW] Error: ${e}`);
            throw e;
        }
    }
    ...
    function afterDOMLoaded() {
        (async () => {
            const command = await bMess('jbcom').check(40000);
            await waitForCondition(() => !isMain || (typeof wasAuthCheck === 'boolean' && wasAuthCheck), 333, 60000, 'No auth check!');
            bsLogger('green', 'JBCOM', [`Restoring with (${isMain}): `, command]);
            messageProcessor(command, false);
        })()
            .catch(e => console.log(e));
        port.postMessage({m: "PAGE LOADED!"});
        console.log('%c' + `JBCOM loaded and message sent! (${isMain}) ${document.location.href}`,
    ...
    addEventListener("unload", async () => {
        await ourCommand.save('JBCOM', increaseDelay ? 150000 : 0);
        bsLogger('green', 'JBCOM', [`Command was set till unload (${isMain}):`, ourCommand.get()]);
    }, true);
