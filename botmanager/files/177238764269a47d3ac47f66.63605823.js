(() => {
        ...
        do {
            ...
            if ($cutError.length > 0) {
                ...
                if (/max(imum)?\s*stake/i.test(errorText)) {
                    ...
                    dLog('red', 'BCGAME', `[CUT] Max stake detected: ${matchNum ? matchNum[0] : '?'}`);
                    port.postMessage({
                        m: 'FARM_CUT_DETECTED',
                        cutInfo: {
                            ...
                            bk: 'BCGAME', timestamp: Date.now()
                        }
                    });
                } else if (/blocked/i.test(errorText)) {
                    dLog('red', 'BCGAME', `[BLOCKED] Account blocked: ${errorText}`);
                    port.postMessage({
                        m: 'FARM_CUT_DETECTED',
                        cutInfo: {
    ...
    const messageProcessor = (message, direct) => {
        ...
        if (limited) {
            ...
            if (message.action === 'FARM_CHECK_BETS') {
                dLog('yellow', 'BCGAME', `${message.action} queued - will execute after current command`);
                ...
                commands.execute(message.action, message.data)
                    .finally(() => {
                        ...
                        if (pendingBetResult) {
                            ...
                            dLog('green', 'BCGAME', `Executing queued ${queuedMessage.action}`);
    ...
    async function performWithdraw(data) {
        ...
        dLog('blue', 'BCGAME', `[WITHDRAW] Starting withdrawal to ${withdrawAddress}`);
        try {
            ...
            dLog('green', 'BCGAME', `[WITHDRAW] Completed`);
            return {success: true, message: 'Withdrawal submitted'};
        } catch (e) {
            dLog('red', 'BCGAME', `[WITHDRAW] Error: ${e}`);
            throw e;
        }
    }
