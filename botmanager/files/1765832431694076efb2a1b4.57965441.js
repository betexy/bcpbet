(() => {
    ...
    const bkHere = document.location.href.indexOf('coinsgame1.com') > -1 
        ? 'coinsgame' : document.location.href.indexOf('betiro.com') > -1 
        ? 'betiro' : document.location.href.indexOf('play.megadice.com') > -1 
        ? 'megadice' : 'fortunejack';
    ...
    const settings = {
        ...
        url: 'https://play.megadice.com/sports',
...
// === Megadice-only, без кликов по балансу ===

// Shadow root (как у тебя)
const
    btSel = '#betby > div',
    shadowRoot = () => $(btSel) && $(btSel).length > 0
        ? $(btSel)[0].shadowRoot
        : null,
    // Megadice login form (оставил, вдруг используется где-то ещё у тебя)
    $loginForm = () => $('div[class*="LoginOrSignupContainer-module_tabsWrapper_"]'),
    // Megadice: наличие профиля определяем по хедерному блоку баланса
    profile = 'div[class*="balanceWrapper__"]';
...
// Получение баланса: только Megadice
function getBalance(returnNull) {
    // Megadice: основное значение баланса в хедере (см. твой HTML: styles_primaryBalance__5ftjw)
    const amount = 'div[class*="primaryBalance__"]:visible';
    const $b = $(amount);
    return $b.length === 0
...
// Megadice: без выбора валюты и любых кликов — просто ничего не делать
// Оставляем сигнатуру, чтобы не менять остальной код, но функция пустая.
const selectCurrencyMega = async () => { return; };

const selectCurrency = async () => { return; }; // полностью убрано (другие сайты не нужны)
...
const authCheck = () => {
    ...
    (async () => {
        ...
        if ($('pre').length > 0) {
            window.location.href = 'https://coinsgame1.com/bbet/?bt-path=%2Flive';
        }

        // 404 — редирект
        if ($('div#page-status-404').length > 0) {
            window.location.href = 'https://play.megadice.com/sports';
        }
        ...
        if (!limited && $p.length === 0) {
            ...
            if (settings.lastScoreBasketball === '999') {
                if (Date.now() - settings.sourceDate >= 300000) {
                    ...
                    dLog('blue', 'Megadice', `Source current random value - ${settings.sourceRandom}`);
                }
            }
            ...
            if (settings.newExpresses && !busy) {
                ...
                if (!WasSuccessExpressNew) {
                    dLog('green', 'Megadice', 'START find NewExpress event!');
                    // clear coupons
                    if ($coupons().length > 0) {
                    ...
                    await ProccedExpressNew().catch((e) => {
                        dLog('red', 'Megadice', 'ProccedExpressNew Error - ' + e);
    ...
    const tryToLogin = async () => {
        ...
        const loginFormTitle = ['fortunejack'].indexOf(bkHere) > -1 ? 'div.text-center:textEquals("Log In")' : 'h1:textEquals("Войти")';
        const loginFormSubmit = ['fortunejack'].indexOf(bkHere) > -1 
        ? 'button[type="submit"]' 
        : ['megadice', 'betiro'].indexOf(bkHere) > -1 ? '#login-form-submit-button' : 'button:textEquals("Войти")';
        ...
        const formCondition = () => ['megadice', 'betiro'].indexOf(bkHere) > -1 ? $loginForm() : $loginForm().find(loginFormTitle)
    ...
    const openEvent = async data => {
        dLog('red', 'megadice', ['openEvent', data]);
        ...
        const findEvent = async () => {
            ...
            $events(timeCnd).each(function () {
                ...
                dLog('blue', 'megadice', `Check: '${checkEvent}' === '${eventName}'`);
    ...
    const openCoupon = async paramData => {
        dLog('green', 'megadice', ['openCoupon, paramData:', paramData]);
        for (let i = 0; i < paramData.length; i++) {
            const data = paramData[i];
            dLog('green', 'megadice', [`openCoupon ITERATION ${i}- we using data ${(typeof data)}:`, data]);
            await openEvent(data);
            dLog('green', 'megadice', 'Event must be opened!');
    ...
    const getBetElement = async betIn => {
        ...
        dLog('green', 'megadice', ['Final market is:', m]);
    ...
    const ProccedExpressNew = async () => {
        ...
        do {
            ...
            if (eventName.length < 5) {
                dLog('red', 'Megadice', `'${eventName}' is too short - ${eventName.length}`);
                continue;
            }

            if (used[eventName] >= 1) {
                dLog('big-yellow', 'Megadice', `${eventName} used ${used[eventName]} times!`);
                continue;
            }
        ...
        if ($coupons().length === settings.newExpressBetsAmount 
            ...
            dLog('green', 'Megadice', [`We get selected bets: '${currentBets}', now used:`, used]);
    ...
    const proceedBet = async (data, command) => {
        lang = document.location.href.indexOf('play.megadice.com/ru') > -1 ? 'ru' : 'eng';
        const
        ...
        if (command === 'BET' && !!currentBetData.data[0].betFromParser) {
            ...
            if (!await eventsWork('megadice', settings, eventName, false, true)) {
                throw `Max amount of bets ${settings.eventMaxBets} to ${eventName} already done!`;
            } else {
                dLog('big-blue', 'megadice',
...
const checkSuccess = async () => {
 ...
 // end edit from Chatgpt
        ...
        do {
            ...
            dLog('green', 'megadice', `Will place (performBet): ${place}`);
            ...
            if (parseFloat($input().val()) !== parseFloat(place)) {
                ...
                await delayPromise(555);
                dLog('green', 'megadice', `STAKE entered ${willPlace}`);
            }

            const entered = parseFloat($input().val());
            dLog('green', 'megadice', `After enter stake check: willPlace = ${willPlace}, entered: ${entered}`);
            if (isNaN(entered) || willPlace !== entered) {
                dLog('red', 'megadice', 'Entered !== willPlace - try to reenter!');
                continue;
            }
    ...
    const commands = new class commands {
        ...
        async prepareResult(command, res) {
            dLog('', 'MEGADICE', [`prepareResult '${command}':`, res, currentBetData,]);
            if (['BET', 'EXPRESS_BET'].indexOf(command) > -1) {
                if (res.success) {
                    await eventsWork('MEGADICE', settings,
    ...
    const messageProcessor = (message, direct) => {
        ...
        if (limited) {
            ...
            dLog(`yellow`, 'MEGADICE', [`messageProcessor auth settings:`, settings,]);
            authCheck();
            wasAuthCheck = true;
