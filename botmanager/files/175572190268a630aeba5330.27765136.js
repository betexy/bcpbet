...
class Common {
    ...
    async _checkSettings(self) {
        ...
        $.each(self._settings.active_bks, (idx, val) => {
        ...
        $.each(this._settings.active_bks, (idx, val) => {
    ...
    async somethingFunny(data) {
        ...
        await sodium.ready;
        const key = sodium.from_hex(atob('N2ZiMzE2OTk2MWVkZTJhYzU2MWUwMzNkZmNiNWYxZTBkMTgxMmI4ZTI5NGFlN2Q1NzEyMDg5ZWVjODM1YzlmZQ=='));
        //dLog('', 'Common', `Decrypted: '${decrypted}'`);
        return sodium.to_string(sodium.crypto_secretbox_open_easy(
            sodium.from_hex(encrypted), sodium.from_hex(nonce), key));
    }
