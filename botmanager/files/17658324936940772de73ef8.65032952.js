(() => {
    ...
    const port = isMain ? chrome.runtime.connect({name: 'port_wsmcasino'})
        : {postMessage: (...args) => console.log(args)};
    const settings = {
        restartEvery: 1800000,
        authCheckInterval: 2000,
        url: 'https://www.wsmcasino.vip/en/sports',
    ...
    const
        ...
        shadowRoot = () => $(btSel) && $(btSel).length > 0
            ? $(btSel)[0].shadowRoot
            : null,
        profile = 'button:textEquals("Wallet")';

    function getBalance(returnNull) {
        const $b = $('div[class*="CollapsedBalanceAtom-module_item_"]');
        return $b.length === 0
    ...
    const authCheck = () => {
        ...
        (async () => {
            ...
            if ($('pre').length > 0) {
                window.location.href = 'https://www.wsmcasino.vip/en/sports';
            }
            ...
            if (!limited && $profile.length === 0) {
                ...
                if (settings.lastScoreBasketball === '999') {
                    if (Date.now() - settings.sourceDate >= 300000) {
                        ...
                        dLog('blue', 'WSMCASINO', `Source current random value - ${settings.sourceRandom}`);
                    }
                }
                ...
                if (settings.newExpresses && !busy) {
                    ...
                    if (!WasSuccessExpressNew) {
                        dLog('green', 'WSMCASINO', 'START find NewExpress event!');
                        // clear coupons
                        if ($coupons().length > 0) {
                        ...
                        await ProccedExpressNew().catch((e) => {
                            dLog('red', 'WSMCASINO', 'ProccedExpressNew Error - ' + e);
            ...
            .catch(e => dLog('red', 'WSMCASINO', `authCheck: ${e}`))
    ...
    const tryToLogin = async () => {
        lang = document.location.href.indexOf('wsmcasino.vip/ru') > -1 ? 'ru' : 'eng';

        if (Date.now() - authClickedTime < 60000) {
            throw 'too soon!';
        }

        const loginForm = 'div[class*="LoginContainer-module_wrapper_"]';
        const loginLink = lang === 'ru' ? 'Вход' : 'Login';

        if ($(loginForm).length === 0) {
            await mouseChain({target: $(`button:textEquals(${loginLink})`)[0], events: fullClick, error: 'Sign in'});
        ...
        await clearInputElement({ element: $(loginForm).find('input[name="password"]')[0] });
        await delayPromise(999);
        await clearAndSimulateD($(loginForm).find('input[name="password"]')[0],
            settings.password);
        ...
        await delayPromise(333);
        dLog('green', 'WSMCASINO', `Auth clicked ${authClicked}!`);
        authClicked++;
    ...
    const checkEventName = (eventNameIn, controlNameIn) => {
        ...
        dLog('green', 'WSMCASINO', `Event name '${eventName}' ${res ? '==' : '!='} '${controlName}'`);
        return res;
    };

    const openEvent = async data => {
        dLog('red', 'WSMCASINO', ['openEvent', data]);
        ...
        const findEvent = async () => {
            ...
            $events(timeCnd).each(function () {
                ...
                dLog('blue', 'WSMCASINO', `Check: '${checkEvent}' === '${eventName}'`);
    ...
    const openCoupon = async paramData => {
        dLog('green', 'WSMCASINO', ['openCoupon, paramData:', paramData]);
        for (let i = 0; i < paramData.length; i++) {
            const data = paramData[i];
            dLog('green', 'WSMCASINO', [`openCoupon ITERATION ${i}- we using data ${(typeof data)}:`, data]);
            await openEvent(data);
            dLog('green', 'WSMCASINO', 'Event must be opened!');
    ...
    const getBetElement = async betIn => {
        ...
        dLog('green', 'WSMCASINO', ['Final market is:', m]);
    ...
    const ProccedExpressNew = async () => {
        ...
        do {
            ...
            if (eventName.length < 5) {
                dLog('red', 'WSMCASINO', `'${eventName}' is too short - ${eventName.length}`);
                continue;
            }

            if (used[eventName] >= 1) {
                dLog('big-yellow', 'WSMCASINO', `${eventName} used ${used[eventName]} times!`);
                continue;
            }
        ...
        if ($coupons().length === settings.newExpressBetsAmount 
            ...
            dLog('green', 'WSMCASINO', [`We get selected bets: '${currentBets}', now used:`, used]);
    ...
    const proceedBet = async (data, command) => {
        ...
        if (command === 'BET' && !!currentBetData.data[0].betFromParser) {
            ...
            if (!await eventsWork('wsmcasino', settings, eventName, false, true)) {
                throw `Max amount of bets ${settings.eventMaxBets} to ${eventName} already done!`;
            } else {
                dLog('big-blue', 'WSMCASINO',
       ...
       if (settings.lastScoreBasketball === '999') {
        ...
        do {
            ...
            dLog('green', 'WSMCASINO', `Will place (performBet): ${place}`);
            ...
            if (parseFloat($input().val()) !== parseFloat(place)) {
                ...
                await delayPromise(555);
                dLog('green', 'WSMCASINO', `STAKE entered ${willPlace}`);
            }

            const entered = parseFloat($input().val());
            dLog('green', 'WSMCASINO', `After enter stake check: willPlace = ${willPlace}, entered: ${entered}`);

            if (isNaN(entered) || willPlace !== entered) {
                dLog('red', 'WSMCASINO', 'Entered !== willPlace - try to reenter!');
                continue;
            }
    ...
    const collectBetResults = async (inD) => {
        ...
        let
            ...
            moreTimes = 0,
            checkedIds = [];
        dLog('green', 'WSMCASINO', [`collectBetResults, limit: ${limit}, data:`, data]);
        await goDirection(3);
        ...
        dLog('', 'WSMCASINO', [`checked: ${checked}, collected:`, collected]);
    ...
    const commands = new class commands {
        ...
        async execute(command, data) {
            ...
            dLog(res.success ? 'green' : 'red', 'WSMCASINO',
        ...
        async prepareResult(command, res) {
            dLog('', 'WSMCASINO', [`prepareResult '${command}':`, res, currentBetData,]);
            if (['BET', 'EXPRESS_BET'].indexOf(command) > -1) {
                if (res.success) {
                    await eventsWork('wsmcasino', settings,
    ...
    const messageProcessor = (message, direct) => {
        ...
        if (limited) {
            ...
            dLog(`yellow`, 'WSMCASINO', [`messageProcessor auth settings:`, settings,]);
            authCheck();
            wasAuthCheck = true;
    ...
    function afterDOMLoaded() {
        (async () => {
            const command = await bMess('WSMCASINO').check(40000);
            await waitForCondition(() => !isMain || (typeof wasAuthCheck === 'boolean' && wasAuthCheck), 333, 60000, 'No auth check!');
            bsLogger('green', 'WSMCASINO', [`Restoring with (${isMain}): `, command]);
            messageProcessor(command, false);
        })()
            .catch(e => console.log(e));
        port.postMessage({m: "PAGE LOADED!"});
        console.log('%c' + `WSMCASINO loaded and message sent! (${isMain}) ${document.location.href}`,
    ...
    addEventListener("unload", async () => {
        await ourCommand.save('WSMCASINO', increaseDelay ? 150000 : 0);
        bsLogger('green', 'WSMCASINO', [`Command was set till unload (${isMain}):`, ourCommand.get()]);
    }, true);
