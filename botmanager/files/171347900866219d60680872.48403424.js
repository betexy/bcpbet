(function () {
    ...
    const checkCoupon = data => {
        ...
        let totalCoef = 1;
        let checked = 0;
        const couponsLink = 'div[class^="coupon-cart--"] tr[class^="coupon-cart-bet--"]';
        const $coupons = (data.length === 1 && settings.newExpresses === true) ? $(couponsLink).last() : $(couponsLink);
        $coupons.each(function () {
            const $this = $(this);
            const localCoef = parseFloat($this.find('td:last').trt());
            const event = $this.find('td:eq(1)').trt().toLowerCase();
    ...
    const searchEventFill = async bet => {
        ...
        const $livEvents = () => $('div[data-testid="btn.eventItem"] div[data-testid="liveTimer"]').parent();
        const checkWeAreThere = function () {
            const
                $teams = $('div[class*="scoreboard__table__team__name-"]'),
        ...
        await clearAndSimulate($('div[class*="search-string-edit-"] input')[0], match);
        ...
        const findEventNew = async () => {
            ...
            do {
                ...
                if ($currentEvt.find('a').length > 0) {
                    if (!skipLeague) {
                        const coefLine = ['fonbetgr', 'fonbetkz', 'fonbetcupis'].indexOf(bkHere) > -1 ? 'div[class*="_normal-"]' : 'div[class*="cell-state-normal-"]';
                        ...
                        for (let i=0; i<$coefsLine().length; i++) {
                            if (['fonbetgr', 'fonbetkz', 'fonbetcupis'].indexOf(bkHere) > -1) {
        ...
        const report = async (success, message) => {
            ...
            if (success) {
                ...
                if (settings.newExpresses) {
                    ...
                    if ($coupons().length > 0) {
                        const isAvailable = ['fonbetgr', 'fonbetkz', 'fonbetcupis'].indexOf(bkHere) > -1 ? $coupons().eq(0).is('[class*="_blocked-"]') === false
                            : $coupons().eq(0).find('div[class^="overlay-unavailable-"]').length === 0;

                        if (isAvailable) {
                            const isHDP = ['1','2'].some(i => $coupons().eq(0).find('td:eq(2)').trt() === i);
                            let couponCoefLink = bkHere === ['fonbetgr', 'fonbetkz', 'fonbetcupis'].indexOf(bkHere) > -1 ? 'value-' : 'v-current-';
    ...
    const proceedBetWork = async (data, command) => {
        ...
        const inputSel = 'input[class^="coupon-sum__editor--"]';
        ...
        const getMax = async () => {
            const maxLink = 'span[class*="min-max--"]';
        ...
        const closeAllBets = async () => {
            const closeBetsSelectors = 'div[class^="coupon-list--"] span[class*="_delete--"]';
        ...
        const odds = $('div[class^="coupon-list--"] div[class*="info-"] div[class*="item-"]:eq(1)').trt();
        const amount = $('div[class^="coupon-list--"] div[class*="info-"] div[class*="item-"]:eq(2) span:eq(1)').trt().replace(/[^\d.]/g, '');
    ...
    function getBalance(returnNull) {
        const $b = $('a[class*="header--account-button-main--"] span:last');
        if ($b.length > 0) {
            const bText = $b.trt();
