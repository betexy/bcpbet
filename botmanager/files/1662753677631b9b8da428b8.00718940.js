        ...
        const enterBet = async stake => {
            ...
            if (1 === 1) {
                ...
                if (parseFloat($(findSel(staked)).trt()) !== stake) {
                    ...
                    await delayPromise(500);
                    await bsType('bet365', stake.toString(), 100);
                    await delayPromise(1000);
                }
            } else {
            ...
            while (!placed && tries < 10) {
                ...
                if ($(findSel(wrapper)).length > 0) {
                    ...
                    await waitForElement(betPlaced,
                        30, 20000, false, 1, 'No Bet placed!');
                    betFinished(true, {
...
`;
    ...
    const openEvent = (data) => new Promise(function (onSuccess, onReject) {
        ...
        let result = function (status, message) {
            if (status) {
                ...
                }, 50, 30000, 'It looks like we are not there!')
                    //.then(checkScore)
                    .then(() => onSuccess())
                    .catch((e) => onReject(e));
            } else {
        ...
        const findEvent = async () => {
            const getScore = function (sport, $this) {
                let res = '';
                if (sport === 'FOOTBALL') {
                    res = $this.find('.ovm-StandardScoresSoccer_TeamOne ').trt() + ':'
                        + $this.find('.ovm-StandardScoresSoccer_TeamTwo').trt()
                    ...
                    res = $this.find('.ipo-TeamPoints_TeamScore-teamone').trt()
                ...
                dLog('red', '365', `getScore '${sport}' => '${res}'`);
                return res;
            };
            let $el = null;
        ...
        const findEventAtLeft = async () => {
            ...
            const leftScore = $el => {
                const $ss = $el.find('div.ipn-ScoresDefault_Score div.ipn-ScoresDefault_Score');
                if ($ss.length === 2) {
                    return $ss.eq(0).trt() + ':' + $ss.eq(1).trt();
                } else {
                    return 'NO SCORE!';
                }
            };
            ...
            const $l = $(`div[class$="CompetitionButton_Text "]:textEqualsIS("${data.league}")`);
            if ($l.length > 0) {
                ...
                if (!$el) {
                    ...
                    || ['CORNER_HDP', 'CORNER_TOTAL'].indexOf(data.market) > -1
                    || checkFixedScore(data.score, leftScore($el))) {
                    await dClick($el[0], true, 'Open event LEFT', false, true);
                    await delayPromise(50);
                    result(true, 'We must be on event page!');
                    return true;
                } else {
                    throw `SCORE_CHANGED: we need: ${data.score}, we have: ${getScore(data.sport, $el)}`;
                }
    ...
    const getBetElement = async data => {
        ...
        return new Promise(function (reportSuccess, reportReject) {
            ...
            dLog('', '365', 'Searchers: ' + printSearchers.join(' || '));
            ...
            const letsFind = async () => {
                await searchers.forEachAsyncBreakable(async searcher => {
                    ...
                    await searcher.roots.forEachAsyncBreakable(async root => {
                        await delayPromise(50);
                        const rootSelectors = [
                            `div[class$="MarketGroup "]:has(div[class$="MarketGroupButton_Text "]:textEqualsI("${root}"))`,
                            `div[class$="MarketGroup "]:has(span[class$="MarketGroupButton_Text "]:textEqualsI("${root}"))`,
                        ];
            ...
            (async () => {
                ...
                if (!globalFound) {
                    ...
                    //await saveHtml('b365-event', $('body').html());
                }
            })()
                .catch(e => onReject(e));
            //#-#-FINISH
