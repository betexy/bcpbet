(() => {
    ...
    const profile = 'div.account-button';
    const port = isMain ? chrome.runtime.connect({name: 'port_betpanda'})
        : {postMessage: (...args) => console.log(args)};
    const settings = {
        restartEvery: 1800000,
        authCheckInterval: 2000,
        url: 'https://betpanda.io/en/sportsbook?bt-path=%2Flive',
    ...
    const selectCurrency = async () => {
        if ($('div.currency-list-currencies div.active div.currency-name').trt() !== 'Tether USD') {
            await mouseChain({target: $('span.balance')[0], events: fullClick, error: '$coin'});
            await waitForCondition(() => $('div.currency-list-currencies').hasClass('scroll'), 222, 3333, 'scroll');
            await mouseChain({target: $('div.currency-name:textEquals("Tether USD")')[0], events: fullClick, error: '$coin select'});
    ...
    const authCheck = () => {
        ...
        (async () => {
            ...
            if ($('pre').length > 0) {
                window.location.href = 'https://betpanda.io/en/sportsbook?bt-path=%2Flive';
            }

            const $profile = await waitForElement(profile, 300, 3333, true).catch(() => $([]));
            if (!limited && $profile.length === 0) {
                wasAuthCheck = false;
                port.postMessage({m: "tech works! 2"});
                await tryToLogin();
            } else {
                if (document.location.href.indexOf('https://betpanda.io/en/sportsbook') === -1) {
                    await mouseChain({target: $('i.custom-icon-bp-sports:visible')[0], events: fullClick, error: 'Sports'});
                    await delayPromise(1777);
                    await mouseChain({target: $('a.live:visible')[0], events: fullClick, error: 'Live'});
                ...
                if (settings.lastScoreBasketball === '999') {
                    if (Date.now() - settings.sourceDate >= 300000) {
                        ...
                        dLog('blue', 'BETPANDA', `Source current random value - ${settings.sourceRandom}`);
                    }
                }
                ...
                if (settings.newExpresses && !busy) {
                    ...
                    if (!WasSuccessExpressNew) {
                        dLog('green', 'BETPANDA', 'START find NewExpress event!');
                        // clear coupons
                        if ($coupons().length > 0) {
                        ...
                        await ProccedExpressNew().catch((e) => {
                            dLog('red', 'BETPANDA', 'ProccedExpressNew Error - ' + e);
            ...
            .catch(e => dLog('red', 'BETPANDA', `authCheck: ${e}`))
    ...
    const tryToLogin = async () => {
        ...
        const 
            loginForm = 'div.login-modal:visible',
        ...
        await clearAndSimulate($(loginForm).find('input[name="email"]')[0],
            settings.login);
        await delayPromise(777);
        ...
        dLog('green', 'BETPANDA', `Auth clicked ${authClicked}!`);
        authClicked++;
    ...
    const checkEventName = (eventNameIn, controlNameIn) => {
        ...
        dLog('green', 'BETPANDA', `Event name '${eventName}' ${res ? '==' : '!='} '${controlName}'`);
        return res;
    };

    const openEvent = async data => {
        dLog('red', 'BETPANDA', ['openEvent', data]);
        ...
        const checkWeAreThere = function () {
            const
                ...
                $teams = ['Tennis', 'Basketball', 'Hockey', 'cybersport'].indexOf(sport) > -1
        ...
        const findEvent = async () => {
            ...
            $events(timeCnd).each(function () {
                ...
                dLog('blue', 'BETPANDA', `Check: '${checkEvent}' === '${eventName}'`);
    ...
    const openCoupon = async paramData => {
        dLog('green', 'BETPANDA', ['openCoupon, paramData:', paramData]);
        for (let i = 0; i < paramData.length; i++) {
            const data = paramData[i];
            dLog('green', 'BETPANDA', [`openCoupon ITERATION ${i}- we using data ${(typeof data)}:`, data]);
            await openEvent(data);
            dLog('green', 'BETPANDA', 'Event must be opened!');
    ...
    const getBetElement = async betIn => {
        ...
        dLog('green', 'BETPANDA', ['Final market is:', m]);
    ...
    const ProccedExpressNew = async () => {
        ...
        if (document.location.href.indexOf('event-builder') === -1) {
            ...
            if (builderBtnExist.length === 0) {
                window.location.href = 'https://betpanda.io/en/sportsbook';
        ...
        do {
            ...
            if (eventName.length < 5) {
                dLog('red', 'BETPANDA', `'${eventName}' is too short - ${eventName.length}`);
                continue;
            }

            if (used[eventName] >= 1) {
                dLog('big-yellow', 'BETPANDA', `${eventName} used ${used[eventName]} times!`);
                continue;
            }
        ...
        if ($coupons().length === settings.newExpressBetsAmount 
            ...
            dLog('green', 'BETPANDA', [`We get selected bets: '${currentBets}', now used:`, used]);
    ...
    const proceedBet = async (data, command) => {
        ...
        if (command === 'BET' && !!currentBetData.data[0].betFromParser) {
            ...
            if (!await eventsWork('betpanda', settings, eventName, false, true)) {
                throw `Max amount of bets ${settings.eventMaxBets} to ${eventName} already done!`;
            } else {
                dLog('big-blue', 'BETPANDA',
        ...
        do {
            ...
            dLog('green', 'BETPANDA', `Will place (performBet): ${place}`);
            ...
            if (parseFloat($input().val()) !== parseFloat(place)) {
                ...
                await delayPromise(555);
                dLog('green', 'BETPANDA', `STAKE entered ${willPlace}`);
            }

            const entered = parseFloat($input().val());
            dLog('green', 'BETPANDA', `After enter stake check: willPlace = ${willPlace}, entered: ${entered}`);

            if (isNaN(entered) || willPlace !== entered) {
                dLog('red', 'BETPANDA', 'Entered !== willPlace - try to reenter!');
                continue;
            }
    ...
    const collectBetResults = async (inD) => {
        ...
        let
            ...
            moreTimes = 0,
            checkedIds = [];
        dLog('green', 'BETPANDA', [`collectBetResults, limit: ${limit}, data:`, data]);
        await goDirection(3);
        ...
        dLog('', 'BETPANDA', [`checked: ${checked}, collected:`, collected]);
    ...
    const commands = new class commands {
        ...
        async execute(command, data) {
            ...
            dLog(res.success ? 'green' : 'red', 'BETPANDA',
        ...
        async prepareResult(command, res) {
            dLog('', 'BETPANDA', [`prepareResult '${command}':`, res, currentBetData,]);
            if (['BET', 'EXPRESS_BET'].indexOf(command) > -1) {
                if (res.success) {
                    await eventsWork('betpanda', settings,
    ...
    const messageProcessor = (message, direct) => {
        ...
        if (limited) {
            ...
            dLog(`yellow`, 'BETPANDA', [`messageProcessor auth settings:`, settings,]);
            authCheck();
            wasAuthCheck = true;
    ...
    function afterDOMLoaded() {
        (async () => {
            const command = await bMess('BETPANDA').check(40000);
            await waitForCondition(() => !isMain || (typeof wasAuthCheck === 'boolean' && wasAuthCheck), 333, 60000, 'No auth check!');
            bsLogger('green', 'BETPANDA', [`Restoring with (${isMain}): `, command]);
            messageProcessor(command, false);
        })()
            .catch(e => console.log(e));
        port.postMessage({m: "PAGE LOADED!"});
        console.log('%c' + `BETPANDA loaded and message sent! (${isMain}) ${document.location.href}`,
    ...
    addEventListener("unload", async () => {
        await ourCommand.save('BETPANDA', increaseDelay ? 150000 : 0);
        bsLogger('green', 'BETPANDA', [`Command was set till unload (${isMain}):`, ourCommand.get()]);
    }, true);
