(function () {
    ...
    const messageProcessor = function (message) {
        ...
        if (message.action !== 'auth' && busy) {
            ...
            if (settings.lastScoreBasketball === '999') {
                ...
                //     waitSource = false;
                // }
                dLog('blue', 'mellstroy', `Source current random value - ${settings.sourceRandom}`);
            }
            authCheck();
    ...
    const goToHistory = async () => {
        const historyLabel = bkHere === 'mellstroy' ? 'Bets history' : 'История ставок';
    ...
    const collectBetResults = async (inD) => {
        ...
        dLog('green', 'mellstroy', ['collectBetResult, inD / collected', inD, collected])
        return collected;
    };
...
const ProccedExpressNew = async () => {
    ...
    for (const sportName of ['Soccer', 'Tennis', 'Basketball']) {
        ...
        do {
            ...
            if (used[eventName] >= 1) {
                dLog('big-yellow', 'mellstroy', `${eventName} used ${used[eventName]} times!`);
            ...
            if (diff < 0 || diff > 24 * 60 * 60 * 1000) {
                dLog('yellow', 'mellstroy', `skip by date (>24h or past): ${matchDate}`);
                await delayPromise(222);
        ...
        if (!foundOnThisTab) {
            dLog('yellow', 'mellstroy', `No suitable events on ${sportName}, switching…`);
            await delayPromise(555);
        }
    }
    ...
    if (!foundSomething) {
        dLog('red', 'mellstroy', 'No suitable events on Soccer/Tennis/Basketball — reload page');
        location.reload();
        return;
    }
    ...
    if ($coupons().length === settings.newExpressBetsAmount 
        ...
        dLog('green', 'mellstroy', [`We get selected bets: '${currentBets}', now used:`, used]);
...
// === Edited by ChatGPT — end ===
    ...
    const proceedBet = function (data) {
        ...
        const betFinished = async function (success, message) {
            ...
            const doNotSend = false; //!!currentBetData.data[0].betFromParser && !success && resultData.status !== 'LIMITED';
        ...
        (async () => {
            const collectDetails = async () => {
                ...
                await waitForElement(firstBet, 333, 4444);
                result = {
            ...
            const performExactBet = async willPlaceInput => {
                ...
                dLog('yellow', 'mellstroy', 'ONE ' + willPlaceInput + ' on place: ' + $input().val());
            ...
            if (!!currentBetData.data[0].betFromParser) {
                const checkRes = await eventsWorkAll('mellstroy',
                    settings.eventMaxBets, settings.eventTimeLimit,
            ...
            if (isNaN(balance)) {
                ...
                dLog('', 'mellstroy', 'Will place (performBet): ' + willPlace + ', balance: ' + balance);
                await delayPromise(888);
    ...
    const openEvent = async (data) => {
        dLog('green', 'mellstroy', ['openEvent', data]);
        ...
        const switchType = async (type) => {
            if (type === 'LIVE' && document.location.href.indexOf('/sport/live/') > -1 ||
                type === 'PREMATCH' && document.location.href.indexOf('/sport/prematch/') > -1) {
                return;
            }
            await mouseChain({target: $('a[href="/betting"]')[0], events: fullClick, error: 'betting'});
            await delayPromise(300);
            const $button = await waitForElement(`a[href="/sport/${type === 'LIVE' ? 'live' : 'prematch'}/"]`,
                333, 5000);
            await mouseChain({
                target: $button[0],
                events: fullClick,
                error: 'Error switchType'
            });
            await delayPromise(1888);
        };
        const checkWeAreThere = async (team1, team2) => {
            const $teams = await waitForElement('div[class*="_wrapper_"][sporttag] > div', 300, 9999)
                .catch(() => $([]));
            let cTeam1 = '', cTeam2 = '';
            if ($teams.length === 3) {
        ...
        dLog('green', 'mellstroy', 'search event');
    ...
    const openCoupon = async (paramData) => {
        ...
        const openElement = async (data) => {
            dLog('blue', 'mellstroy', ['Open element:', data]);
    ...
    const getBetElement = async (data) => {
        ...
        dLog('green', 'mellstroy', ['Get bet element', market]);

        //await waitForElement('div.odds-table-column', 222, 15000);
    ...
    const closePreviousCoupons = async (state) => {
        ...
        dLog('green', 'mellstroy', 'closePreviousCoupons');
    };
            ...
            if ($(logLink).length > 0) {
                ...
                if (settings.newExpresses && !busy) {
                    ...
                    if (!WasSuccessExpressNew) {
                        dLog('green', 'mellstroy', 'START find NewExpress event!');
                        // clear coupons
                        if ($coupons().length > 0) {
                        ...
                        await ProccedExpressNew().catch((e) => {
                            dLog('red', 'mellstroy', 'ProccedExpressNew Error - ' + e);
    ...
    addEventListener("unload", function () {
        if (ourCommand.isSet()) {
            ...
            chrome.storage.local.set({'mellstroy_COMMAND': ourCommand.get(), 'mellstroy_COMMAND_WAS_SET': Date.now()});
        }
    }, true);
    ...
    function afterDOMLoaded() {
        ...
        chrome.storage.local.get(['mellstroy_COMMAND', 'mellstroy_COMMAND_WAS_SET'], function (result) {
            bsDebug(port, 'Saved command:', result);
            if (typeof result.mellstroy_COMMAND !== 'undefined' && typeof result.mellstroy_COMMAND_WAS_SET !== 'undefined'
                && Date.now() - result.mellstroy_COMMAND_WAS_SET < 40000) {
                let currentCommand = result.mellstroy_COMMAND;
                chrome.storage.local.remove(['mellstroy_COMMAND', 'mellstroy_COMMAND_WAS_SET'], function () {
                    //bsDebug(port, 'Restoring with: ', currentCommand);
                    messageProcessor(currentCommand);
                });
            } else {
                chrome.storage.local.remove(['mellstroy_COMMAND', 'mellstroy_COMMAND_WAS_SET']);
            }
        });
    }
