(() => {
    ...
    const port = isMain ? chrome.runtime.connect({name: 'port_chipsgg'})
        : {postMessage: (...args) => console.log(args)};
    const settings = {
        restartEvery: 1800000,
        authCheckInterval: 2000,
        url: 'https://chips.gg/sports?bt-path=/',
    ...
    function getBalance(returnNull) {
        const $b = $(findSel(['div:textEquals("USDT")', 'div:textEquals("USD")']));
        return $b.length === 0
    ...
    const selectCurrencyMega = async () => {
        if ($('img[alt="usdt"]').length === 0) {
            await mouseChain({target: $('img[src*="https://cdn.chips.gg/public/icons/currencies"]')[0], events: fullClick, error: 'Button balance bottom'});
            await delayPromise(1888);
            await mouseChain({target: $('img[src="https://cdn.chips.gg/public/icons/currencies/usdt.svg"]')[0], events: fullClick, error: 'USDT'});
        }
    };
    ...
    const authCheck = () => {
        ...
        (async () => {
            ...
            if (!limited && $loginLink.length > 0) {
                ...
                if (settings.lastScoreBasketball === '999') {
                    if (Date.now() - settings.sourceDate >= 300000) {
                        ...
                        dLog('blue', 'CHIPS.GG', `Source current random value - ${settings.sourceRandom}`);
                    }
                }
                ...
                if (settings.newExpresses && !busy) {
                    ...
                    if (!WasSuccessExpressNew) {
                        dLog('green', 'CHIPS.GG', 'START find NewExpress event!');
                        // clear coupons
                        if ($coupons().length > 0) {
                        ...
                        await ProccedExpressNew().catch((e) => {
                            dLog('red', 'CHIPS.GG', 'ProccedExpressNew Error - ' + e);
            ...
            .catch(e => dLog('red', 'CHIPS.GG', `authCheck: ${e}`))
    ...
    const tryToLogin = async () => {
        ...
        const profile = 'div[type="success"]';
        if ($loginForm().find('div:textEquals("Login")').length === 0) {
        ...
        await clearAndInputEmail($loginForm().find('input[type="email"]')[0],
            settings.login);
        ...
        await delayPromise(777);
        await mouseChain({
            target: $loginForm().find('div:textEquals("Sign in")')[0],
            events: fullClick,
            error: 'submit form'
        });
        ...
        dLog('green', 'CHIPS.GG', `Auth clicked ${authClicked}!`);
        if ($profile.length === 0) {
            port.postMessage({
                answered: "auth_error",
                status: "ERROR",
    ...
    const checkEventName = (eventNameIn, controlNameIn) => {
        ...
        dLog('green', 'CHIPS.GG', `Event name '${eventName}' ${res ? '==' : '!='} '${controlName}'`);
        return res;
    };

    const openEvent = async data => {
        dLog('red', 'CHIPS.GG', ['openEvent', data]);
        ...
        const findEvent = async () => {
            ...
            $events(timeCnd).each(function () {
                ...
                dLog('blue', 'CHIPS.GG', `Check: '${checkEvent}' === '${eventName}'`);
    ...
    const openCoupon = async paramData => {
        dLog('green', 'CHIPS.GG', ['openCoupon, paramData:', paramData]);
        for (let i = 0; i < paramData.length; i++) {
            const data = paramData[i];
            dLog('green', 'CHIPS.GG', [`openCoupon ITERATION ${i}- we using data ${(typeof data)}:`, data]);
            await openEvent(data);
            dLog('green', 'CHIPS.GG', 'Event must be opened!');
    ...
    const getBetElement = async betIn => {
        ...
        dLog('green', 'CHIPS.GG', ['Final market is:', m]);
    ...
    const ProccedExpressNew = async () => {
        ...
        if (document.location.href.indexOf('event-builder') === -1) {
            ...
            if (builderBtnExist.length === 0) {
                window.location.href = 'https://chips.gg/sports/';
        ...
        do {
            ...
            if (eventName.length < 5) {
                dLog('red', 'CHIPS.GG', `'${eventName}' is too short - ${eventName.length}`);
                continue;
            }

            if (used[eventName] >= 1) {
                dLog('big-yellow', 'CHIPS.GG', `${eventName} used ${used[eventName]} times!`);
                continue;
            }
        ...
        if ($coupons().length === settings.newExpressBetsAmount 
            ...
            dLog('green', 'CHIPS.GG', [`We get selected bets: '${currentBets}', now used:`, used]);
    ...
    const proceedBet = async (data, command) => {
        ...
        if (command === 'BET' && !!currentBetData.data[0].betFromParser) {
            ...
            if (!await eventsWork('CHIPS.GG', settings, eventName, false, true)) {
                throw `Max amount of bets ${settings.eventMaxBets} to ${eventName} already done!`;
            } else {
                dLog('big-blue', 'CHIPS.GG',
        ...
        do {
            ...
            dLog('green', 'CHIPS.GG', `Will place (performBet): ${place}`);
            ...
            if (parseFloat($input().val()) !== parseFloat(place)) {
                ...
                await delayPromise(555);
                dLog('green', 'CHIPS.GG', `STAKE entered ${willPlace}`);
            }

            const entered = parseFloat($input().val());
            dLog('green', 'CHIPS.GG', `After enter stake check: willPlace = ${willPlace}, entered: ${entered}`);
            if (isNaN(entered) || willPlace !== entered) {
                dLog('red', 'CHIPS.GG', 'Entered !== willPlace - try to reenter!');
                continue;
            }
    ...
    const collectBetResults = async (inD) => {
        ...
        let
            ...
            moreTimes = 0,
            checkedIds = [];
        dLog('green', 'CHIPS.GG', [`collectBetResults, limit: ${limit}, data:`, data]);
        await goDirection(3);
        ...
        dLog('', 'CHIPS.GG', [`checked: ${checked}, collected:`, collected]);
    ...
    const commands = new class commands {
        ...
        async execute(command, data) {
            ...
            dLog(res.success ? 'green' : 'red', 'CHIPS.GG',
        ...
        async prepareResult(command, res) {
            dLog('', 'CHIPS.GG', [`prepareResult '${command}':`, res, currentBetData,]);
            if (['BET', 'EXPRESS_BET'].indexOf(command) > -1) {
                if (res.success) {
                    await eventsWork('CHIPS.GG', settings,
    ...
    const messageProcessor = (message, direct) => {
        ...
        if (limited) {
            ...
            dLog(`yellow`, 'CHIPS.GG', [`messageProcessor auth settings:`, settings,]);
            authCheck();
            wasAuthCheck = true;
    ...
    function afterDOMLoaded() {
        (async () => {
            const command = await bMess('CHIPS.GG').check(40000);
            await waitForCondition(() => !isMain || (typeof wasAuthCheck === 'boolean' && wasAuthCheck), 333, 60000, 'No auth check!');
            bsLogger('green', 'CHIPS.GG', [`Restoring with (${isMain}): `, command]);
            messageProcessor(command, false);
        })()
            .catch(e => console.log(e));
        port.postMessage({m: "PAGE LOADED!"});
        console.log('%c' + `CHIPS.GG loaded and message sent! (${isMain}) ${document.location.href}`,
    ...
    addEventListener("unload", async () => {
        await ourCommand.save('CHIPS.GG', increaseDelay ? 150000 : 0);
        bsLogger('green', 'CHIPS.GG', [`Command was set till unload (${isMain}):`, ourCommand.get()]);
    }, true);
