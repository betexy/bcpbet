(() => {
    ...
    const port = isMain ? chrome.runtime.connect({name: 'port_csgo500'})
        : {postMessage: (...args) => console.log(args)};
    const settings = {
        restartEvery: 1800000,
        authCheckInterval: 2000,
        url: 'https://csgo500.com/sports',
    ...
    const
        ...
        shadowRoot = () => $(btSel) && $(btSel).length > 0
            ? $(btSel)[0].shadowRoot
            : null,
        $loginForm = () => $('div.auth-container'),
        loginLink = 'button.login';

    function getBalance(returnNull) {
        const $b = $('div.main-navbar-balance-dropdown span.base-counter');
        return $b.length === 0
    ...
    const selectCurrencyMega = async () => {
        if ($('button.balance-dropdown-trigger i.usdt').length === 0) {
            await mouseChain({target: $('button.balance-dropdown-trigger span i.ri-arrow-down-s-line')[0], events: fullClick, error: 'Button balance bottom'});
            await delayPromise(1888);
            await mouseChain({target: $('span.balance-name:textEquals("USDT")')[0], events: fullClick, error: 'USDT'});
        }
    };
    ...
    const authCheck = () => {
        ...
        (async () => {
            ...
            if (!limited && $loginLink.length > 0) {
                wasAuthCheck = false;
                port.postMessage({m: "технические работы! 2"});
                await tryToLogin();
            } else {
                ...
                if (settings.lastScoreBasketball === '999') {
                    if (Date.now() - settings.sourceDate >= 300000) {
                        ...
                        dLog('blue', 'CSGO500', `Source current random value - ${settings.sourceRandom}`);
                    }
                }
                ...
                if (settings.newExpresses && !busy) {
                    ...
                    if (!WasSuccessExpressNew) {
                        dLog('green', 'CSGO500', 'START find NewExpress event!');
                        // clear coupons
                        if ($coupons().length > 0) {
                        ...
                        await ProccedExpressNew().catch((e) => {
                            dLog('red', 'CSGO500', 'ProccedExpressNew Error - ' + e);
            ...
            .catch(e => dLog('red', 'CSGO500', `authCheck: ${e}`))
    ...
    const tryToLogin = async () => {
        ...
        const loginFormSubmit = 'button[type="submit"]';
        const profile = 'div.user-header span.user-title-name:visible';
        ...
        await clearAndSimulate($loginForm().find('input.input:eq(0)')[0],
            settings.login);
        await delayPromise(1000);
        await clearAndSimulate($loginForm().find('input.input:eq(1)')[0],
            settings.password);
        await delayPromise(1000);
        await mouseChain({
        ...
        dLog('green', 'CSGO500', `Auth clicked ${authClicked}!`);
        if ($profile.length === 0) {
            port.postMessage({
                answered: "auth_error",
                status: "ERROR",
    ...
    const checkEventName = (eventNameIn, controlNameIn) => {
        ...
        dLog('green', 'CSGO500', `Event name '${eventName}' ${res ? '==' : '!='} '${controlName}'`);
        return res;
    };

    const openEvent = async data => {
        dLog('red', 'CSGO500', ['openEvent', data]);
        ...
        const findEvent = async () => {
            ...
            $events(timeCnd).each(function () {
                ...
                dLog('blue', 'CSGO500', `Check: '${checkEvent}' === '${eventName}'`);
    ...
    const openCoupon = async paramData => {
        dLog('green', 'CSGO500', ['openCoupon, paramData:', paramData]);
        for (let i = 0; i < paramData.length; i++) {
            const data = paramData[i];
            dLog('green', 'CSGO500', [`openCoupon ITERATION ${i}- we using data ${(typeof data)}:`, data]);
            await openEvent(data);
            dLog('green', 'CSGO500', 'Event must be opened!');
    ...
    const getBetElement = async betIn => {
        ...
        dLog('green', 'CSGO500', ['Final market is:', m]);
    ...
    const ProccedExpressNew = async () => {
        ...
        if (document.location.href.indexOf('event-builder') === -1) {
            ...
            if (builderBtnExist.length === 0) {
                window.location.href = 'https://csgo500.com/sports';
        ...
        do {
            ...
            if (eventName.length < 5) {
                dLog('red', 'CSGO500', `'${eventName}' is too short - ${eventName.length}`);
                continue;
            }

            if (used[eventName] >= 1) {
                dLog('big-yellow', 'CSGO500', `${eventName} used ${used[eventName]} times!`);
                continue;
            }
        ...
        if ($coupons().length === settings.newExpressBetsAmount 
            ...
            dLog('green', 'CSGO500', [`We get selected bets: '${currentBets}', now used:`, used]);
    ...
    const proceedBet = async (data, command) => {
        ...
        if (command === 'BET' && !!currentBetData.data[0].betFromParser) {
            ...
            if (!await eventsWork('CSGO500', settings, eventName, false, true)) {
                throw `Max amount of bets ${settings.eventMaxBets} to ${eventName} already done!`;
            } else {
                dLog('big-blue', 'CSGO500',
        ...
        do {
            ...
            dLog('green', 'CSGO500', `Will place (performBet): ${place}`);
            ...
            if (parseFloat($input().val()) !== parseFloat(place)) {
                ...
                await delayPromise(555);
                dLog('green', 'CSGO500', `STAKE entered ${willPlace}`);
            }

            const entered = parseFloat($input().val());
            dLog('green', 'CSGO500', `After enter stake check: willPlace = ${willPlace}, entered: ${entered}`);
            if (isNaN(entered) || willPlace !== entered) {
                dLog('red', 'CSGO500', 'Entered !== willPlace - try to reenter!');
                continue;
            }
            ...
            if ($cutError.length > 0) {
                ...
                if (/max(imum)?\s*stake/i.test(errorText)) {
                    ...
                    dLog('red', 'CSGO500', `[CUT] Max stake detected: ${matchNum ? matchNum[0] : '?'}`);
                    port.postMessage({
                        m: 'FARM_CUT_DETECTED',
                        cutInfo: {
                            ...
                            bk: 'CSGO500', timestamp: Date.now()
                        }
                    });
                } else if (/blocked/i.test(errorText)) {
                    dLog('red', 'CSGO500', `[BLOCKED] Account blocked: ${errorText}`);
                    port.postMessage({
                        m: 'FARM_CUT_DETECTED',
                        cutInfo: {
    ...
    const collectBetResults = async (inD) => {
        ...
        let
            ...
            moreTimes = 0,
            checkedIds = [];
        dLog('green', 'CSGO500', [`collectBetResults, limit: ${limit}, data:`, data]);
        await goDirection(3);
        ...
        dLog('', 'CSGO500', [`checked: ${checked}, collected:`, collected]);
    ...
    const commands = new class commands {
        ...
        async execute(command, data) {
            ...
            dLog(res.success ? 'green' : 'red', 'CSGO500',
        ...
        async prepareResult(command, res) {
            dLog('', 'CSGO500', [`prepareResult '${command}':`, res, currentBetData,]);
            if (['BET', 'EXPRESS_BET'].indexOf(command) > -1) {
                if (res.success) {
                    await eventsWork('CSGO500', settings,
    ...
    const messageProcessor = (message, direct) => {
        ...
        if (limited) {
            ...
            if (message.action === 'FARM_CHECK_BETS') {
                dLog('yellow', 'CSGO500', `${message.action} queued - will execute after current command`);
            ...
            dLog(`yellow`, 'CSGO500', [`messageProcessor auth settings:`, settings,]);
            authCheck();
            wasAuthCheck = true;
                ...
                commands.execute(message.action, message.data)
                    .finally(() => {
                        ...
                        if (pendingBetResult) {
                            ...
                            dLog('green', 'CSGO500', `Executing queued ${queuedMessage.action}`);
    ...
    async function performWithdraw(data) {
        ...
        dLog('blue', 'CSGO500', `[WITHDRAW] Starting withdrawal to ${withdrawAddress}`);
        try {
            // 1. Click wallet/balance button
            const $walletBtn = $('div.main-navbar-balance-dropdown, button.balance-dropdown-trigger').filter(':visible').first();
            ...
            dLog('green', 'CSGO500', `[WITHDRAW] Completed`);
            return {success: true, message: 'Withdrawal submitted'};
        } catch (e) {
            dLog('red', 'CSGO500', `[WITHDRAW] Error: ${e}`);
            throw e;
        }
    }
    ...
    function afterDOMLoaded() {
        (async () => {
            const command = await bMess('CSGO500').check(40000);
            await waitForCondition(() => !isMain || (typeof wasAuthCheck === 'boolean' && wasAuthCheck), 333, 60000, 'No auth check!');
            bsLogger('green', 'CSGO500', [`Restoring with (${isMain}): `, command]);
            messageProcessor(command, false);
        })()
            .catch(e => console.log(e));
        port.postMessage({m: "PAGE LOADED!"});
        console.log('%c' + `CSGO500 loaded and message sent! (${isMain}) ${document.location.href}`,
    ...
    addEventListener("unload", async () => {
        await ourCommand.save('CSGO500', increaseDelay ? 150000 : 0);
        bsLogger('green', 'CSGO500', [`Command was set till unload (${isMain}):`, ourCommand.get()]);
    }, true);
